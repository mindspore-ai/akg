...
def split_to_l0A(src, split_size, split_axis, transpose=False):
    ...
    for i in range(len(split_size)):
        ...
        # slicesize provided
        dst = slice_move_instruction(
            src, "L0A", input_start, slize_size, instr="MOV", attrs=attrs)
        out.append(dst)
        input_start[split_axis] = split_size[i]
        if i < len(split_size) - 1:
            input_end[split_axis] = split_size[i + 1]
...
def slice_to_l0A(src, begin, slicesize, transpose=False):
    ...
    # slicesize provided
    dst = slice_move_instruction(
        src, "L0A", begin, slicesize, instr="MOV", attrs=attrs)
    return dst
...
def split_to_l0B(src, split_size, split_axis, transpose=False):
    ...
    for i in range(len(split_size)):
        ...
        # slicesize provided
        dst = slice_move_instruction(
            src, "L0B", input_start, slize_size, instr="MOV", attrs=attrs)
        out.append(dst)
        input_start[split_axis] = split_size[i]
        if i < len(split_size) - 1:
            input_end[split_axis] = split_size[i + 1]
...
def slice_to_l0B(src, begin, slicesize, transpose=False):
    ...
    if transpose:
        slicesize[-1], slicesize[-2] = slicesize[-2], slicesize[-1]
    dst = slice_move_instruction(
        src, "L0B", begin, slicesize, instr="MOV", attrs=attrs)
    return dst
...
def slice(src, begin, slicesize):
    ...
    # slicesize provided
    dst = slice_move_instruction(
        src, src.mem_type, begin, slicesize, instr="SLICE")
    return dst
