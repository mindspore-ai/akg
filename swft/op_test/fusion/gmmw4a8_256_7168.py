...
def i4toi8(y_int8):
    ...
    input_x = input_x.reshape((-1, 8, 56, 128, 32)).transpose(0,
                                                              2, 1, 3, 4).reshape((-1, 4, 8192)).transpose(0, 2, 1)
    input_x = input_x.reshape(
        (-1, 56, 8, 128, 32)).transpose(0, 2, 1, 3, 4).reshape(-1)
...
def grouped_matmul(x, w, group_list, x_scale, y_scale, out):
    ...
    l0b_tiling_size = 64
    l0b_tiling = K // l0b_tiling_size
    w = change_view(w, new_shape=[EXP, K // 32, 56, N // 56 * 32 // 4], new_dtype="INT16")
    for i in dynamic_loop(EXP):
        end_batch = move_to_scalar(ub_list[i])
        for j in dynamic_loop(end_batch - start_batch):
            x_ub = slice_to_ub(x, [start_batch + j, 0], [1, K])
            x_ub_nz = change_view(x_ub, new_format="NZ")
            x_ub_pad = pad_to_ub(x_ub_nz, [16, K])
            x_l1 = move_to_l1(x_ub_pad)
            ub_scale_x = slice_to_ub(x_scale, [start_batch + j], [1])
            for n in dynamic_loop(l1_tiling):
                w_ub = slice_to_ub(
                    w, [i, 0, block_idx * l1_tiling + n, 0], [1, K//32, 1, l1_tiling_size * 32 // 4])
                w_ub_nd = change_view(w_ub, [1*l1_tiling_size*K//4], "ND")
                w_ub_int8 = vconv_s42s8(w_ub_nd)
                w_ub_nz = change_view(w_ub_int8, [1, l1_tiling_size, K], "NZ")
                w_l1 = move_to_l1(w_ub_nz)
                ub_out = vector_dup(Scalar("FP16", 0), [
                                    1, l1_tiling_size], False)
                for k in range(l0b_tiling):
                    x_l0a = slice_to_l0A(
                        x_l1, [0, k * l0b_tiling_size], [16, l0b_tiling_size])
                    w_l0b = slice_to_l0B(w_l1, [
                                         0, 0, k * l0b_tiling_size], [1, l1_tiling_size, l0b_tiling_size], transpose=True)
                    ...
                    ub_y_scale = slice_to_ub(y_scale, [
                                             i, k, 0, block_idx * percore_size + n * l1_tiling_size], [1, 1, 1, l1_tiling_size])
                    ub_y_scale_cv = change_view(
                        ub_y_scale, [1, l1_tiling_size])
                    ub_l0c_fp16 = vconv(ub_l0c_quant, "FP16")
                    ub_l0c_nd = nz_to_nd(ub_l0c_fp16)
                    ub_l0c_slice = slice_to_ub(ub_l0c_nd, [0, 0, 0], [
                                               1, 1, l1_tiling_size])
                    ub_l0c_nd_1 = change_view(
                        ub_l0c_slice, [1, l1_tiling_size])
                    ub_out_mul = vmul(ub_l0c_nd_1, ub_y_scale_cv)
                    ub_out = vadd(ub_out, ub_out_mul)
                insert_to_gm(out, ub_out, [
                             start_batch + j,  block_idx * percore_size + n * l1_tiling_size], [1, l1_tiling_size])
...
if __name__ == "__main__":
    ...
    gm_weight = Tensor(
        "GM", "INT4", [EXP, K // 32, 56, N // 56 * 32], "ND", False)
