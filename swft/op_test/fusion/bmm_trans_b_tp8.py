...
def gen_golden_data():
    ...
    golden = np.matmul(input0, input1.transpose(0, 2, 1)).astype(np.float16)
...
def batch_matmul_tp8(gm_a, gm_w, gm_out):
    idx = get_block_idx()
    ...
    for i in range(BS):
        ...
        for j in range(N // 16):
            ...
            ub_c_nz = slice_to_ub(ub_c, [0, 0, 0], [B // CORE_NUM, 1, 16])
            ub_c_nd = nz_to_nd(ub_c_nz)
            ub_c_nd = change_view(ub_c_nd, [1, B // CORE_NUM, 16])
            insert_to_gm(gm_out, ub_c_nd, [i, B // CORE_NUM * idx, 16 * j], [1, B // CORE_NUM, 16])
...
if __name__ == "__main__":
    ...
    batch_matmul_tp8(gm_a, gm_w, gm_out)
