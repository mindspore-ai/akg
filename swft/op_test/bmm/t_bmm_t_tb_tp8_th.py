...
OP_NAME = 'transpose_batch_matmul_transpose_16_512'
...
def gen_golden_data():
    ...
    input1 = np.random.uniform(-1, 1, [B, N, K]).astype(np.float16)
    input1_nz = input1.reshape((B, N, K // 16, 16)).transpose(0, 2, 1, 3)
    golden = np.matmul(input0.transpose(1, 0, 2), input1.transpose(0, 2, 1)).transpose(1, 0, 2).astype(np.float16)
...
def transpose_batch_matmul_transpose_16_512(gm_a, gm_w, gm_out, token_num):
    idx = get_block_idx()
    l1_w = slice_to_l1(gm_w, [B // CORE_NUM * idx, 0, 0], [B // CORE_NUM, N, K])
    ...
    for i in dynamic_loop(tile_num):
        ...
        for j in range(N // 16):
            l0b = slice_to_l0B(l1_w, [0, j * 16, 0], [B // CORE_NUM, 16, K], transpose=True)
            l0c = mmad(l0a, l0b)
            ub_c = move_to_ub(l0c, "FP16")
            ub_c_nd = nz_to_nd(ub_c)
            ub_c_nd = transpose(ub_c_nd, [1, 0, 2])
            insert_to_gm(gm_out, ub_c_nd, [i * 16, B // CORE_NUM * idx, 16 * j], [16, B // CORE_NUM, 16])
    if res_num > 0:
        for i in dynamic_loop(res_num):
            ...
            for j in range(N // 16):
                ...
                ub_c = move_to_ub(l0c, "FP16")
                ub_c_nd = nz_to_nd(ub_c)
                ub_c_nd = slice_to_ub(ub_c_nd, [0, 0, 0], [B // CORE_NUM, 1, 16])
                ub_c_nd = change_view(ub_c_nd, [1, B // CORE_NUM, 16])
                insert_to_gm(gm_out, ub_c_nd,
                             [token_num - res_num + i, B // CORE_NUM * idx, 16 * j], [1, B // CORE_NUM, 16])
...
if __name__ == '__main__':
    ...
    gm_w = Tensor("GM", "FP16", [B, N, K], "NZ", False)
    gm_out = Tensor("GM", "FP16", [TN, B, N], "ND", False)
    token_num = Scalar("INT32")
    compile_func(transpose_batch_matmul_transpose_16_512, globals())(gm_a, gm_w, gm_out, token_num)
