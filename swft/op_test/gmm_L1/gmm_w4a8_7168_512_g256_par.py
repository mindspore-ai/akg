...
OP_NAME = 'grouped_matmul_3584_512_transpose'
...
def grouped_matmul_3584_512_transpose(x, w, group_list, x_scale, y_scale, bias, out):
    ...
    for i in dynamic_loop(EXP):
        ...
        if res > 0:
            for j in dynamic_loop(res):
                ub_scale_x = slice_to_ub(x_scale, [start_batch + j + rep * 16], [1])
                ...
                for n in dynamic_loop(ub_tiling):
                    ub_x = slice_to_ub(x, [start_batch + j + rep * 16, n * ub_tiling_size], [1, ub_tiling_size])
                ...
                insert_to_gm(out, ub_final_out, [start_batch + j + rep *16, block_idx * N // 8], [1, N // 8])
        for j in dynamic_loop(rep):
            ...
            ub_out_high_new = vector_dup(Scalar("FP32", 0), [16, N // 8], False)
            ub_out_high_new = change_view(ub_out_high_new, new_format="NZ")
            ub_out_low_new = vector_dup(Scalar("FP32", 0), [16, N // 8], False)
            ub_out_low_new = change_view(ub_out_low_new, new_format="NZ")
            for n in dynamic_loop(ub_tiling):
                ub_x = slice_to_ub(x, [start_batch + j * 16, n * ub_tiling_size], [16, ub_tiling_size])
                ub_high_x = change_view(ub_x, new_shape=[16, ub_tiling_size // 2], new_dtype="INT16")
                high_mask = vector_dup(Scalar("INT16", 0xf0f0), [16], False)
                ub_high_x = vand(ub_high_x, high_mask)
                ub_high_x = change_view(ub_high_x, new_shape=[16, ub_tiling_size], new_dtype="INT8")
                ub_x_half = vconv(ub_high_x, "FP16")
                ub_high_x = vmuls(ub_x_half, 0.0625)
                ub_high_x = vconv(ub_high_x, "INT4")
                ub_high_x = nd_to_nz(ub_high_x)
                l1_high_x = move_to_l1(ub_high_x)
                ub_low_x = change_view(ub_x, new_shape=[16, ub_tiling_size // 2], new_dtype="INT16")
                low_mask = vector_dup(Scalar("INT16", 0x0f0f), [16], False)
                ub_low_x = vand(ub_low_x, low_mask)
                ub_low_x = change_view(ub_low_x, new_shape=[16, ub_tiling_size], new_dtype="INT8")
                ub_low_x = vconv(ub_low_x, "FP16")
                ub_low_x = vsubs(ub_low_x, 8)
                ub_low_x = vconv(ub_low_x, "INT4")
                ...
                for k in range(l0b_tiling):
                    ...
                    ub_y_scale = slice_to_ub(
                        y_scale, [i, n*l0b_tiling + k, 0, block_idx * N//8], [1, 1, 1, N // 8])
                    ub_y_scale_cv = change_view(ub_y_scale, [N//8], "ND")
            ...
            ub_out_new = vadd(ub_out_high, ub_out_low_new)
            ub_bias = slice_to_ub(bias, [i, block_idx * N // 8], [1, N // 8])
            ub_bias = change_view(ub_bias,new_shape = [N//8], new_format = "ND")
            ...
            insert_to_gm(out, ub_final_out, [start_batch + j * 16, block_idx * N // 8], [16, N // 8])
...
if __name__ == "__main__":
    ...
    compile_func(grouped_matmul_3584_512_transpose, globals())(gm_x, gm_weight,
