...
OP_NAME = 'grouped_matmul_256_7168_transpose'
...
def gen_data():
    ...
    print(np.mean(np.abs((y_fp16.reshape(-1) - y_int_O.reshape(-1)))))
    before = 0
    for i in range(EXP):
        gl[i] = random.randint(before, min(
            M,before + ((M + EXP - 1) // EXP * 2)))
        for j in range(gl[i] - before):
            ...
            output[before + j, : ] = np.matmul(x_int8[before + j, : ], y_int_O[i, : ,:]).astype(
                np.float32) * x_scale[before + j, :]
        before = gl[i]
    ...
    print(np.mean(np.abs((out - output) / (output + 0.0000001))))
...
def grouped_matmul_256_7168_transpose(x, w, group_list, x_scale, y_scale, bias, out):
    block_idx = get_block_idx()
    ub_list = slice_to_ub(group_list, [0], slicesize=[EXP])
    percore_size = N // CORE_NUM
    l1_tiling_size = 896
    l1_tiling = percore_size // l1_tiling_size
    start_batch = Scalar("INT32", 0)
    l0b_tiling_size = 448
    l0b_tiling = l1_tiling_size // l0b_tiling_size
    for i in dynamic_loop(EXP):
        end_batch = move_to_scalar(ub_list[i])
        if end_batch == start_batch:
            continue
        rep = (end_batch - start_batch) // 16
        res = (end_batch - start_batch) % 16
        w_l1 = slice_to_l1(
                w, [i, block_idx * percore_size, 0], [1, percore_size, K])
        w_l1 = change_view(w_l1,[percore_size,K])
        if res > 0:
            for j in dynamic_loop(res):
                ub_x = slice_to_ub(x, [start_batch + rep * 16 + j, 0], [1, K])
                ub_x = change_view(ub_x, new_format="NZ")
                ub_high_x = change_view(
                    ub_x, new_shape=[1, K//2], new_dtype="INT16")
                high_mask = vector_dup(Scalar("INT16",0xf0f0),[16],False)
                ub_high_x = vand(ub_high_x, high_mask)
                ub_high_x = change_view(ub_high_x, new_shape=[
                                        1,K],new_dtype= "INT8")
                ub_x_half = vconv(ub_high_x,"FP16")
                ub_high_x = vmuls(ub_x_half, 0.0625)
                ub_high_x = vconv(ub_high_x, "INT4")
                ub_high_x = pad_to_ub(ub_high_x, [16, K])
                l1_high_x = move_to_l1(ub_high_x)
                ub_low_x = change_view(
                    ub_x, new_shape=[1, K//2], new_dtype="INT16")
                low_mask = vector_dup(Scalar("INT16",0x0f0f),[16],False)
                ub_low_x = vand(ub_low_x, low_mask)
                ub_low_x = change_view(ub_low_x, new_shape=[
                                        1,K],new_dtype= "INT8")
                ub_low_x = vconv(ub_low_x, "FP16")
                ub_low_x = vsubs(ub_low_x, 8)
                ub_low_x = vconv(ub_low_x, "INT4")
                ub_low_x = pad_to_ub(ub_low_x, [16, K])
                l1_low_x = move_to_l1(ub_low_x)
                ub_scale_x = slice_to_ub(x_scale, [start_batch + rep * 16 + j], [1])
                for n in dynamic_loop(l1_tiling):
                    for k in dynamic_loop(l0b_tiling):
                        ub_out_high_new = vector_dup(Scalar("FP32", 0),[1, l0b_tiling_size], False)
                        ub_out_high_new = change_view(ub_out_high_new, new_format="NZ")
                        ub_out_low_new = vector_dup(Scalar("FP32", 0),[1, l0b_tiling_size], False)
                        ub_out_low_new = change_view(ub_out_low_new, new_format="NZ")
                        for m in dynamic_loop(2):
                            x_l0a_high = slice_to_l0A(
                                l1_high_x, [0, m*K//2],[16, K//2])
                            w_l0b = slice_to_l0B(
                                w_l1,[n * l1_tiling + k * l0b_tiling_size, m*K//2],[l0b_tiling_size, K//2], transpose=True)
                            l0c_high = mmad(x_l0a_high,w_l0b)
                            ub_out_high = move_to_ub(l0c_high)
                            ub_out_high = vconv(ub_out_high,"FP32")
                            x_l0a_low = slice_to_l0A(
                                l1_low_x, [0, m*K//2],[16, K//2])
                            l0c_low = mmad(x_l0a_low,w_l0b)
                            ub_out_low = move_to_ub(l0c_low)
                            ub_out_low = vconv(ub_out_low,"FP32")
                            ub_y_scale = slice_to_ub(y_scale, [
                                                    i, m, 0, block_idx * percore_size + n * l1_tiling_size + k * l0b_tiling_size],[1, 1, 1, l0b_tiling_size])
                            ub_y_scale_cv = change_view(
                                ub_y_scale,[l0b_tiling_size],"NZ")
                            ub_high_slice = slice_to_ub(
                                ub_out_high,[0, 0],[1, l0b_tiling_size])
                            ub_out_high_mul = vmul(ub_high_slice, ub_y_scale_cv)
                            ub_low_slice = slice_to_ub(
                                ub_out_low, [0, 0],[1, l0b_tiling_size])
                        ...
                        ub_bias = slice_to_ub(
                            bias,[i,block_idx * percore_size + n * l1_tiling_size + k * l0b_tiling_size],[1,l0b_tiling_size])
                        ...
                        insert_to_gm(out, ub_final_out,[
                            start_batch + rep * 16 + j, block_idx * percore_size + n *l1_tiling_size + k*l0b_tiling_size],[1,l0b_tiling_size])
        
        for j in dynamic_loop(rep):
            ub_x = slice_to_ub(x, [start_batch + j * 16 , 0],[16, K])
            ub_high_x = change_view(
                ub_x, new_shape=[16, K//2], new_dtype="INT16")
            high_mask = vector_dup(Scalar("INT16",0xf0f0),[16],False)
            ub_high_x = vand(ub_high_x, high_mask)
            ub_high_x = change_view(ub_high_x, new_shape=[
                                16,K],new_dtype= "INT8")
            ...
            l1_low_x = move_to_l1(ub_low_x)
            ub_scale_x = slice_to_ub(x_scale, [start_batch + j * 16], [16])
            ub_scale_x = vconv(ub_scale_x, "FP16")
            ub_scale_x = change_view(ub_scale_x, new_shape=[16, 1], new_format="NZ")
            ub_scale_x = vbrcb(ub_scale_x, broadcast_axis=-1,broad_size=l0b_tiling_size)
            for n in dynamic_loop(l1_tiling):
                for k in dynamic_loop(l0b_tiling):
                    ub_out_high_new = vector_dup(Scalar("FP32", 0),[16, l0b_tiling_size], False)
                    ub_out_high_new = change_view(ub_out_high_new, new_format="NZ")
                    ub_out_low_new = vector_dup(Scalar("FP32", 0),[16, l0b_tiling_size], False)
                    ub_out_low_new = change_view(ub_out_low_new, new_format="NZ")
                    for m in dynamic_loop(2):
                        x_l0a_high = slice_to_l0A(
                            l1_high_x, [0, m*K//2],[16, K//2])
                        ...
                        ub_y_scale = slice_to_ub(y_scale, [
                                                i, m, 0, block_idx * percore_size + n * l1_tiling_size + k * l0b_tiling_size],[1, 1, 1, l0b_tiling_size])
                        ub_y_scale_cv = change_view(
                            ub_y_scale,[l0b_tiling_size],"ND")
                        ub_out_high_mul = vmul(ub_out_high, ub_y_scale_cv)
                        ub_out_low_mul = vmul(ub_out_low, ub_y_scale_cv)
                        ub_out_high_new = vadd(ub_out_high_new, ub_out_high_mul)
                        ub_out_low_new = vadd(ub_out_low_new, ub_out_low_mul)
                    ub_out_high = vmuls(ub_out_high_new, 16)
                    ub_out_new = vadd(ub_out_high, ub_out_low_new)
                    ub_bias = slice_to_ub(
                        bias,[i,block_idx * percore_size + n * l1_tiling_size + k * l0b_tiling_size],[1,l0b_tiling_size])
                    ub_bias = change_view(ub_bias, new_shape=[l0b_tiling_size], new_format="ND")
                    ub_bias = vconv(ub_bias, "FP16")
                    ub_out_new = vconv(ub_out_new, "FP16")
                    ub_out_add = vadd(ub_out_new, ub_bias)
                    ub_final_out = vmul(ub_out_add, ub_scale_x)
                    ub_final_out = nz_to_nd(ub_final_out)
                    insert_to_gm(out, ub_final_out,[
                        start_batch + j * 16, block_idx * percore_size + n *l1_tiling_size + k*l0b_tiling_size],[16, l0b_tiling_size])
...
if __name__ == "__main__":
    ...
    compile_func(grouped_matmul_256_7168_transpose,globals())(gm_x,gm_weight,
