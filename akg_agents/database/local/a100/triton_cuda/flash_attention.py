# 注意：FlashAttention需要严格按照下面代码实现算子的调度
# 不改变代码逻辑的情况下，根据入参调整tune config并删除不满徐条件的if判断（如STAGE，HAS_ATTN_MASK等）
# FlashAttention 的重点是分块，所以需要对Q,K,V进行切分
...
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_M': 64, 'BLOCK_N': 32}, num_stages=s, num_warps=w)
        for s in [1, 2, 3] for w in [4, 8]
    ],
    key=["KV_CTX", "HEAD_DIM"],
)
@triton.jit
def flash_attention_kernel(
    ...
    STAGE: tl.constexpr, HAS_ATTN_MASK: tl.constexpr,
    PRE_LOAD_V: tl.constexpr
):
    ...
    if STAGE & 1:
        acc, l_i, m_i = _attn_fwd_inner(
            ...
            start_m, sm_scale, q_load_mask, BLOCK_M, HEAD_DIM, BLOCK_N,
            4 - STAGE, offs_m, offs_n, KV_CTX,
            V.dtype.element_ty == tl.float8e5, HAS_ATTN_MASK, PRE_LOAD_V
        )
...
def flash_attention_triton_torch(
    query,
    key,
    value,
    attn_mask=None
    ...
    _attn_fwd[grid](
        ...
        query.shape[2], key.shape[2], HEAD_DIM_K,
        STAGE=stage, HAS_ATTN_MASK=HAS_ATTN_MASK,
