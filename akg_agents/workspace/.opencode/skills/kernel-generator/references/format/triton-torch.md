# ModelNew 格式规范 — Triton + PyTorch

适用于 `dsl` 为 `triton_cuda` 或 `triton_ascend`，`framework` 为 `torch` 的场景。
...
class ModelNew(torch.nn.Module):
    def __init__(self, ...):
        super().__init__()
        # 如果有 nn.Module 参数（如 nn.Linear, nn.Conv2d），需要：
        # 1. 在 __init__ 开始时设置固定随机种子（与验证模板中的种子一致）
        # 2. 通过 nn.Linear/nn.Conv2d 等构建参数，然后提取 weight 和 bias
        # 3. 使用 nn.Parameter 包装，确保与原始 Model 的权重一致
        torch.manual_seed(0)  # 固定种子（与 kernel_verify_template.j2 中的种子一致）
        # 例如：
...
请务必保证 `nn` 中调用的 module 要与任务代码中调用的 module 要一致，使用的参数要一致，device 设置请参考不同的硬件后端（`"cuda"`, `"npu"`, `"ascend"`）。
