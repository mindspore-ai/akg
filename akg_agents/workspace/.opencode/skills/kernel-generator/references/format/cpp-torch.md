# ModelNew 格式规范 — C++ (load_inline) + PyTorch

适用于 `dsl` 为 `cpp`，`framework` 为 `torch` 的场景。
...
class ModelNew(nn.Module):
    def __init__(self, ...):
        super().__init__()
        # 如果有 nn.Module 参数，需要设置随机种子并提取参数
...
class ModelNew(nn.Module):
    ...
    def forward(self, x):
        return xxx_module.xxx_kernel(x)
```
...
class ModelNew(nn.Module):
    ...
    def forward(self, x):
        return xxx_module.xxx_kernel(x, self.weight, self.bias)
```
