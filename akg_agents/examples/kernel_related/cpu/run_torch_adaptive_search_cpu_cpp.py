...
"""
自适应搜索示例 - CPU C++ (x86_64)

小 shape ReLU 算子的自适应搜索，用于 CPU x86_64 + C++ DSL。
...
用法:
  ...
  python examples/kernel_related/run_torch_adaptive_search_cpu_cpp.py

  # 自定义参数
  python examples/kernel_related/run_torch_adaptive_search_cpu_cpp.py --max-tasks 10
"""
...
async def run_adaptive_search(args):
    ...
    print("注册 Worker")
    print(f"{'='*60}")
    print(f"注册 LocalWorker: devices={args.devices}")
    await register_local_worker(args.devices, backend=backend, arch=arch)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="运行自适应搜索示例 (CPU C++ x86_64)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 默认配置（小规模测试）
  python run_torch_adaptive_search_cpu_cpp.py

  # 自定义参数
  python run_torch_adaptive_search_cpu_cpp.py --max-tasks 10 --max-concurrent 2
        """
    )
