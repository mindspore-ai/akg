...
Enable and configure through the `docs_dir` field in the Task Orchestration Plan. Examples: `akg_agents/python/akg_agents/config/vllm_triton_ascend_coderonly_config.yaml` (Ascend) or `akg_agents/python/akg_agents/config/vllm_triton_cuda_coderonly_config.yaml` (CUDA):
