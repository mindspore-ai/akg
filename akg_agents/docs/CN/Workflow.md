...
配置文件参考：[`python/akg_agents/config/`](../../python/akg_agents/config/) 目录。

### 1. 标准工作流 ([default_workflow.yaml](../../python/akg_agents/config/default_workflow.yaml))
**流程**: `designer` → `coder` ←→ `verifier` → `finish`
- 完整的设计→编码→验证流程
- 验证失败时可回退到coder

### 2. 全连接工作流 ([conductor_connect_all_workflow.yaml](../../python/akg_agents/config/conductor_connect_all_workflow.yaml))  
**流程**: 支持所有Agent间灵活跳转
- 最大灵活性，Agent可自我修复

### 3. 仅编码工作流 ([coder_only_workflow.yaml](../../python/akg_agents/config/coder_only_workflow.yaml))
**流程**: `coder` ←→ `verifier` → `finish`
- 跳过设计阶段，直接生成代码

### 4. 仅验证工作流 ([verifier_only_workflow.yaml](../../python/akg_agents/config/verifier_only_workflow.yaml))
**流程**: `verifier` → `finish`
- 最简化流程，仅用于验证
...
配置文件参考：[`python/akg_agents/config/`](../../python/akg_agents/config/) 目录下的各种workflow.yaml文件。
