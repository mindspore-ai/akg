...
通过“任务编排方案配置”的 `docs_dir` 字段启用与配置。示例：`akg_agents/python/akg_agents/config/vllm_triton_ascend_coderonly_config.yaml`（Ascend）或 `akg_agents/python/akg_agents/config/vllm_triton_cuda_coderonly_config.yaml`（CUDA）：
