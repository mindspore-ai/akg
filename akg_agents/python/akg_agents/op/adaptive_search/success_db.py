...
class SuccessDB:
    ...
    def add(self, record: SuccessRecord) -> bool:
        ...
        logger.info(
            f"Added record {record.id} to SuccessDB. gen_time={record.gen_time:.4f}us, speedup={record.speedup:.2f}x. DB size: {len(self._records)}")
        self._save_to_disk()
        return True
    ...
    def decrement_selection(self, record_id: str) -> None:
        ...
        if record_id in self._records:
            ...
            if self._total_selections > 0:
                self._total_selections -= 1
            logger.debug(
                f"Record {record_id} selection count decremented to: {self._records[record_id].selection_count}")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "generations": list({r.generation for r in self._records.values()})
        }
