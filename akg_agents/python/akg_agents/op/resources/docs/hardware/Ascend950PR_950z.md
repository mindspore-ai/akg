# Ascend950PR_950z NPU 硬件规格

## 1. 硬件架构

**芯片配置**
- 8 个 AI Core，每个包含: 2 个 VEC（向量计算单元） + 1 个 CUBE（矩阵计算单元）
- 整个芯片: 16个 VEC, 8个 CUBE
...
|---------|------|---------|------|------|
| GM | 16GB | 全设备 | - | 设备主存储 |
| L2 Cache | 16MB | 8 个 AI Core | - | 自动缓存 |
...
**并行:** 8个AI Core按需并行分配任务，CUBE与VEC可同时执行不同任务
