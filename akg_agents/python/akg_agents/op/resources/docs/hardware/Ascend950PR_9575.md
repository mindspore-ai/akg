# Ascend950PR_9575 NPU 硬件规格

## 1. 硬件架构

**芯片配置**
- 28 个 AI Core，每个包含: 2 个 VEC（向量计算单元） + 1 个 CUBE（矩阵计算单元）
- 整个芯片: 56个 VEC, 28个 CUBE
...
|---------|------|---------|------|------|
| GM | 96GB | 全设备 | - | 设备主存储 |
| L2 Cache | 128MB | 28 个 AI Core | - | 自动缓存 |
...
**并行:** 28个AI Core按需并行分配任务，CUBE与VEC可同时执行不同任务
