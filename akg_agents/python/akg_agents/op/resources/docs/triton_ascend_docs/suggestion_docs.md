...
### Grid与UB联合约束
- Ascend后端同时受grid总大小限制和单program UB/L0容量限制。不能为了让`grid <= 65535`而盲目增大`BLOCK_SIZE`、`BLOCK_M`、`BLOCK_N`或`BLOCK_K`。
- 当grid超限时，优先使用固定核心数grid-stride循环或连续分块处理，把逻辑tile数量放到kernel内部循环；每次实际处理的子块仍需满足UB/L0限制。
...
### Grid与Block配置检查
- [ ] Grid总大小是否不超过65535？
- [ ] 是否同时检查了单program的UB/L0占用，避免为了降低Grid数量而增大BLOCK导致`ub overflow`？
