# Triton CUDA Core API 参考手册

本文档是 Coder 默认展示的 Triton CUDA core API 文档。这里列出的 API 会被视为“已展示 API”；Triton API database 召回到同名 API 时只补充候选关系，不重复展开详细文档。
...
- `tl.dot` 适合矩阵/批矩阵 contraction，输入通常为 rank-2/rank-3 block tensor，形状为 `(M, K)` 和 `(K, N)`；不要用 `A * B` 冒充矩阵乘。
- fp32 输入使用 `tl.dot` 时注意 NVIDIA 默认可能使用 TF32；验证要求接近 PyTorch fp32 时显式使用 `input_precision="ieee"` 或更稳健的精度设置。
...
- **约束**:
  ...
  - `other` 应匹配归约语义：sum/dot 用 `0.0`，min 用 `+inf`，max 用 `-inf`。
...
- **约束**: `shape` 必须是编译期可确定 tuple，元素总数需与输入一致。
...
- **约束**: 也可以使用 `x[:, None]` / `x[None, :]` 形成广播维度，但不要使用不支持的复杂 Python tensor indexing。
...
### tl.dot(a, b, acc=None, input_precision=None, allow_tf32=None)
```python
result = tl.dot(a, b, acc=accumulator, input_precision="ieee")
```
- **参数**:
  - `a`, `b`: 输入矩阵
  - `acc`: 累加器 (可选)
  - `input_precision`: 输入精度设置，例如 `"tf32"` 或 `"ieee"`
  - `allow_tf32`: 旧式 TF32 控制参数，优先使用 `input_precision`
- **返回**: 矩阵乘法结果
- **用途**: 核心矩阵乘法操作
- **约束**:
  ...
  - `BLOCK_K` 通常取 tensor-core 友好的大小，常见至少 16；不要使用 `(1, BLOCK_K) @ (BLOCK_K, 1)` 伪装矩阵化。
  - `acc += A * B` 不能替代 `tl.dot(A, B)`，它只做逐元素乘法/广播，不会沿 K 做矩阵乘归约。
  - fp32 输入若验证要求接近 PyTorch fp32，显式设置 `input_precision="ieee"`；默认 TF32 可能导致数值误差。
