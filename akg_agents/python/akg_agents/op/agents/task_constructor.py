...
"""
    ...
    _FINISH_TOOL = {
        ...
        "function": {
            "name": "finish",
            "description": "任务完成或失败时调用。成功时传入 task_code + summary + op_name，失败时传入 error。",
            "parameters": {
                "type": "object",
                "properties": {
                    "task_code": {"type": "string", "description": "任务文件路径或代码"},
    ...
    def _build_tools_for_api(self) -> List[Dict]:
        ...
        tools = ToolRegistry.get_tools_for_prompt(
            scope="task_constructor", format="openai_json"
        )
        return tools + [self._FINISH_TOOL]
    ...
    def _build_system_prompt(self) -> str:
        """构建 system prompt（工具列表通过 API tools 参数传递，不注入 prompt）"""
    ...
    async def _call_llm_async(
        self, messages: List[Dict[str, str]], model_level: str,
        tools: Optional[List[Dict]] = None,
    ) -> Dict[str, Any]:
        """异步调用 LLM（支持 native function calling）"""
        client = self._get_llm_client(model_level)
        return await client.generate(messages, tools=tools)

    async def _get_next_action(
        self, step: int, model_level: str,
    ) -> Tuple[Optional[Dict], str, Optional[Dict]]:
        """使用 OpenAI native function calling 获取下一步动作

        Returns:
            (action_dict, raw_response_str, tool_call_raw)
            - action_dict: {"thought", "action", "arguments"}
            - raw_response_str: 原始响应（用于日志）
            - tool_call_raw: API 返回的原始 tool_call 字典（包含 id，用于 role:tool 消息）
              当模型返回纯文本（隐式 ask_user）时为 None
        """
        ...
        for attempt in range(self.MAX_RETRIES_PER_STEP + 1):
            try:
                result = await self._call_llm_async(
                    self.messages, model_level, tools=tools_for_api,
                )
                ...
                if self.session_log:
                    self.session_log.log_llm_call(
                        step, len(self.messages), raw_response, messages=self.messages,
                    )
                ...
                if tool_calls:
                    if len(tool_calls) > 1:
                        ...
                        logger.warning(
                            f"模型返回 {len(tool_calls)} 个 tool_calls {names}，"
                            f"当前仅处理第一个（ReAct 串行设计）"
                        )
                    tc = tool_calls[0]
                ...
                if content:
                    ...
                    logger.info("[Function Calling] 模型返回纯文本，作为 ask_user 处理")
    ...
    def _manage_history(self):
        ...
        # 先确定 recent 的切分点，确保不拆分 assistant(tool_calls) + tool 消息对
        cut_idx = len(self.messages) - (self.MAX_MESSAGES - 4)
        while cut_idx < len(self.messages) and self.messages[cut_idx].get("role") == "tool":
            cut_idx -= 1

        removed = self.messages[2:cut_idx]
            ...
            if msg["role"] == "assistant":
                ...
                if tool_calls:
                    for tc in tool_calls:
                        func = tc.get("function", {})
                        name = func.get("name", "")
                        summary_parts.append(f"[{name}] {content[:80] if content else ''}")
                elif content:
                    summary_parts.append(f"[text] {content[:80]}")
    ...
    async def run(
        ...
        **kwargs,
        ...
        for step in range(1, self.MAX_STEPS + 1):
            ...
            # 3.1 调用 LLM（native function calling）
            action, raw_response, tool_call_raw = await self._get_next_action(step, model_level)
            if action is None:
                result = self._build_result("error",
                                            error="LLM 多次返回无效响应，终止")
                if self.session_log:
            ...
            if act_name == "ask_user":
                ...
                if tool_call_raw:
                    self.messages.append({
                        "role": "tool",
                        "tool_call_id": tool_call_raw.get("id", f"call_{step}"),
            ...
            # 3.6 追加到消息（tool_calls + role:tool 格式）
            self.messages.append({
                "role": "assistant",
                "content": thought or None,
                "tool_calls": [tool_call_raw],
