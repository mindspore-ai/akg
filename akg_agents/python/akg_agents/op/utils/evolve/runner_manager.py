...
async def run_single_evolve(op_name: str = None, task_desc: str = None, evolve_config: RunnerConfig = None) -> Dict[str, Any]:
    ...
    # 兜底补齐 agent_model_config（各 Agent 需要通过此配置获取模型级别）
...
def run_single_task_subprocess(task_file: Path, output_dir: Path, index: int, total: int,
    ...
    try:
        env = os.environ.copy()
        # get_project_root() 返回 python/akg_agents，需要向上两级到达真正的项目根目录
...
async def run_batch_evolve(config_path: str = None) -> None:
    ...
    task_dir = os.path.expanduser(config["task_dir"]) if config["task_dir"] else os.path.expanduser("~/akg_agents_tasks")
    output_dir = Path(os.path.expanduser(config["output_dir"])) if config["output_dir"] else Path(
        os.path.expanduser("~/akg_agents_batch_results"))
    ...
    print(f"设备配置: {config['device_pool'] or '使用 AKG_AGENTS_WORKER_URL 指定远程 Worker'}")
    print("="*80)
