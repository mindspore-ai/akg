...
# 当所有引用方（langgraph nodes 等）迁移完成后将删除此文件
import warnings
warnings.warn(
    "akg_agents.core.agent.test_case_generator is deprecated, will be moved to akg_agents.op.agents",
    DeprecationWarning,
    stacklevel=2,
)
    ...
    async def run(self, task_info: dict) -> Tuple[str, str, str]:
        ...
        try:
            ...
            llm_output, prompt, reasoning = await self.run_llm(
                ...
                input_data,
                self.model_config.get("test_case_generator") or self.model_config.get("default") or "standard"
            )
