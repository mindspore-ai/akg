...
class OpTaskBuilder(AgentBase):
    ...
    def __init__(self, config: dict):
        ...
        if config:
            ...
            raise ValueError("config is required for OpTaskBuilder")
    ...
    async def _call_llm_and_parse(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """调用 LLM 并解析输出
        ...
        # 调用LLM
        model_level = self.model_config.get("op_task_builder") or "standard"
