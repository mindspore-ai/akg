...
from akg_agents.op.langgraph_op.conversational_state import ConversationalOpGenState, Message
from akg_agents.utils.common_utils import ParserFactory

from akg_agents.op.utils.main_op_agent_utils import (
...
from akg_agents.op.utils.main_op_agent_display import (
...
class MainOpAgent(AgentBase):
    ...
    async def _op_task_build_node(self, state: ConversationalOpGenState) -> Dict[str, Any]:
        ...
        # 如果对话历史接近上限，发出警告
        from akg_agents.op.langgraph_op.conversational_state import MAX_CONVERSATION_HISTORY_LENGTH
    ...
    async def _select_sub_agent_node(self, state: ConversationalOpGenState) -> Dict[str, Any]:
        ...
        try:
            ...
            # 调用 LLM 选择子 Agent
            model_level = self.model_config.get("sub_agent_selector") or "standard"
    ...
    async def _sub_agent_execution_node(self, state: ConversationalOpGenState) -> Dict[str, Any]:
        ...
        try:
            ...
            base_log_dir = result.get("log_dir", "")  # evolve/adaptive_search 返回的是配置路径（如 ~/akg_agents_logs）
    ...
    async def _analyze_user_action(self, state: Dict[str, Any], user_input: str) -> tuple:
        ...
        try:
            ...
            # 调用 LLM 分析用户动作
            model_level = self.model_config.get("action_analyzer") or "standard"
            llm_result, prompt, reasoning = await self.run_llm(
    ...
    async def continue_conversation(self, 
        ...
        from akg_agents.op.utils.main_op_agent_display import is_simple_command
