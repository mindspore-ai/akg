...
from akg_agents.op.utils.workflow_manager import WorkflowManager
from akg_agents.utils.workflow_controller import WorkflowController
...
class Conductor(AgentBase):
    ...
    async def _llm_decide_next_agent(self, current_agent: str, valid_next_agents: Set[str]) -> str:
        ...
        try:
            ...
            model_level = self.model_config.get('conductor') or "standard"
            content, formatted_prompt, reasoning = await self.run_llm(self.conductor_template, input_data, model_level)
