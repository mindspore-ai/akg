  ...
  - 检查冒号、逗号等符号是否遗漏""",
    ...
    def _resolve_doc_path(self, doc_path: str) -> str:
        ...
        try:
            ...
            if not self.config:
                ...
                return os.path.join("op", "resources", "docs", docs_subdir, doc_path)
    ...
    async def _llm_check(self, code: str, task_info: Optional[dict]) -> Tuple[bool, str, List[Dict]]:
        ...
        try:
            ...
            model_level = model_config.get("code_checker") or model_config.get("conductor") or "standard"
            
            client = create_llm_client(model_level=model_level)
            
            # 调用 LLM
            messages = [{"role": "user", "content": prompt}]
            result = await client.generate(messages, stream=False, temperature=0.1, max_tokens=2000)
