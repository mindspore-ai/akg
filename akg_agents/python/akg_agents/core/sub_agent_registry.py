...
class EvolveSubAgent(SubAgentBase):
    ...
    def _get_evolve_config_path(self) -> str:
        ...
        config_path = os.path.join(get_project_root(), "op", "config", "evolve_config.yaml")
        return config_path
    ...
    async def execute(self, 
        ...
        try:
            ...
            result = {
                ...
                "log_dir": evolution_result.get("log_dir", ""),  # 基础 log 路径（如 ~/akg_agents_logs），需要拼接 op_name
...
class AdaptiveSearchSubAgent(SubAgentBase):
    ...
    def _load_adaptive_search_config(self) -> Dict[str, Any]:
        ...
        # 尝试加载自适应搜索配置文件
        config_path = os.path.join(get_project_root(), "op", "config", "adaptive_search_config.yaml")
