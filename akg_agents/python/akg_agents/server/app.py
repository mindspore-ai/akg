...
from akg_agents.server.job_manager import get_job_manager
...
def start_server(host: Optional[str] = None, port: Optional[int] = None):
    ...
    Args:
        host: 监听地址。可从环境变量 AKG_AGENTS_SERVER_HOST 设置。
              - IPv4: "0.0.0.0" (所有接口), "127.0.0.1" (本地)
              - IPv6: "::" (所有接口，双栈), "::1" (本地)
              默认: "0.0.0.0"
        port: 监听端口。可从环境变量 AKG_AGENTS_SERVER_PORT 设置。
              默认: 8000
    """
    import uvicorn
    ...
    # 从环境变量读取配置，参数优先
    if host is None:
        host = os.environ.get("AKG_AGENTS_SERVER_HOST", "0.0.0.0")
    if port is None:
        port = int(os.environ.get("AKG_AGENTS_SERVER_PORT", "8000"))
    ...
    ws_ping_interval = _parse_ws_env(os.environ.get("AKG_AGENTS_WS_PING_INTERVAL"))
    ws_ping_timeout = _parse_ws_env(os.environ.get("AKG_AGENTS_WS_PING_TIMEOUT"))
