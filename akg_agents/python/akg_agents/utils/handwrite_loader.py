...
class HandwriteLoader:
    ...
    def _init_filesystem_mode(self) -> None:
        """初始化文件系统模式的路径"""
        akg_agents_root = Path(get_project_root()).parent.parent
        self.akg_kernels_bench_root = akg_agents_root / "benchmark" / "akg_kernels_bench"
        self.torch_base_dir = self.akg_kernels_bench_root
        self.triton_impl_base_dir = self.akg_kernels_bench_root / self.dsl / "impl"
        self.triton_docs_base_dir = self.akg_kernels_bench_root / self.dsl / "docs"
    ...
    def _load_data_pairs(self) -> None:
        ...
        for improvement_path in self.triton_docs_base_dir.rglob("*.md"):
            ...
            # 构建对应的torch文件路径
            # 例如: akg_kernels_bench/dynamic_shape/reduction/softmax_001.py
