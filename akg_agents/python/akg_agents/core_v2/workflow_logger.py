...
class WorkflowLogger:
    ...
    def save_parameters_to_files(self, agent_name: str, params: list):
        """兼容旧 Trace：写文件，不自增"""
        step = len(self._step_records)
        self._write_files(agent_name, params, None, step)
    ...
    def insert_agent_record(self, agent_name: str, result: str = "",
        ...
        """兼容旧 Trace 的 insert_agent_record 接口（会自增 step）"""
    ...
    def insert_conductor_agent_record(self, res: str, prompt: str,
                                      reasoning: str,
                                      agent_name: str) -> None:
        """兼容旧 Trace：conductor 写文件，不自增（与原始 Trace 行为一致）"""
        self.write_record(f"{agent_name}_decision", [
            ('result', res),
            ('prompt', prompt),
            ('reasoning', reasoning),
