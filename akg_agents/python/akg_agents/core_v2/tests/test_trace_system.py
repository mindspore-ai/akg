...
class TestTreeVisualization:
    ...
    def trace(self):
        """创建初始化的 TraceSystem"""
        temp = tempfile.mkdtemp()
        trace = TraceSystem("test_task_009", base_dir=temp)
        trace.initialize()
        yield trace
        shutil.rmtree(temp, ignore_errors=True)
    
    def test_visualize_tree(self, trace):
        """测试可视化树"""
        node1 = trace.add_node({"type": "call_designer"}, {"success": True})
        node2 = trace.add_node({"type": "call_coder"}, {"success": True, "lines": 120})
        node3 = trace.add_node({"type": "verify"}, {"success": True, "performance": 0.65})
        
        tree_str = trace.visualize_tree()
        
        assert "Trace Tree" in tree_str
    ...
    def test_get_node_detail(self, trace):
        ...
        detail = trace.get_node_detail(node1)
    ...
    def test_get_path_detail(self, trace):
        ...
        detail = trace.get_path_detail(node2)
