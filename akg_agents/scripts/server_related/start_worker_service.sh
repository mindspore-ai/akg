...
# IPv4/IPv6 配置说明:
# ========================================
# 通过环境变量 AKG_AGENTS_WORKER_HOST 来控制监听地址:
#   - IPv4 监听所有接口: export AKG_AGENTS_WORKER_HOST=0.0.0.0 (默认)
#   - IPv6 监听所有接口: export AKG_AGENTS_WORKER_HOST=::
#   - IPv4 本地回环:    export AKG_AGENTS_WORKER_HOST=127.0.0.1
#   - IPv6 本地回环:    export AKG_AGENTS_WORKER_HOST=::1
#   - 指定 IPv4 地址:   export AKG_AGENTS_WORKER_HOST=192.168.1.100
#   - 指定 IPv6 地址:   export AKG_AGENTS_WORKER_HOST=2001:db8::1
#
...
ARCH=${2:-a100}
DEVICES=${3:-0}
PORT=${4:-${AKG_AGENTS_WORKER_PORT:-9001}}
# 从环境变量获取 host，默认为 0.0.0.0 (IPv4 全接口)
HOST=${AKG_AGENTS_WORKER_HOST:-0.0.0.0}
...
python -m akg_agents.worker.server

# 或者使用 uvicorn 直接启动（更多控制选项）
# uvicorn akg_agents.worker.server:app --host "$HOST" --port $PORT
