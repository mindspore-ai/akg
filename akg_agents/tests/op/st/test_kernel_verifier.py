...
@pytest.mark.parametrize("op_name", ["relu"])
@pytest.mark.asyncio
async def test_kernel_verifier_profilling_tilelang_cuda_a100_torch(op_name):
...
@pytest.mark.parametrize("op_name", ["relu"])
@pytest.mark.asyncio
async def test_kernel_verifier_profilling_tilelang_ascend_ascend910b4_torch(op_name):
    framework = "torch"
    dsl = "tilelang_ascend"
    backend = "ascend"
    arch = "ascend910b4"
