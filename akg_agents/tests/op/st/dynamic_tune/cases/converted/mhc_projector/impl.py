...
class ModelNew(nn.Module):
    ...
    def forward(self, hidden_states, config=None):
        ...
        h_res_logits = torch.empty(
            (seq_len, batch_size, self.rate * self.rate), dtype=torch.float32, device=hidden_states.device
        )
        ...
        for start in range(0, seq_len, chunk_seq):
            ...
            post_process_kernel[(core_num,)](
                ...
                alpha_post,
                alpha_res,
                float(self.expand_post),
                bias_pre,
                bias_post,
                bias_res,
                BLOCK_SIZE=post_block_size,
                CORE_NUM=core_num,
            )
