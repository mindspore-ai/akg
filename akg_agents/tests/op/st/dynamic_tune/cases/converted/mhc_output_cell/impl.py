...
class ModelNew(nn.Module):
    ...
    def forward(self, h_res, h_post, original_streams, sublayer_out, config=None):
        S, B, R, _ = h_res.shape
        ...
        if config is None:
            shape_key = (int(original_streams.shape[0]),)
            config = self._select_config(shape_key)
        chunk_seq = config.param("CHUNK_SEQ")
        ...
        for start in range(0, S, chunk_seq):
            ...
            akg_agents_fused_hyper_connection_output_cell_kernel[(core_num,)](
                ...
                R,
                H,
                h_res_chunk.stride(0),
                h_res_chunk.stride(1),
                h_res_chunk.stride(2),
                h_res_chunk.stride(3),
                h_post_chunk.stride(0),
                h_post_chunk.stride(1),
                h_post_chunk.stride(2),
                orig_chunk.stride(0),
                orig_chunk.stride(1),
                self.hidden_size,
                1,
