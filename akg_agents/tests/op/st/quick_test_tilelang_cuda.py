...
"""Quick standalone test for tilelang_cuda DSL adapter + KernelVerifier."""
...
def test_dsl_registration():
    ...
    dsl = normalize_dsl("tilelang_cuda", backend="cuda")
    assert dsl == "tilelang_cuda", f"normalize_dsl failed: {dsl}"
    print(f"  normalize_dsl('tilelang_cuda') -> '{dsl}' OK")

    try:
        check_dsl("tilelang_cuda")
        print("  check_dsl('tilelang_cuda') OK")
...
def test_adapter():
    ...
    code = adapter.create_impl_module("torch", None)
    assert "ModelNew" in code, f"Wrong create_impl_module: {code}"
    print(f"  create_impl_module OK")

    call_code = adapter.call_impl("ModelNew", "inputs", 0, None, "test_op")
    assert "impl_model" in call_code, f"Wrong call_impl: {call_code}"
    print("  call_impl OK")

    bench_code = adapter.benchmark_impl("ModelNew", "inputs", 5, 50, "cuda", "test_op")
    assert "tilelang_benchmark_fn" in bench_code, f"Wrong benchmark: {bench_code[:200]}"
    assert "torch.cuda.Event" in bench_code
    print("  benchmark_impl OK")

    return True


async def test_kernel_verifier():
    """Step 3: Run KernelVerifier for relu with tilelang_cuda."""
    print("=" * 60)
...
def main():
    print("TileLang-CUDA AKG Agents Quick Test")
    print()
