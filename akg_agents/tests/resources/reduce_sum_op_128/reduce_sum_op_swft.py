...
def reduce_sum_op_impl_npu(gm_input, gm_output):
    ...
    ub_output_float = vcadd(ub_input_float, reduce_axis=-1)
...
def reduce_sum_op_host_run():
    ...
    gm_input = Tensor("GM", "FP16", [128, 128], format="ND", multi_core=False)
    gm_output = Tensor("GM", "FP16", [128], format="ND", multi_core=False)
