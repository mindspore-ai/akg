...
环境变量配置（需要 LLM API）：
    export AKG_AGENTS_API_KEY=your-api-key
    export AKG_AGENTS_MODEL_NAME=deepseek-chat           # 可选，默认 deepseek-chat
    export AKG_AGENTS_BASE_URL=https://api.deepseek.com  # 可选
    export AKG_AGENTS_MODEL_ENABLE_THINK=enabled         # 可选，启用思考模式
"""
...
def get_test_config(use_full_config: bool = False) -> dict:
    ...
    return {
        ...
        "log_dir": os.path.join(tempfile.gettempdir(), "akg_agents_test_logs"),
        "docs_dir": {
...
def create_llm_chat_openai():
    ...
    创建 LangChain ChatOpenAI，但是这个接口不会有reasoning_content
        AKG_AGENTS_MODEL_ENABLE_THINK: 启用思考模式（可选，enabled/disabled）
    """
    import httpx
    ...
    env_api_key = os.getenv("AKG_AGENTS_API_KEY")
    env_base_url = os.getenv("AKG_AGENTS_BASE_URL", "https://api.deepseek.com")
    ...
    if not env_api_key:
        raise RuntimeError(
            "需要配置 API Key:\n"
            "  export AKG_AGENTS_API_KEY=your-api-key\n"
            "  export AKG_AGENTS_MODEL_NAME=deepseek-chat  # 可选\n"
            "  export AKG_AGENTS_BASE_URL=https://api.deepseek.com  # 可选\n"
            "  export AKG_AGENTS_MODEL_ENABLE_THINK=enabled  # 可选，启用思考模式"
        )
...
def create_llm_chat_deepseek():
    ...
    if not env_api_key:
        raise RuntimeError(
            ...
            "  export AKG_AGENTS_MODEL_NAME=deepseek-reasoner  # 推荐使用 reasoner 模型\n"
            "  export AKG_AGENTS_MODEL_ENABLE_THINK=enabled  # 可选，启用思考模式"
        )
...
def test_read_skill_via_read_file():
    ...
    from akg_agents.core.tools.basic_tools import read_file
    from akg_agents.core.skills import SkillLoader
...
def test_list_all_tools():
    """列出所有注册的工具"""
    from akg_agents.core.agent.react_agent import ReActAgent
...
def test_create_llm():
    ...
    if reasoning:
        ...
        print("")
        print("可能原因：")
        print("  1. 未启用 thinking 模式: export AKG_AGENTS_MODEL_ENABLE_THINK=enabled")
...
def test_stream_chat_openai():
    ...
    if full_reasoning:
        ...
        print("  export AKG_AGENTS_MODEL_NAME=deepseek-reasoner")
        print("  export AKG_AGENTS_MODEL_ENABLE_THINK=enabled")
...
pytest:
    ...
    if not check_llm_env():
        print("❌ 需要配置 API Key:")
        print("   export AKG_AGENTS_API_KEY=your-api-key")
        print("   export AKG_AGENTS_MODEL_NAME=deepseek-chat           # 可选")
        print("   export AKG_AGENTS_BASE_URL=https://api.deepseek.com  # 可选")
        print("   export AKG_AGENTS_MODEL_ENABLE_THINK=enabled         # 可选")
        sys.exit(1)
