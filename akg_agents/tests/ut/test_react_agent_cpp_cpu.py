...
def create_llm(callback: LLMTraceCallback):
    ...
    env_model = os.getenv("AKG_AGENTS_MODEL_NAME", "deepseek-chat")
    env_think = os.getenv("AKG_AGENTS_MODEL_ENABLE_THINK")
    
    if not env_api_key:
        raise RuntimeError("需要 AKG_AGENTS_API_KEY 或 DEEPSEEK_API_KEY")
...
if __name__ == "__main__":
    ...
    if not check_llm_env():
        print("❌ 需要: export AKG_AGENTS_API_KEY=your-key")
        sys.exit(1)
