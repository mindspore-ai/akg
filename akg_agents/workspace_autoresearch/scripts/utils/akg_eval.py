...
def _build_verifier(task_dir: str, config, ref_code: str, backend: str,
    ...
    return KernelVerifier(
        ...
        config={
            ...
            # per-round kernel) read the same values so timing stays
            # comparable.
...
async def _eval_async(task_dir: str, config, device_id: int,
    ...
    # per-task — it varies per machine, scaffold autodetects via npu-smi.
    ...
    if not arch and worker_url:
        ...
        # orchestrator may not have an NPU to probe. Fetch from the worker
        # daemon's /api/v1/status (already up when we reach here, by
        # construction of the worker --start → scaffold ordering).
