...
def check_editable_files(task_dir: str, config) -> list:
    ...
    for fname in config.editable_files:
        ...
        with open(fpath, "r", encoding="utf-8") as f:
            code = f.read()
        # CodeChecker.check is async (upstream API). Bridge to sync via
        # asyncio.run() so this CLI doesn't need to be async itself.
        passed, report, errors = asyncio.run(checker.check(code))
        if not passed:
...
def _run_smoke_test(task_dir: str, config) -> list:
    ...
    try:
        r = subprocess.run(
            ...
            timeout=config.smoke_test_timeout, cwd=task_dir, check=False,
        )
