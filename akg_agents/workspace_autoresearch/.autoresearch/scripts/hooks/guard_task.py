...
def main():
    ...
    if state.action == DIAGNOSE_READY:
        block_decision(
            "[AR] DIAGNOSE artifact already validated for plan_version="
            f"{state.plan_version}. Do not re-issue Task; write "
            ".ar_state/plan_items.xml from the diagnosis and run "
            "create_plan.py."
        )

    if state.action == DIAGNOSE_MANUAL_FALLBACK:
        block_decision(
            "[AR] DIAGNOSE subagent already failed "
            f"{DIAGNOSE_ATTEMPTS_CAP} times for plan_version="
            f"{state.plan_version}. Switch to manual planning: read "
            ".ar_state/history.jsonl + plan.md, Write <items>...</items> "
            "to .ar_state/plan_items.xml, then run create_plan.py. The "
            "artifact gate on create_plan.py is relaxed in this state."
        )
    ...
    if subagent_type != _REQUIRED_SUBAGENT:
        block_decision(
            "[AR] DIAGNOSE phase requires Task with "
            ...
            "only diagnostician the host accepts here."
        )
