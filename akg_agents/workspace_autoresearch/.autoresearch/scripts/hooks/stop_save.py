...
the agent can't fix (reference.py invalid, worker / env / device down).
on_baseline_settled refuses to advance past BASELINE in that case;
without this carve-out the agent would loop forever because guard_edit
blocks any attempt to "fix" ref / external files. The emitted status
text tells the user what to fix and how to resume.
"""
...
def main():
    ...
    if stuck:
        if progress.get("baseline_error_source") == "ref":
            ...
            emit_status(
                "[AR] Check worker / env / device / eval.timeout, then "
