...
append_history) and side effects (git commit / rollback / phase transition).
This module owns the Progress field transitions so baseline and round cannot
quietly drift on baseline anchor, seed metric, and round counter rules.
"""
...
def reduce_baseline_init(existing: Progress, config: Any, eval_data: dict,
    ...
    """`best_commit` is the git HEAD pinned for Progress.best_commit when the
    SEED kernel produced a valid timing."""
    outcome = read_outcome(eval_data)
    ...
    anchor = resolve_baseline_init_anchor(
        existing, metrics, config, seed_metric=seed_metric)
...
def reduce_round_progress(progress: Progress, config: Any,
                          eval_result: EvalResult, round_num: int,
                          consecutive_failures: int,
                          best_metric: Optional[float],
                          best_commit: Optional[str]) -> RoundReduction:
    anchor = refresh_round_anchor(progress, eval_result.metrics, config)
