...
bool ScheduleTreeAnalyzer::AnalyzeScheduleTree() {
  ...
  for (auto &band_node : band_nodes_) {
    ...
    analyzer_->logger_.AppendLog(ANA_SCHETREE, ss);
    analyzer_->logger_.AppendLine(ANA_SCHETREE, prefix_schedule.to_str());
...
void ScheduleTreeAnalyzer::ConstructBandNode() {
  ...
  isl::schedule_node root_node = Transform::GetOuterBand(sch_.get_root());
  ...
  for (const auto &band : band_list_) {
    for (auto loop : band) {
      ...
      auto GetReplacedName = [this, loop]() -> std::string {
        ...
        for (const auto &it : analyzer_->scop_->params_rev_map_) {
...
void ScheduleTreeAnalyzer::AnalyzeCubeInfo() {
  std::string res = analyzer_->scop_->GetCName();
  for (const auto &it : provides_map_) {
    std::vector<const Provide *> pros = it.second;
    ...
    auto GetDeepestCall = [this, &op_list](const NodeRef &op) {
      if (const Call *call = op.as<Call>()) {
        ...
        if (call->name != analyzer_->scop_->GetAName() && call->name != analyzer_->scop_->GetBName() &&
            call->name != analyzer_->scop_->GetCName()) {
          return;
        }
    ...
    auto SortMatrixInCBAOrder = [this](const Call *c1, const Call *c2) {
      if (c1->name == this->analyzer_->scop_->GetCName() || c2->name == this->analyzer_->scop_->GetCName()) {
        return (c1->name == this->analyzer_->scop_->GetCName());
      } else if (c1->name == this->analyzer_->scop_->GetBName() || c2->name == this->analyzer_->scop_->GetBName()) {
        return (c2->name == this->analyzer_->scop_->GetBName());
      }
      return true;
    };
...
void ScheduleTreeAnalyzer::MatchConvVarNames(const Call *call) {
  ...
  for (auto arg : call->args) {
    count += 1;
    VarNames var_names;
    var_names = analyzer_->VisitVarNames(arg, var_names, call->name == analyzer_->scop_->GetCName());
    ...
    if (var_names.size() == 1U) {
      ...
      if (name == "0") {
        ...
        if (call->name == analyzer_->scop_->GetCName()) {
          cube_var_map_[name] = analyzer_->NC1HWC0[count];
        } else if (call->name == analyzer_->scop_->GetAName()) {
      ...
      CHECK(call->name != analyzer_->scop_->GetCName());
      if (call->name == analyzer_->scop_->GetAName()) {
      ...
      } else if (call->name == analyzer_->scop_->GetBName()) {
...
void ScheduleTreeAnalyzer::MatchConvFilterVarNames(const Call *call) {
  if (call->name != analyzer_->scop_->GetAName() && call->name != analyzer_->scop_->GetCName()) return;
  int count = -1;
  for (auto arg : call->args) {
    count += 1;
    VarNames var_names;
    ...
    if (var_names.size() == 1U) {
      ...
      CHECK(call->name == analyzer_->scop_->GetAName());
      for (const auto &name : var_names) {
        if (cube_var_map_.find(name) == cube_var_map_.end()) {
          cube_var_map_[name] = analyzer_->FilterInput_Matrix[count];
          break;
