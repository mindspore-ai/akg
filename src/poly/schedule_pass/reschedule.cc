 ...
 * the whole schedule.
 */
void Reschedule::CollectTileBandData(const isl::schedule_node &node, struct TileBandData *tile_band_data) {
...
isl::schedule_node Reschedule::RetrieveNodeList(isl::schedule_node node,
...
isl::schedule_node Reschedule::RetrieveAstBuildOptions(isl::schedule_node node, const isl::union_set &options) {
...
bool Reschedule::ValidateReorderedSchedule(const isl::schedule &new_schedule) {
  isl::union_map new_dependence = ComputeAllDependences(new_schedule, scop_info_.analysis_result_.GetReads(),
                                                        scop_info_.analysis_result_.GetWrites());
  bool is_valid = new_dependence.is_subset(pass_info_.dependences_);
  return is_valid;
}

isl::schedule_node Reschedule::TryRestoreStmtOrder(const isl::schedule_node &node,
...
isl::schedule Reschedule::RescheduleSerializeSccs(const isl::union_set &active_domain, const bool need_dist) const {
...
isl::schedule_node Reschedule::ReschedulePreserveFilterOrder(const isl::schedule_node &node,
                                                             const isl::union_set &active_domain,
...
PointBandInfo Reschedule::SavePointBand(const isl::schedule_node &node) {
 ...
 */
isl::schedule_node Reschedule::RestorePointBandInfo(isl::schedule_node node, const PointBandInfo &point_band_info) {
  // Retrieve point band if a sequence/set node is introduced
...
isl::schedule_node Reschedule::RescheduleSchTree(const isl::schedule_node &root) {
  ...
  auto scalar_filter = [](const isl::schedule_node &node) {
 ...
 *
 * Reschedule::Reschedule assumes the mark nodes are the outer bands.
...
void Reschedule::Dump() {
  std::ofstream of;
  of.open("transform.log", std::ios::out);
  if (!of.is_open()) {
    return;
  }
...
isl::schedule Reschedule::Run(isl::schedule curr_schedule) {
  isl::schedule sched = curr_schedule;
  isl::schedule_node root = sched.get_root();
  if (scop_info_.user_config_.GetTileInnerBand())
