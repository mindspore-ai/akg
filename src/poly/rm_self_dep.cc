 ...
 */
bool CheckIsStmtReduceOp(const isl::union_map &reads, const isl::union_map &writes, const isl::id &tuple_id,
                         std::vector<std::string> &reduce_axis_list) {
  ...
  for (const auto &it : read_tensors) {
    if (it.first != reduce_tensor) {
      ...
      if (!FindReduceAxis(reduce_read_access, other_read_access, reduce_axis_list)) return false;

      if (!last_reduce_axis_list.empty() && reduce_axis_list != last_reduce_axis_list) return false;
...
static bool IsMultiAxisSelfDependence(const isl::union_map &dependences, const isl::id &tuple_id) {
  bool foundMultiAxisSelfDependence = false;
  dependences.foreach_map([&](const isl::map &m) -> void {
    if (m.domain().get_tuple_id() == m.range().get_tuple_id() && m.domain().get_tuple_id() == tuple_id &&
        m.n_basic_map() >= 2) {
      foundMultiAxisSelfDependence = true;
    }
  });
  return foundMultiAxisSelfDependence;
}

/*
 * Removes self dependence of (multi-axis) reduce operations.
 */
isl::union_map Transform::RemoveReduceOpSelfDependence(bool multiAxisOnly) {
  isl::union_map preserved_dependences = isl::union_map::empty(dependences_.get_space());
  std::unordered_map<std::string, bool> is_tuple_reduce_op;
  dependences_.foreach_map([&, this](const isl::map m) -> void {
    if (m.domain().get_tuple_id() != m.range().get_tuple_id()) {
      ...
      if (is_tuple_reduce_op.count(tuple_id_key) == 0) {
        std::vector<std::string> reduce_axis_list;
        if (multiAxisOnly && !IsMultiAxisSelfDependence(dependences_, tuple_id)) {
          is_tuple_reduce_op[tuple_id_key] = false;
        } else {
          is_tuple_reduce_op[tuple_id_key] =
            CheckIsStmtReduceOp(scop_.data_.reads, scop_.data_.writes, tuple_id, reduce_axis_list) ||
            CheckIsStmtReduceOp(dependences_, tuple_id, reduce_axis_list);
        }

        if (is_tuple_reduce_op[tuple_id_key]) {
          scop_.RecordReduceStmt(tuple_id, reduce_axis_list);
        }
      }
      if (!is_tuple_reduce_op[tuple_id_key]) {
        preserved_dependences = preserved_dependences.add_map(m);
 ...
 */
isl::union_map Transform::RemoveSelfDependence() {
  isl::union_map preserved = isl::union_map::empty(dependences_.get_space());
  isl::union_map removed = isl::union_map::empty(dependences_.get_space());
