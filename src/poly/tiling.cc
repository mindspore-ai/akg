...
class TilingGenerator {
 public:
  ...
  std::vector<Scop::DimensionInfo> Generate() {
    TraverseSolver solver(analyzer_);
    this->cand_ = solver.Solve();
    return ConvertToDims();
  }

  std::vector<Scop::DimensionInfo> GenerateQuickly() {
  ...
  std::pair<std::vector<Scop::DimensionInfo>, std::deque<Scop::ParamInfo>> GenerateDynamic() {
 ...
 private:
  std::vector<Scop::DimensionInfo> ConvertToDims() {
    std::vector<Scop::DimensionInfo> dims;
  ...
  static Scop::DimensionInfo ConvertDefaultInfo(TileAxis *axis) {
    Scop::DimensionInfo dimInfo;
    dimInfo.index = axis->index;
    dimInfo.dim_seq = axis->seq_index;
  ...
  void ConvertVarTilesToDims() {
    ...
    auto Convert = [this, &var_to_prime_record](TileAxis *axis) {
      ...
      Expr l1_val;
      Expr l0_val;
      Scop::DimensionInfo dimInfo = ConvertDefaultInfo(axis);
      std::tie(l1_val, l0_val) = this->cand_->GetTileVal(axis);
      l1_val = CanonicalSimplify(l1_val);
      l0_val = CanonicalSimplify(l0_val);
      const auto l1 = l1_val.as<IntImm>();
  ...
  void ConvertShiftBoundToDims() {
    ...
    auto Convert = [this](TileAxis *axis) {
      ...
      if (!bound_value.empty()) {
        ...
        Scop::DimensionInfo bound_info = ConvertDefaultInfo(axis);
  ...
  std::vector<Scop::DimensionInfo> dims_;
  bool tile_success_{true};
};

// Map result to required format.
std::vector<Scop::DimensionInfo> NullTiling() {
  std::vector<Scop::DimensionInfo> dims;
  Scop::DimensionInfo dimInfo;
  dimInfo.index = 0;
  dimInfo.axis = "0";
...
std::pair<std::vector<Scop::DimensionInfo>, std::deque<Scop::ParamInfo>> GenerateTiling(
  Scop *scop, const isl::schedule &sch, const std::vector<NodeRef> &custom_tiling,
  const std::vector<NodeRef> &dynamic_shape) {
  CHECK(scop);
  scop->is_tiled_ = false;
  std::vector<Scop::DimensionInfo> dims = NullTiling();
  std::deque<Scop::ParamInfo> param_info;
  TilingAnalyzer analyzer(scop, sch, custom_tiling, dynamic_shape);
  bool need_tiling = analyzer.Prepare();

  std::stringstream ss;
  ss << scop->GenHalide(sch);
  ...
  } else if (analyzer.scop_->pragma_speedup_tiling_ && analyzer.op_type_ == VECTOR_OP) {
