...
class Stage : public NodeRef {
 public:
  ...
  /*!
   * \brief Set alignment requirement for realize bound of each axis
   * \params List of alignment requirement for each axis.
   *    Each list alignment element is a tuple (min_align, extent_align)
   * \return reference to self
   * */
  TVM_DLL Stage& buffer_align(Array<Array<Expr>> buffer_aligns);

  TVM_DLL Stage& buffer_tile(Array<Array<Expr>> buffer_tiles);
  /*!
...
class StageNode : public Node {
 public:
  ...
  /*! \brief Whether sepecifies alignment requirement of each axis */
  bool specify_realize_align{false};
  /*! \brief Alignment requirement of each axis */
  std::unordered_map<IterVar, std::pair<Expr, Expr>> realize_aligns;

  bool specify_realize_bound{false};
  std::unordered_map<IterVar, Range> realize_bound;
  /*!
