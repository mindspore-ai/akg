...
class HybridOpNode : public OperationNode {
 public:
  ...
  /*! \brief Buffers already declared for input tensors. */
  Map<Tensor, Buffer> input_buffers_;
  /*! \brief Buffers already declared for output tensors. */
  Map<Tensor, Buffer> output_buffers_;
  /*! \brief Regions corresponds to input_buffers_. */
  Map<Tensor, Region> input_regions_;
  /*! \brief Regions corresponds to output_buffers_. */
  Map<Tensor, Region> output_regions_;
...
inline Tensor compute(Array<Expr> shape,
                      std::function<Expr(Var, Var, Var, Var, Var)> f,
                      std::string name = "tensor",
                      std::string tag = "",
                      Map<std::string, NodeRef> attrs = {}) {
  FCompute fc = [f] (const Array<Var>& i) { return f(i[0], i[1], i[2], i[3], i[4]); };
  return compute(shape, fc, name, tag, attrs);
}
