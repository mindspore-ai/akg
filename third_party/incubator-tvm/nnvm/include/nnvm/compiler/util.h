 ...
 */
inline air::Array<air::Expr> ShapeToArray(TShape shape) {
  air::Array<air::Expr> result;
  for (auto i : shape) {
    result.push_back(air::make_const(air::Int(32), i));
  }
  return result;
}
 ...
 * \return An Array of Expr, where each element is a constant int32
 */
inline air::Array<air::Integer> ShapeToIntArray(TShape shape) {
  return air::Downcast<air::Array<air::Integer> >(ShapeToArray(shape));
}
}  // namespace compiler
}  // namespace nnvm
