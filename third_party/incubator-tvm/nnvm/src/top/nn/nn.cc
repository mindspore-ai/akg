...
.set_attr<FTVMCompute>(
  "FTVMCompute", [](const NodeAttrs& attrs,
    ...
    CHECK(pad_width.ndim() == inputs[0]->shape.size() &&
      pad_width[0].ndim() == 2)
      << "Illegal pad_width";
    Array<air::Expr> pad_before;
    for (size_t i = 0; i < pad_width.ndim(); ++i) {
      pad_before.push_back(air::make_const(air::Int(32), pad_width[i][0]));
    }
    Array<air::Expr> pad_after;
    for (size_t i = 0; i < pad_width.ndim(); ++i) {
      pad_after.push_back(air::make_const(air::Int(32), pad_width[i][1]));
    }
    return Array<Tensor>{ topi::pad(inputs[0], pad_before, pad_after,
                          air::make_const(inputs[0]->dtype, param.pad_value)) };
})
.set_support_level(1);
