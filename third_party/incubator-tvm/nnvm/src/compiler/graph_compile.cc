...
nnvm::Graph GraphCompile(const nnvm::Graph& g) {
  ...
  // Start lowering.
  Array<air::LoweredFunc> func_list;
  std::unordered_set<const air::Node*> func_set;
  const IndexedGraph& idx = g.indexed_graph();
  ...
  for (const auto& kv : old_new) {
    ...
    for (uint32_t i = 0; i < inode.source->num_outputs(); ++i) {
      ...
      new_dltype_vec[new_eid] = air::runtime::TVMType2String(
  ...
  air::runtime::Module module = fbuild(func_list, target, target_host);
