 ...
 */
int GetTypeFlag(air::Type type) {
  if (type == air::Float(32)) return 0;
  if (type == air::Float(64)) return 1;
  if (type == air::Float(16)) return 2;
  if (type == air::UInt(8)) return 3;
  if (type == air::Int(32)) return 4;
  if (type == air::Int(8)) return 5;
  if (type == air::Int(64)) return 6;
  if (type == air::Int(16)) return 7;
  if (type == air::UInt(16)) return 8;
  if (type == air::UInt(32)) return 9;
  if (type == air::UInt(64)) return 10;
  if (type == air::UInt(1)) return 11;
...
class CompileEngine {
 public:
  ...
  GraphFunc Lower(Graph graph,
    ...
    auto n = air::make_node<GraphCacheEntryNode>();
    n->graph_func = f;
    n->use_count = 1;
  ...
  Array<NodeRef> ListCacheItems() {
    ...
    for (auto& kv : cache_) {
      ...
      auto n = air::make_node<GraphCacheEntryNode>(*(kv.second.operator->()));
  ...
  std::tuple<Schedule, Array<air::Tensor>, Graph>
  GetScheduleArgs(Graph graph,
                  const Array<air::Tensor> &inputs,
                  const std::string &target,
                  int master_idx,
                  std::string *readable_name,
                  Array<air::Tensor> *outputs) {
    // shape, type
    ...
    for (const air::Tensor t : inputs) {
      std::vector<dim_t> shape;
      for (Expr v : t->shape) {
        CHECK(v.as<air::ir::IntImm>());
        shape.push_back(v.as<air::ir::IntImm>()->value);
      }
    ...
    std::vector<air::Tensor> tensor_vec(idx.num_node_entries());
    ...
    for (uint32_t nid = 0; nid < idx.num_nodes(); ++nid) {
      ...
      for (const IndexedGraph::NodeEntry& e : inode.inputs) {
        const air::Tensor& t = tensor_vec[idx.entry_id(e)];
        CHECK(t.defined());
      ...
      for (uint32_t i = 0; i < out.size(); ++i) {
        ...
        air::Tensor inferred_tensor = out[i];
        air::Tensor computed_tensor = out_info[i];
  ...
  GraphFunc DoLower(Graph graph,
    ...
    auto gf = air::make_node<GraphFuncNode>();
    gf->target = target;
...
.set_body([](air::runtime::TVMArgs args, air::runtime::TVMRetValue *rv) {
...
.set_body([](TVMArgs args, TVMRetValue *rv) {
    ...
    const Array<air::Tensor> &inputs = key->inputs;
