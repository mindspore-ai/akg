...
static inline int DetectROCMmaxThreadsPerBlock() {
  ...
  tvm_ctx.device_id = 0;
  air::runtime::DeviceAPI* api = air::runtime::DeviceAPI::Get(tvm_ctx, true);
  if (api != nullptr) {
    TVMRetValue val;
    api->GetAttr(tvm_ctx, air::runtime::kExist, &val);
    if (val.operator int() == 1) {
      air::runtime::DeviceAPI::Get(tvm_ctx)->GetAttr(tvm_ctx, air::runtime::kMaxThreadsPerBlock,
                                                     &val);
...
class CodeGenAMDGPU : public CodeGenLLVM {
 public:
  ...
  unsigned GetGlobalAddressSpace() { return 1; }
...
inline int DetectROCMComputeVersion(const std::string& target) {
  ...
  if (api != nullptr) {
    ...
    api->GetAttr(tvm_ctx, air::runtime::kExist, &val);
    if (val.operator int() == 1) {
      air::runtime::DeviceAPI::Get(tvm_ctx)->GetAttr(tvm_ctx, air::runtime::kGcnArch, &val);
      return val.operator int();
    }
  }
...
runtime::Module BuildAMDGPU(Array<LoweredFunc> funcs, std::string target) {
  ...
  const auto* find_rocm_bitcodes = air::runtime::Registry::Get("tvm_callback_rocm_bitcode_path");
  ...
  const auto* f = air::runtime::Registry::Get("tvm_callback_rocm_link");
...
TVM_REGISTER_API("codegen.build_rocm").set_body_typed(BuildAMDGPU);
