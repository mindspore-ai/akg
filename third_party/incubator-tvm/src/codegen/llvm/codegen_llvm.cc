  ...
  for (Var arg : f->args) {
    ...
    if (t.is_handle()) {
      ...
      if (it != f->handle_data_type.end()) {
        arg_types.push_back(LLVMType((*it).second.type())->getPointerTo(GetGlobalAddressSpace()));
      } else {
...
void CodeGenLLVM::Optimize() {
  ...
  fpass.add(llvm::createTargetTransformInfoWrapperPass(
      target_machine_ ? target_machine_->getTargetIRAnalysis() : llvm::TargetIRAnalysis()));
  fpass.add(llvm::createLowerExpectIntrinsicPass());
  fpass.add(llvm::createCFGSimplificationPass());
  fpass.add(llvm::createSROAPass());
  fpass.add(llvm::createEarlyCSEPass());
  fpass.add(llvm::createPromoteMemoryToRegisterPass());
...
//
void CodeGenLLVM::AddAliasInfo(llvm::Instruction* inst, const Variable* buffer, Expr index,
                               Type type) {
...
void CodeGenLLVM::GetAlignment(Type t, const Variable* buf_var, const Expr& index, int* p_alignment,
                               int* p_native_bits) {
...
void CodeGenLLVM::CreateSerialFor(llvm::Value* begin, llvm::Value* end, llvm::Value* stride,
  ...
  builder_->CreateCondBr(CreateLT(loop_var.type(), loop_value, end), for_body, for_end,
...
llvm::Value* CodeGenLLVM::CreateCallExtern(const Call* op) {
  ...
  llvm::FunctionType* ftype = llvm::FunctionType::get(LLVMType(op->type), arg_type, false);
...
llvm::Value* CodeGenLLVM::CreateIntrinsic(const Call* op) {
  if (op->is_intrinsic("llvm_intrin")) {
    ...
    llvm::Intrinsic::ID id = static_cast<llvm::Intrinsic::ID>(op->args[0].as<UIntImm>()->value);
    ...
    unsigned addrspace;
    if (!r) {
      ptr = CreateBufferPtr(l->type, MakeValue(l->buffer_var), MakeValue(l->index));
      addrspace = llvm::dyn_cast<llvm::PointerType>(ptr->getType())->getAddressSpace();
    } else {
      Expr index = r->base / make_const(Int(32), r->lanes);
      ptr = CreateBufferVecPtr(l->type, MakeValue(l->buffer_var), MakeValue(index));
      addrspace = llvm::dyn_cast<llvm::PointerType>(ptr->getType())->getAddressSpace();
    }
    ...
    CHECK_EQ(op->args[0].type().lanes(), 1) << "if_then_else can only take scalar condition";
    using llvm::BasicBlock;
...
void CodeGenLLVM::Scalarize(const Expr& e, std::function<void(int i, llvm::Value* v)> f) {
...
// Visitors
llvm::Value* CodeGenLLVM::VisitExpr_(const Variable* op) { return GetVarValue(op); }
...
llvm::Value* CodeGenLLVM::VisitExpr_(const StringImm* op) { return GetConstString(op->value); }

#define DEFINE_CODEGEN_BINARY_OP(Op)                                             \
  llvm::Value* CodeGenLLVM::Create##Op(Type t, llvm::Value* a, llvm::Value* b) { \
    if (t.is_int()) {                                                            \
      if (t.bits() >= 32) {                                                      \
...
llvm::Value* CodeGenLLVM::VisitExpr_(const Ramp* op) {
  ...
  for (int i = 0; i < op->lanes; ++i) {
    vec = builder_->CreateInsertElement(
        vec, MakeValue(op->base + op->stride * make_const(op->stride.type(), i)), ConstInt32(i));
  }
  return vec;
}
...
void CodeGenLLVM::VisitStmt_(const For* op) {
  ...
  CreateSerialFor(MakeValue(op->min), MakeValue(op->min + op->extent), ConstInt32(1), op->loop_var,
                  op->body);
}
...
void CodeGenLLVM::VisitStmt_(const Allocate* op) {
  ...
  if (op->new_expr.defined()) {
    ...
    llvm::AllocaInst* alloca = WithFunctionEntry(
        [&]() { return builder_->CreateAlloca(LLVMType(op->type), ConstInt32(constant_size)); });
#if TVM_LLVM_VERSION >= 110
...
#if TVM_LLVM_VERSION >= 110
    alignment = static_cast<unsigned>(alloca->getAlign().value());
#else
  ...
  buf = builder_->CreatePointerCast(
      buf, LLVMType(op->type)->getPointerTo(buf->getType()->getPointerAddressSpace()));
...
void CodeGenLLVM::VisitStmt_(const AttrStmt* op) {
  if (op->attr_key == attr::thread_extent) {
    ...
    const Variable* v = op->node.as<Variable>();
    CHECK(v);
    alloc_storage_info_[v].scope = runtime::StorageScope::make(op->value.as<StringImm>()->value);
  } else if (op->attr_key == ir::attr::storage_alignment) {
    const Variable* v = op->node.as<Variable>();
    CHECK(v);
    alloc_storage_info_[v].alignment = static_cast<int>(op->value.as<IntImm>()->value);
...
void CodeGenLLVM::VisitStmt_(const ProducerConsumer* op) { this->VisitStmt(op->body); }
...
llvm::Value* CodeGenLLVM::CreateExp(const Call* op) {
  ...
  Expr int_exp = ir::Call::make(int_type, ir::Call::shift_left, {integer + param_8, param_9},
                                ir::Call::Intrinsic);
...
llvm::Value* CodeGenLLVM::CreateMatrixTransposeBase(llvm::Value* dst_buffer,
                                                    llvm::Value* src_buffer, size_t row, size_t col,
                                                    size_t bits) {
#if TVM_LLVM_VERSION >= 110
  ...
  llvm::Value* dst_ptr =
      CreateBufferVecPtr(DataType(kDLUInt, bits, row * col), dst_buffer, ConstInt32(0));
  return builder_->CreateAlignedStore(dst, dst_ptr, align, true);
}

llvm::Value* CodeGenLLVM::CreateMatrixTranspose4x4(llvm::Value* dst_buffer, llvm::Value* src_buffer,
                                                   size_t row, size_t col, size_t bits) {
#if TVM_LLVM_VERSION >= 110
  ...
  ptr0 = CreateBufferVecPtr(DataType(kDLUInt, bits, col), dst_buffer, ConstInt32(0));
  ptr1 = CreateBufferVecPtr(DataType(kDLUInt, bits, col), dst_buffer, ConstInt32(1));
  ptr2 = CreateBufferVecPtr(DataType(kDLUInt, bits, col), dst_buffer, ConstInt32(2));
  ptr3 = CreateBufferVecPtr(DataType(kDLUInt, bits, col), dst_buffer, ConstInt32(3));

  auto row0 = builder_->CreateShuffleVector(tmp0, tmp2, low_h);
  store = builder_->CreateAlignedStore(row0, ptr0, llvm::Align(row), is_volatile);
  auto row1 = builder_->CreateShuffleVector(tmp0, tmp2, high_h);
  store = builder_->CreateAlignedStore(row1, ptr1, llvm::Align(row), is_volatile);
  auto row2 = builder_->CreateShuffleVector(tmp1, tmp3, low_h);
  store = builder_->CreateAlignedStore(row2, ptr2, llvm::Align(row), is_volatile);
  auto row3 = builder_->CreateShuffleVector(tmp1, tmp3, high_h);
  store = builder_->CreateAlignedStore(row3, ptr3, llvm::Align(row), is_volatile);
  return store;
}

llvm::Value* CodeGenLLVM::CreateMatrixTranspose8x4(llvm::Value* dst_buffer, llvm::Value* src_buffer,
                                                   size_t row, size_t col, size_t bits) {
#if TVM_LLVM_VERSION >= 110
  auto align = llvm::Align(col);
  ...
  ptr0 = CreateBufferVecPtr(DataType(kDLUInt, bits, row), dst_buffer, ConstInt32(0));
  ptr1 = CreateBufferVecPtr(DataType(kDLUInt, bits, row), dst_buffer, ConstInt32(1));
  ptr2 = CreateBufferVecPtr(DataType(kDLUInt, bits, row), dst_buffer, ConstInt32(2));
  ptr3 = CreateBufferVecPtr(DataType(kDLUInt, bits, row), dst_buffer, ConstInt32(3));
...
llvm::Value* CodeGenLLVM::CreateMatrixTranspose(const Call* op) {
  ...
  const int col_pos = 3;
  const int bit_pos = 4;
  llvm::Value* dst_buffer = MakeValue(op->args[0]);
  llvm::Value* src_buffer = MakeValue(op->args[1]);
  if (op->args[row_pos].as<IntImm>() == nullptr || op->args[col_pos].as<IntImm>() == nullptr) {
    LOG(FATAL) << "Fail to call MatrixTranspose function, The row or col must be IntImm!";
  }
  unsigned row = op->args[row_pos].as<IntImm>()->value;
  unsigned col = op->args[col_pos].as<IntImm>()->value;
  unsigned bits = 32;
  if (op->args.size() > bit_pos) {
    bits = op->args[bit_pos].as<IntImm>()->value;
  }
  if (row == 4 && col == 4) {
    return CreateMatrixTranspose4x4(dst_buffer, src_buffer, row, col, bits);
  } else if (row == 8 && col == 4) {
    return CreateMatrixTranspose8x4(dst_buffer, src_buffer, row, col, bits);
  }
  return CreateMatrixTransposeBase(dst_buffer, src_buffer, row, col, bits);
}
