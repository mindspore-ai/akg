...
class ThreadPool {
 public:
  ThreadPool(): num_workers_(air::runtime::threading::MaxConcurrency()) {
    ...
    threads_ = std::unique_ptr<air::runtime::threading::ThreadGroup>(
        new air::runtime::threading::ThreadGroup(
 ...
 private:
  ...
  std::unique_ptr<air::runtime::threading::ThreadGroup> threads_;
};
...
int TVMBackendParallelLaunch(
    ...
    int num_task) {
#if !TVM_THREADPOOL_USE_OPENMP
  int res = air::runtime::ThreadPool::ThreadLocal()->Launch(
      flambda, cdata, num_task, 1);
  return res;
#else
  int num_workers = air::runtime::threading::MaxConcurrency();
  ...
  #pragma omp parallel num_threads(num_workers)
  {
    ...
    std::atomic<int32_t>* sync_counter = new std::atomic<int>[num_task * air::runtime::kSyncStride];
    for (int i = 0; i < num_task; ++i) {
      sync_counter[i * air::runtime::kSyncStride].store(
          0, std::memory_order_relaxed);
    }
