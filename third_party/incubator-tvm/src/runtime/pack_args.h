...
/*
 * 2020.7.14 - Add function PackFuncVoidAddr_, PackFuncVoidAddrCCE
 */
...
template<int N, typename F>
inline PackedFunc PackFuncVoidAddr_(F f, const std::vector<ArgConvertCode>& codes, const int shape_nums) {
  int num_args = static_cast<int>(codes.size());
  auto ret = [f, codes, num_args, shape_nums](TVMArgs args, TVMRetValue* ret) {
    TempArray<void*, N> addr_(num_args);
    TempArray<ArgUnion, N> holder_(num_args);
    TempArray<int64_t, N> shape_(shape_nums);
    void** addr = addr_.data();
    ArgUnion* holder = holder_.data();
    int64_t* shape = shape_.data();
    int tempNum_shape = 0;
    for (int i = 0; i < num_args; ++i) {
      switch (codes[i]) {
        case INT64_TO_INT64: {
	        holder[i].v_int32 = static_cast<int32_t>(args.values[i].v_int64);
	        shape[tempNum_shape++] = args.values[i].v_int64;
	        break;
      	}
        case FLOAT64_TO_FLOAT64:
        case HANDLE_TO_HANDLE: {
    ...
    f(args, ret, addr, shape, tempNum_shape);
  };
  return PackedFunc(ret);
}
...
template<typename F>
inline PackedFunc PackFuncVoidAddrCCE(F f, const std::vector<TVMType>& arg_types) {
  std::vector<detail::ArgConvertCode> codes(arg_types.size());
  int shape_nums = 0;
  for (size_t i = 0; i < arg_types.size(); ++i) {
    codes[i] = detail::GetArgConvertCode(arg_types[i]);
    if (arg_types[i].code == kDLInt || arg_types[i].code == kDLUInt) {
      shape_nums++;
    }
  }
  size_t num_void_args = arg_types.size();
  // specialization
  if (num_void_args <= 4) {
    return detail::PackFuncVoidAddr_<4>(f, codes, shape_nums);
  } else if (num_void_args <= 8) {
    return detail::PackFuncVoidAddr_<8>(f, codes, shape_nums);
  } else {
    return detail::PackFuncVoidAddr_<0>(f, codes, shape_nums);
  }
}
