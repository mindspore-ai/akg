...
Stage& Stage::buffer_align(Array<Array<Expr>> buffer_aligns) {
  Array<IterVar> root_iter_vars = (*this)->op->root_iter_vars();
std::cout << "THIS:" << root_iter_vars << std::endl;
  size_t root_iter_size = root_iter_vars.size();
  CHECK_EQ(buffer_aligns.size(), root_iter_size)
    << "dim of buffer_aligns should be root vars's dim ("
    <<root_iter_size<<")\n";

  for (size_t i = 0; i < root_iter_size; i++) {
    auto dom = buffer_aligns[i];
    CHECK_EQ(dom.size(), 2)
      << "Range should be (min_align, extent_align)\n";
    (*this)->realize_aligns[root_iter_vars[i]] = std::pair<Expr, Expr>(dom[0], dom[1]);
  }

  (*this)->specify_realize_align = true;
  return *this;
}

Stage& Stage::buffer_tile(Array<Array<Expr>> buffer_tiles) {
  Array<IterVar> root_iter_vars = (*this)->op->root_iter_vars();
  size_t root_iter_size = root_iter_vars.size();
  CHECK_EQ(buffer_tiles.size(), root_iter_size)
          << "dim of buffer_tile should be root vars's dim ("
          <<root_iter_size<<")\n";

  for (size_t i = 0; i < root_iter_size; i++) {
    auto dom = buffer_tiles[i];
    CHECK_EQ(dom.size(), 2)
            << "Range should be (min, extent)\n";
    (*this)->realize_bound[root_iter_vars[i]] = Range::make_by_min_extent(dom[0], dom[1]);
  }

  (*this)->specify_realize_bound = true;
  return *this;
}
