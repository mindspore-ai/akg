...
bool is_depthwise(const QnnConv2DAttrs* param) {
  return param->channels.defined() && air::ir::Equal(param->channels, param->groups) &&
         param->groups != 1;
}
 ...
 */
WorkloadType GetWorkload(const Array<air::relay::Type>& arg_types, const QnnConv2DAttrs* param) {
  // Get conv parameters.
