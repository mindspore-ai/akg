...
class ExprBinder : public ExprMutator, PatternMutator {
 public:
  explicit ExprBinder(const air::Map<Var, Expr>& args_map)
    : args_map_(args_map) {
  }
 ...
 private:
  const air::Map<Var, Expr>& args_map_;
};

Expr Bind(const Expr& expr, const air::Map<Var, Expr>& args_map) {
