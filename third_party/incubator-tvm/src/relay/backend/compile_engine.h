...
struct CachedFuncNode : public Node {
  ...
  air::Array<Integer> shape_func_param_states;
...
class CCacheKeyNode : public Node {
 public:
  ...
  TVM_DECLARE_NODE_TYPE_INFO(CCacheKeyNode, air::Node);
...
class CCacheValueNode : public Node {
 public:
  ...
  TVM_DECLARE_NODE_TYPE_INFO(CCacheValueNode, air::Node);
};
...
// overload hash
template<>
struct hash<::air::relay::CCacheKey> {
  size_t operator()(const ::air::relay::CCacheKey& key) const {
    CHECK(key.defined());
    return key->Hash();
  }
};
