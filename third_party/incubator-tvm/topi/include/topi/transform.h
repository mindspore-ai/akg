...
inline Tensor concatenate(const Array<Tensor>& inputs,
  ...
  join_size = air::ir::Simplify(join_size);
  Array<Expr> out_shape;
  ...
  if (mode == "clip") {
    return compute(
        out_shape, [&](const Array<Var>& out_index) {
          auto idx = air::min(air::max(0, indices(out_index)), a_size - 1);
...
inline Tensor sequence_mask(const Tensor& data,
  ...
  Tensor out = compute(
      out_shape, [&](const Array<Var>& out_index) {
        ...
        Expr ret = air::if_then_else(air::cast(valid_length->dtype, tid) >= valid_length(len_index),
                                     air::make_const(data->dtype, mask_value), data(out_index));
        return ret;
      }, name, tag);
  return out;
}
  ...
  if (mode == "clip") {
    return compute(
        out_shape, [&](const Array<Var>& out_index) {
          ...
          auto idx = air::min(air::max(0, indices(indices_position)),
                              axis_dim - 1);
  ...
  if (condition->shape.size() != 1) {
    ...
    out = compute(
      oshape, [&](const Array<Var>& indices) {
        return air::ir::Select::make(condition(indices) != 0, x(indices), y(indices));
      }, name, tag);
  } else {
    ...
    out = compute(
      oshape, [&](const Array<Var>& indices) {
        ...
        return air::ir::Select::make(condition(condition_idx) != 0,
                                     x(indices), y(indices));
      }, name, tag);
  }
  return out;
          ...
          for (size_t i = 0; i < indices_dim0; ++i) {
            ...
            if (indices->dtype.is_int()) {
              ...
              real_indices.push_back(
                  air::cast(air::Int(32), indices(indices_position)));
            }
          }
 ...
 */
inline air::Tensor matmul(const air::Tensor& A,
                           const air::Tensor& B,
                           bool trans_a = false,
                           bool trans_b = false,
                           std::string name = "T_matmul",
                           std::string tag = kMatMul) {
  air::Array<air::Expr> output_shape{A->shape[trans_a ? 1 : 0],
                                     B->shape[trans_b ? 0 : 1]};
  auto k = air::reduce_axis(air::Range{0, A->shape[trans_a ? 0 : 1]}, "k");
  auto l = [&](air::Var i, air::Var j) {
    return air::sum((trans_a ? A[k][i] : A[i][k]) * (trans_b ? B[j][k] : B[k][j]),
                    {k});
  };
...
inline Tensor arange(const Expr& start,
                     ...
                     std::string tag = kInjective) {
  Expr num_elem = air::cast(air::Int(32), air::ceil(
      air::cast(air::Float(32), stop - start) / step));
  Array<Expr> shape;
  return compute({num_elem}, [&](const Array<Var>& indices) {
    return air::cast(dtype, start + step * indices[0]);
  }, name, tag);
}
...
inline Tensor shape(const Tensor& src,
  ...
  return compute(out_shape, [&](const Array<Var>& indices) {
    ...
    for (int i = 0; i < ndim; ++i) {
      ret = air::if_then_else(idx == i, src->shape[i], ret);
    }
    return air::cast(dtype, ret);
  }, name, tag);
}
