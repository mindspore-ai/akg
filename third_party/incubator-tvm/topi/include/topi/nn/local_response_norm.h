...
inline Tensor lrn(const Tensor& data,
  ...
  auto rxs = air::reduce_axis(Range(0, size), "rxs");
  Tensor sqr_sum;
  if (axis == 1) {
    sqr_sum = air::compute(input_shape,
                           [&](Var i, Var l, Var j, Var k) {
                           return air::sum(pad_data(i, l + rxs, j, k) *
    ...
    sqr_sum = air::compute(input_shape,
                           [&](Var i, Var l, Var j, Var k) {
                           return air::sum(pad_data(i, l, j, k + rxs) *
                                           pad_data(i, l, j, k + rxs),
                                           {rxs});
                           });
  }
  auto sqrt_sum_up = air::compute(
      input_shape,
