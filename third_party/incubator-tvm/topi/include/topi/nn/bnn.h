...
*/
inline air::Tensor binarize_pack(const air::Tensor& data,
                                 int axis,
  ...
  for (size_t i = 0; i < n; ++i) {
    oshape.push_back(i == static_cast<size_t>(axis) ?
                     air::ir::Simplify(indexdiv(ishape[i], 32)) :
                     ishape[i]);
  }
  ...
  return air::compute(
    ...
    [&](const Array<Var>& indices) {
      ...
      for (size_t j = 0; j < 32; ++j) {
        ...
        auto sign = air::cast(UInt(32), data(idx) >= 0);
        packed = (packed | sign);
        if (j == 31) {
          return packed;
        }
...
*/
inline air::Tensor binary_dense(const air::Tensor& data,
  ...
  auto k = air::reduce_axis(Range(0, in_dim), "k");
  auto matmul = air::compute(
    { batch, out_dim },
    [&](Var i, Var j) {
      return air::sum(popcount(data(i, k) ^ weight(j, k)), { k });
    }, "tensor", "binary_dense");
