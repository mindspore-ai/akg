...
"""operator dsl fuction: distr_normal_diag_logprob_ad"""
...
def dist_normal_diag_logprob_ad_set_dim_func(head, x, mean, scale):
    """
...
@ct_util.reg_set_dim_func(dist_normal_diag_logprob_ad_set_dim_func)
def normal_diag_logprob_ad(head, x, mean, scale):
    """
    An example of differentiating normal_diag.log_prob in all inputs and paramters
    ...
    """
    mod = normal_diag.normal_diag(mean, scale).log_prob(x)
    auto_diff_outs = list(akg.differentiate(mod, [x, mean, scale], head))
