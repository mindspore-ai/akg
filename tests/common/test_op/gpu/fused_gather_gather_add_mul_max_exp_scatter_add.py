...
@utils.check_input_type(tvm.tensor.Tensor, tvm.tensor.Tensor, tvm.tensor.Tensor)
...
@utils.check_input_type(tvm.tensor.Tensor, tvm.tensor.Tensor, tvm.tensor.Tensor, tvm.tensor.Tensor, int, (str, type(None)))
def fused_gather_gather_add_mul_max_exp_scatter_add(inp1, inp2, inp3, inp4, axis, target=utils.CUDA):
    ndim = len(inp1.shape)
    axis = axis + ndim if axis < 0 else axis
    assert axis >= 0
    assert axis < ndim
