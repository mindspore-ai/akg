...
def add_b_conv_compute(fmap_shape, filter_shape, pad_, stride_, dilation_,
    ...
    kernel_name = "add_b_conv_layer_" + str(in_n) + "_" + str(in_c) + "_" + str(in_h) + "_" + str(in_w) \
    ...
    a_value = akg.tvm.placeholder(input_shape_nc1hwc0, dtype=conv_dtype, name='a_value')
    # B_placeholder (fractal)
    b_tmp = akg.tvm.placeholder(kernel_shape_fractal, dtype=conv_dtype, name='b_tmp')
    b_value = akg.tvm.compute(b_tmp.shape, lambda ko, c1, c0, ki: b_tmp[ko, c1, c0, ki] + 1, name='b_value', attrs={'no_inline': 1})
    ...
    info = dim.Dim()
    if f_ko > 1:
        info.setdim(index=index, axis="KO", tilel1=f_ko, tilel0=f_ko)  # ko
    if f_no > 1:
        info.setdim(index=index, axis="C1", tilel1=c1_cut, tilel0=c1_cut)  # c1
    if f_ni > 1:
        info.setdim(index=index, axis="C0", tilel1=out_c0, tilel0=out_c0)  # c0
    if f_ki > 1:
        info.setdim(index=index, axis="KI", tilel1=f_ki, tilel0=f_ki)  # ki
    ...
    if in_c1 > 1:
        info.setdim(index=index, axis="KC1", tilel1=in_c1, tilel0=0)  # kc1
    if k_h > 1:
    ...
    if use_bias:
        ...
        cube = c_value
    return cube, a_value, b_tmp, bias_value, kernel_name, str(info)
...
def conv_relu(fmap_shape, filter_shape, pad_, stride_, dilation_,
              tile_hh=0, tile_coco=0, tile_mm=0, tile_kk=0, tile_nn=0, bypass_l1=False,
              use_bias=False, block_size=16, conv_dtype='float16'):
    conv, a_value, b_value, bias_value, kernel_name, dim_info = add_b_conv_compute(fmap_shape, 
                                                                                   filter_shape, pad_, stride_, dilation_,
