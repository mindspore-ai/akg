...
def conv_compute_forward(fmap_shape, filter_shape, pad_, stride_, dilation_, A, B, bias_value=None,
    ...
    # set dim
    set_dims(fmap_shape, filter_shape, pad_, stride_, dilation_,
...
def conv_01(fmap_shape, filter_shape, pad_, stride_, dilation_,
    ...
    conv, _ = Conv(data, fmap_shape, filter_shape, pad_, stride_, dilation_, use_bias)
...
def conv_02(fmap_shape, filter_shape, pad_, stride_, dilation_,
    ...
    with akg.build_config(add_lower_pass=debug_mode(0), dump_pass_ir=True):
        _ = akg.build(s_data, [pld_Head_strided, pld_B_flipped, conv_data], "cce",
    ...
    with akg.build_config(add_lower_pass=debug_mode(0), dump_pass_ir=True):
        akg.build(s_weight, [pld_A_transposed, pld_Head_transposed_converted, conv_weight], "cce",
