...
def bouding_box_encode(anchor_box, groundtruth_box, anchor_samples, scale_factors, epsilon=1e-5, target="cce"):
    """
    ...
    ymin_a = akg.tvm.compute(anchor_coordinate_shape, lambda j0: reducer(
        akg.lang.ascend.extract0(anchor_box[j0, k0]), axis=k0), name="ymin_a")
    k1 = akg.tvm.reduce_axis((0, 8), name='k1')
    xmin_a = akg.tvm.compute(anchor_coordinate_shape, lambda j1: reducer(
        akg.lang.ascend.extract1(anchor_box[j1, k1]), axis=k1), name="xmin_a")
    k2 = akg.tvm.reduce_axis((0, 8), name='k2')
    ymax_a = akg.tvm.compute(anchor_coordinate_shape, lambda j2: reducer(
        akg.lang.ascend.extract2(anchor_box[j2, k2]), axis=k2), name="ymax_a")
    k3 = akg.tvm.reduce_axis((0, 8), name='k3')
    xmax_a = akg.tvm.compute(anchor_coordinate_shape, lambda j3: reducer(
        akg.lang.ascend.extract3(anchor_box[j3, k3]), axis=k3), name="xmax_a")
    # get center coordinates and sizes for anchor
    width_a_raw = akg.lang.ascend.vsub(xmax_a, xmin_a)
    height_a_raw = akg.lang.ascend.vsub(ymax_a, ymin_a)
    height_a_half = akg.lang.ascend.vmuls(height_a_raw, akg.tvm.const(0.5, dtype))
    ycenter_a_raw = akg.lang.ascend.vadd(ymin_a, height_a_half)
    width_a_half = akg.lang.ascend.vmuls(width_a_raw, akg.tvm.const(0.5, dtype))
    xcenter_a_raw = akg.lang.ascend.vadd(xmin_a, width_a_half)
    ...
    width = akg.lang.ascend.vsub(xmax, xmin)
    height = akg.lang.ascend.vsub(ymax, ymin)
    height_half = akg.lang.ascend.vmuls(height, akg.tvm.const(0.5, dtype))
    ycenter = akg.lang.ascend.vadd(ymin, height_half)
    width_half = akg.lang.ascend.vmuls(width, akg.tvm.const(0.5, dtype))
    xcenter = akg.lang.ascend.vadd(xmin, width_half)
    ...
    epsilon_ = akg.lang.ascend.broadcast(akg.tvm.const(epsilon, dtype), anchor_samples_shape)
    h_a = akg.lang.ascend.vadd(height_a, epsilon_)
    w_a = akg.lang.ascend.vadd(width_a, epsilon_)
    h = akg.lang.ascend.vadd(height, epsilon_)
    w = akg.lang.ascend.vadd(width, epsilon_)
    xc_sub_xc_a = akg.lang.ascend.vsub(xcenter, xcenter_a)
    w_a_rec = akg.lang.ascend.vrec(w_a)
    h_a_rec = akg.lang.ascend.vrec(h_a)
    tx = akg.lang.ascend.vmul(xc_sub_xc_a, w_a_rec)
    yc_sub_yc_a = akg.lang.ascend.vsub(ycenter, ycenter_a)
    ty = akg.lang.ascend.vmul(yc_sub_yc_a, h_a_rec)
    h_div_h_a = akg.lang.ascend.vmul(h, h_a_rec)
    th = akg.lang.ascend.vlog(h_div_h_a)
    w_div_w_a = akg.lang.ascend.vmul(w, w_a_rec)
    tw = akg.lang.ascend.vlog(w_div_w_a)
    if scale_factors:
        ty = akg.lang.ascend.vmuls(ty, akg.tvm.const(scale_factors[0], dtype))
        tx = akg.lang.ascend.vmuls(tx, akg.tvm.const(scale_factors[1], dtype))
        th = akg.lang.ascend.vmuls(th, akg.tvm.const(scale_factors[2], dtype))
        tw = akg.lang.ascend.vmuls(tw, akg.tvm.const(scale_factors[3], dtype))
