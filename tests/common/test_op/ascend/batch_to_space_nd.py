...
def check_inputs(data, block_shape, crops):
    ...
    utils.check_equal("length of block_shape", "length of crops", len(block_shape), len(crops))

@utils.check_input_type(akg.tvm.tensor.Tensor, (tuple, list), (tuple, list), (str, type(None)))
def batch_to_space_nd(data, block_shape, crops, target=utils.CCE):
    """
