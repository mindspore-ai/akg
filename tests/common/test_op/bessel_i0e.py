...
from tests.common.test_op.minimum import minimum
...
def _bessel_i0e_compute(input_data):
    ...
    broad_const_limit = akg.lang.cce.broadcast(akg.tvm.const(CONST_LIMIT, "float32"), shape_input)
    before_abs_data = minimum(abs_data, broad_const_limit)
    data = topi.multiply(before_abs_data, 1.0 / CONST_LIMIT)
    square_data = mul(data, data)
    ...
    for iter_number in ITR_BEFORE[LEN_BEFORE-3::-1]:
        before_res = mul(before_res, square_data)
        before_res = topi.add(before_res, iter_number)
    exp_data = exp(neg(before_abs_data))
    before_res = mul(before_res, exp_data)
    ...
    rsqrt_data = rsqrt(abs_data)
    after_res = mul(after_res, rsqrt_data)
    after_res = minimum(before_res, after_res)

    # chose the type of data in end
    if dtype_input == "float16":
        after_res = cast(after_res, "float16")
