...
from tests.common.test_op import axpy
...
def axpy_run(shape1, shape2, alpha, dtype, attrs):
    ...
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(axpy.axpy, [shape1, shape2], [dtype, dtype], kernel_name=kernel_name,
        ...
        mod = utils.op_build_test(axpy.axpy, [shape1, shape2], [dtype, dtype], kernel_name='axpy', op_attrs=[alpha],
                                  attrs=attrs)
        args, exp_output, inputs1, inputs2 = gen_data(alpha, dtype, shape1, shape2)
        # result_tvm
