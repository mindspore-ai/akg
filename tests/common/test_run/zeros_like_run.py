...
from akg.ops.nn import zeros_like
...
def zeros_like_run(shape, dtype, attrs):
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(zeros_like.zeros_like, [shape], [dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(zeros_like.zeros_like, [shape], [dtype], kernel_name='zeros_like', attrs=attrs)
        input, expect, output = gen_data(dtype, shape)
