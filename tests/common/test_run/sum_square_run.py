...
from tests.common.test_op import abs_sum
from akg.utils.dsl_create import get_reduce_out_shape
from tests.common.gen_random import random_gaussian

def sum_square_run(shape, reduce_axis, keepdims, dtype, attrs):
    ...
    if 'tuning' in attrs.keys():
        ...
        expect, input1, output = gen_data(dtype, keepdims, reduce_axis, shape)
        mod = utils.op_build_test(abs_sum.abs_sum, [shape], [dtype], op_attrs, attrs=attrs)
...
def gen_data(dtype, keepdims, reduce_axis, shape):
    ...
    input1_square = np.abs(input1)
    expect = np.sum(input1_square, axis=reduce_axis, keepdims=keepdims)
