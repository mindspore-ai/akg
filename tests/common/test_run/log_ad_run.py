...
from akg.ops.math import log_ad
...
def log_ad_run(shape, dtype, kernel_name, attrs):
    ...
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(log_ad.log_ad, input_shape, input_dtype, kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(log_ad.log_ad, input_shape, input_dtype, kernel_name=kernel_name, attrs=attrs)
        expect, head_np, input, output = gen_data(dtype, shape)
...
def gen_data(dtype, shape):
    input = np.random.uniform(low=1e-4, high=10.0, size=shape).astype(dtype)
    head_np = np.random.uniform(low=0, high=1.0, size=shape).astype(dtype)
    expect = 1 / input * head_np
