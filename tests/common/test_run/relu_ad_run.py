...
from akg.ops.nn.relu_ad import relu_ad
...
def relu_ad_run(shape, dtype, attrs):
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(relu_ad, [shape, shape], [dtype, dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(relu_ad, [shape, shape], [dtype, dtype], kernel_name='relu_ad', attrs=attrs)
        expect, head_np, input_np, output = gen_data(dtype, shape)
