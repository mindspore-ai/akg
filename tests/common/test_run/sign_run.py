...
from tests.common.test_op import sign
...
def sign_run(shape, dtype, attrs):
    ...
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(sign.sign, in_shape, in_dtype, kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        expect, input, output = gen_data(dtype, shape)
        mod = utils.op_build_test(sign.sign, in_shape, in_dtype, kernel_name='sign', attrs=attrs)
