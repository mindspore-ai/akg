...
def matmul_tensoradd(x, y, c, b, out_dtype, left_format="zZ", right_format="nZ", out_format="zN", transpose_x=False, transpose_y=False, attrs={}):
    matmul_res, attrs = matmul.matmul(x, y, b, out_dtype, left_format, right_format, out_format, transpose_x, transpose_y, attrs=None)
    res, attr_map= add.TensorAdd(matmul_res, c)
    return res, attrs

def matmul_add_execute(shape_x, shape_y, bias, cadd, left_format, right_format, out_format, adj_x, adj_y, dtype, bias_dtype, out_dtype, kernel_name, attrs={}):
    ...
    mod = matmul_add_compile(shape_x, shape_y, bias, out_shape, left_format, right_format, out_format, adj_x, adj_y, dtype, bias_dtype, out_dtype, kernel_name, attrs={})
    # Generate data
    ...
    cadd_data = random_gaussian(out_shape, miu=10, sigma=0.01).astype(out_dtype)
    bench_mark = np.add(bench_mark , cadd_data)
    # mod launch
...
def matmul_add_compile(shape_x, shape_y, bias, cadd, left_format, right_format, output_format, adj_x, adj_y, dtype, bias_dtype, out_dtype, kernel_name, attrs, tuning=False):
    batch_tuple, m, k, n = extract_dim(shape_x, shape_y, adj_x, adj_y)
    m = (m + 15) // 16 * 16
    n = (n + 15) // 16 * 16
    k = (k + 15) // 16 * 16
    ...
    return utils.op_build_test(matmul_tensoradd, input_shapes, input_types, op_attrs, kernel_name, attrs=attrs, tuning=tuning)
