...
from akg.ops.math import floordiv
...
def floordiv_run(shape1, shape2, dtype, kernel_name, attrs):
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(floordiv.floordiv, [shape1, shape2], [dtype, dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(floordiv.floordiv, [shape1, shape2], [dtype, dtype], kernel_name=kernel_name, attrs=attrs)
        expect, input, input1, input2, output = gen_data(dtype, shape1, shape2)
