...
from akg.ops.math import floor
...
def floor_run(shape, dtype, kernel_name, attrs):
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(floor.floor, [shape], [dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(floor.floor, [shape], [dtype], kernel_name=kernel_name, attrs=attrs)
        expect, input, output = gen_data(dtype, shape)
