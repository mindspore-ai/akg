...
from test_op import minimum
from akg.utils import validation_check as vc_util
from tensorio import compare_tensor
from gen_random import random_gaussian

def minimum_run(shape1, shape2, dtype, attrs):
    """minimum_run"""
    if 'tuning' in attrs.keys():
        t = attrs.get("tuning", False)
        kernel_name = attrs.get("kernel_name", False)
        mod = utils.op_build_test(minimum.minimum, [shape1, shape2], [dtype, dtype],
        ...
        mod = utils.op_build_test(minimum.minimum, [shape1, shape2], [dtype, dtype], kernel_name='minimum', attrs=attrs)
        expect, lhd, output, rhd = gen_data(dtype, shape1, shape2)
        # result_tvm
