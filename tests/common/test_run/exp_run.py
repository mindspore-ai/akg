...
def exp_run(shape, dtype, attrs):
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(exp.exp, [shape], [dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(exp.exp, [shape], [dtype], kernel_name='exp', attrs=attrs)
        expect, input, output = gen_data(dtype, shape)
