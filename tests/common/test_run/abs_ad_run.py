...
from akg.ops.math import abs_ad
...
def abs_ad_run(shape, dtype, attrs):
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(abs_ad.abs_ad, [shape, shape], [dtype, dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(abs_ad.abs_ad, [shape, shape], [dtype, dtype], kernel_name='abs_ad', attrs=attrs)
        expect, head_np, input_np, output = gen_data(dtype, shape)
