...
from akg.ops.nn.ascend import MaxPool, OldMaxPool
...
def maxpool_run(shape, kernel, stride, pad, hybrid, dtype, attrs_op={}, polyhedral=True, attrs={}):
    ...
    if 'tuning' in attrs.keys():
        ...
        if polyhedral:
            if hybrid:
                ...
                mod = utils.op_build_test(OldMaxPool, [build_shape], [dtype], op_attrs=[kernel, stride, pad],
                                          kernel_name='maxpool_old', attrs=attrs)
        else:
            from akg.ops.nn.ascend.maxpool import maxpool_manual_schedule
            mod = maxpool_manual_schedule(build_shape, kernel, stride, pad, dtype,attrs=attrs, polyhedral=polyhedral)
        expect, input, out_shape, res = gen_data(dtype, kernel, pad, shape, stride)
