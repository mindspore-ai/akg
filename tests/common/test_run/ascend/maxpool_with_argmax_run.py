...
from akg.ops.nn.ascend import MaxPool, MaxPoolWithArgmax, MaxpoolWithArgmaxDynamic
...
def maxpool_with_argmax_run(shape, kernel, stride, pad, dsl, dtype, attrs_op={}, polyhedral=True, attrs={}):
    build_shape = []
    arg_list = []
    ...
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(MaxPoolWithArgmax,
        ...
        if polyhedral:
            if attrs.get("dynamic") and len(build_shape) > 0:
                mod = utils.op_build_test(MaxpoolWithArgmaxDynamic,
            ...
            mod = maxpool_manual_schedule(shape, kernel, stride, pad, dtype, attrs=attrs, polyhedral=polyhedral)
        input, expects, outputs = \
