...
from tests.common.test_op.ascend.sqrt_ad import sqrt_ad


def sqrt_ad_run(shape, dtype, attrs):
    if 'tuning' in attrs.keys():
        t = attrs.get("tuning", False)
        kernel_name = attrs.get("kernel_name", False)
        mod = utils.op_build_test(sqrt_ad, [shape, shape], [dtype, dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        expect, head_np, input_np, output = gen_data(dtype, shape)
        mod = utils.op_build_test(sqrt_ad, [shape, shape], [dtype, dtype], kernel_name='sqrt_ad', attrs=attrs)
...
def gen_data(dtype, shape):
    ...
    expect = (0.5 * np.power(input_np, -0.5)) * head_np
