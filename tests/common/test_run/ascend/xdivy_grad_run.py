...
"""run function for xdivy_grad"""

import numpy as np
from tests.common.tensorio import compare_tensor
from akg.utils import kernel_exec as utils
from tests.common.test_op.ascend import xdivy_grad
from tests.common.gen_random import random_gaussian
from tests.common.base import get_rtol_atol
from akg.utils.dsl_create import produce_shapes


def xdivy_grad_run(shape1, shape2, dtype, attrs):
    _, _, grad_shape = produce_shapes(shape1, shape2)
    mod = utils.op_build_test(xdivy_grad.xdivy_grad,
                              [shape1, shape2, grad_shape],
                              [dtype, dtype, dtype],
                              kernel_name="xdivy_grad", attrs=attrs)
    expects, inputs, outputs = gen_data(shape1, shape2, dtype)
    reses = utils.mod_launch(
        mod, (*inputs, *outputs), expect=expects,
        outputs=(-2, -1))

    rtol, atol = get_rtol_atol("xdivy_grad", dtype)
    TestCase_Results = list(map(lambda x, y: compare_tensor(
        x, y, rtol=rtol, atol=atol, equal_nan=False), reses, expects))
