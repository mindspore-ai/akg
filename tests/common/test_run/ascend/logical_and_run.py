...
from akg.ops.math import logical_and

def logical_and_run(shape1, shape2, dtype, kernel_name, attrs_op=None, attrs=None):
    ...
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(logical_and, [shape1, shape2], [dtype, dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(logical_and, [shape1, shape2], [dtype, dtype], kernel_name=kernel_name, attrs=attrs)
        expect, input1, input2, output = gen_data(shape1, shape2)
