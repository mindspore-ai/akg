...
def realdiv_run(shape, shape2, dtype, kernel_name, attrs_op={}, cce_path="./", attrs={}):
    attrs.update(attrs_op)
    if 'tuning' in attrs.keys():
        t = attrs.get("tuning", False)
        kernel_name = attrs.get("kernel_name", False)
        mod = utils.op_build_test(RealDiv, [shape, shape2], [dtype, dtype], kernel_name=kernel_name,
