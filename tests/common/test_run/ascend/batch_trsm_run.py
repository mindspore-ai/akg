...
from tests.common.test_op.ascend import batch_trsm
def batch_trsm_run(shape1, shape2, dtype, attrs):
    mod = utils.op_build(batch_trsm.batch_trsm, [shape1, shape2], [dtype,dtype], kernel_name='batch_trsm', attrs=attrs)
    exp_output, inputs1, inputs2, output = gen_data(dtype, shape1,shape2)
    #result_tvm
    acu_output=utils.mod_launch(mod,(inputs1, inputs2, output), expect=exp_output)
...
def gen_data(dtype, shape1, shape2):
    ...
    for batch_idx in range(batch_size):
        ...
        batch_input = np.linalg.cholesky(batch_input)
        batch_input = np.transpose(batch_input)
        batch_input = batch_input.astype(dtype)
        inputs1[batch_idx, :, :] = batch_input
        inputs2[batch_idx, :, :] = np.identity(dim).astype(dtype)

        batch_exp_out = np.linalg.solve(batch_input, inputs2[batch_idx, :, :])
