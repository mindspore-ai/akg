...
from akg.ops.math.ascend import LogAd
...
def log_ad_run(shape, dtype, kernel_name, attrs):
    ...
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(LogAd, input_shape, input_dtype, kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(LogAd, input_shape, input_dtype, kernel_name=kernel_name, attrs=attrs)
        expect, head_np, input, output = gen_data(dtype, shape)
