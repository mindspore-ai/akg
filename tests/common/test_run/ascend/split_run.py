...
def split_run(shape, num_or_size_splits, split_axis, dtype, attrs):
    ...
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(Split, [shape], [dtype], op_attrs, kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        arg1, arg2, expect, input = gen_data(dtype, num_or_size_splits, shape, split_axis)
        mod = utils.op_build_test(Split, [shape], [dtype], op_attrs, kernel_name='split', attrs=attrs)
