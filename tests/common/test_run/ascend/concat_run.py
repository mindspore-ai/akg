...
def concat_run(shapes, dtype, axis, attrs):
    ...
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(Concat, [shapes], [dtype.lower()], op_attrs, kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(Concat, [shapes], [dtype.lower()], op_attrs, kernel_name='concat', attrs=attrs)
        args, expect, inputs = gen_data(axis, dtype, shapes)
