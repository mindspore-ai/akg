...
from tests.common.test_op import not_equal
from tests.common.gen_random import random_gaussian


def not_equal_run(shapes, dtype, kernel_name, attrs, cce_path="./"):
    if 'tuning' in attrs.keys():
        t = attrs.get("tuning", False)
        kernel_name = attrs.get("kernel_name", False)
        mod = utils.op_build_test(not_equal.not_equal, shapes, [dtype, dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(not_equal.not_equal, shapes, [dtype, dtype], kernel_name=kernel_name, attrs=attrs)
        benchMark1, inputs1, output1 = gen_data(shapes)
