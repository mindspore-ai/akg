...
from tests.common.test_op import logical_not



def logical_not_run(shape1, dtype, kernel_name, attrs, cce_path="./"):
    if 'tuning' in attrs.keys():
        t = attrs.get("tuning", False)
        kernel_name = attrs.get("kernel_name", False)
        mod = utils.op_build_test(logical_not.logical_not, [shape1], [dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(logical_not.logical_not, [shape1], [dtype], kernel_name=kernel_name, attrs=attrs)
        expect, input1, output = gen_data(shape1)
