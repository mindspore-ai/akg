...
from tests.common.test_op import logical_or



def logical_or_run(shape1, shape2, dtype, kernel_name, attrs, cce_path="./"):
    if 'tuning' in attrs.keys():
        t = attrs.get("tuning", False)
        kernel_name = attrs.get("kernel_name", False)
        mod = utils.op_build_test(logical_or.logical_or, [shape1, shape2], [dtype, dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(logical_or.logical_or, [shape1, shape2], [dtype, dtype], kernel_name=kernel_name, attrs=attrs)
        expect, input1, input2, output = gen_data(shape1, shape2)
...
def gen_data(shape1, shape2):
    ...
    expect = np.logical_or(input1, input2)
