...
import numpy as np
import akg
from akg.ops.nn.gpu import ConvFusion
...
def conv_fusion_run(shape_data, shape_filter1, shape_filter2, stride1, stride2, padding1, padding2, dilation1, dilation2, dtype, out_dtype="float32", poly_sch=True, attrs=None):
    if not attrs:
        attrs = {"target": "cuda"}
    ...
    mod = utils.op_build_test(ConvFusion, (shape_data, shape_filter1, shape_filter2), (dtype, dtype, dtype), op_attrs=op_attrs, attrs=attrs, polyhedral=poly_sch, kernel_name="conv_fusion_auto")
    ...
    if attrs["profiling"]:
        ...
        target_profiling(mod, data, weight1, weight2, output, target=target_name, repeat_time=attrs["repeat_times"])
