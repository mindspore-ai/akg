...
TEST(TestLabelRealizeOutPosition, TestCase1) {
  isl::schedule input_sch;
  isl::schedule expect_ouput_sch;

  std::tie(input_sch, expect_ouput_sch) = ScheduleTreeHelper("label_realize_out_position_case").Prepare();
  input_sch = ir::poly::LabelRealizeOutPosition().Run(input_sch);
