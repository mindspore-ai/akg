...
TEST(TestMarkOuterMost, TestCase1) {
  ...
  std::tie(input_sch, expect_ouput_sch) = ScheduleTreeHelper("mark_outer_most_case").Prepare();
  input_sch = ir::poly::MarkOuterMost(scop_info).Run(input_sch);
