...
TEST(TestComputeSchedule, TestCase1) {
  isl::schedule input_schedule;
  isl::schedule expect_schedule;
  std::tie(input_schedule, expect_schedule) = ScheduleTreeHelper("compute_schedule_case").Prepare();
...
"{ S_1[i0, i1, i2, k1] -> S_1[i0' = i0, i1' = i1, i2' = i2, k1' = 1 + k1] : 0 <= i0 <= 63 and 0 <= i1 <= 15 and 0 <= i2 <= 127 and 0 <= k1 <= 126;"
 ...
 " S_7[ax0, ax1, ax2, ax3] -> S_8[ax0' = ax0, ax1' = ax1, ax2' = ax2, ax3' = ax3] : 0 <= ax0 <= 63 and 0 <= ax1 <= 15 and 0 <= ax2 <= 127 and 0 <= ax3 <= 127 }";
  ...
  // compute schedule 
  input_schedule = ir::poly::ComputeSchedule(pass_info, scop_info).Run(input_schedule);

  // ungroup
  input_schedule = ir::poly::UnGroupStatements(pass_info).Run(input_schedule);

  ret = input_schedule.plain_is_equal(expect_schedule);
  EXPECT_EQ(ret, true);
