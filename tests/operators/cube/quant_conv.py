...
def test_CCE_Conv(fmap_shape, filter_shape, pad_, stride_,
    ...
    Quant = akg.tvm.compute(out_shape_nc1hwc0,
                        lambda qn, qc1, qh, qw, qc0: (A[qn, qc1 + qc0 // 16, qh, qw, qc0 % 16] * ScaleQ[0] + OffsetQ[0]).astype('int8'), name='QuantOUT', attrs={'no_inline': 1})
    ...
    if use_bias:
        cube = akg.tvm.compute(out_shape_nc1hwc0, lambda n, c1, h, w, c0: C[n, c1, h, w, c0] + bias_value[0, c1, 0, 0, c0], name=output_bias_name)
    else:
        cube = C
    ...
    with akg.build_config(add_lower_pass=cce.debug_mode(0), dump_pass_ir=True):
        if fusion:
            if use_bias:
                mod = akg.build(s, [A, B, ScaleQ, OffsetQ, bias_value, out], "cce", name=kernel_name, attrs=attrs, attrs={"dim": info}, polyhedral=True)
            else:
...
if __name__ == '__main__':
    ...
    for conv_layer in resnet50_workload:
        ...
        test_CCE_Conv((in_n, in_c, in_h, in_w), (cout, in_c, w_h, w_w), (pad_top, pad_left), (stride_h, stride_w),
                      tile_hh=cutH, tile_coco=cutCo, tile_mm=cutM, tile_kk=cutK, tile_nn=cutN, bypass_l1=conv_layer.bypass_l1,
