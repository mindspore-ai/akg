...
def test_ms_conv(shape_data, shape_filter, stride, padding, dilation, dtype, out_dtype="float32", poly_sch=True):
    op_attrs = [stride, padding, dilation]
    attrs = {"target":"cuda", "enable_auto_fuse":False, "shared_memory_tensors":"input_1 input_2",
             "dim":" 0 0 1 1 0 1 1 1 0 2 8 8 0 3 56 56 0 4 2 2", "bind_block":"64 32", "bind_thread":"28 4"}

    if poly_sch:
        mod = utils.op_build_test(conv, (shape_data, shape_filter), (dtype, dtype), op_attrs=op_attrs, attrs=attrs, kernel_name="conv_auto")

    data, weight, output, expect = gen_data(shape_data, shape_filter, stride, padding, dilation, dtype, out_dtype)
    ...
    data, weight, output, expect = to_tvm_nd_array([data, weight, output, expect])
