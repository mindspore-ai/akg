...
def test_fused_gather_gather_add_mul_max_exp_scatter_add(input1_shape, input2_shape, input3_shape, input4_shape,
    ...
    if poly_sch:
        mod = utils.op_build_test(fused_gather_gather_add_mul_max_exp_scatter_add,
                                  ...
                                  op_attrs=op_attrs, attrs=default_attrs,
    ...
    inputs = to_tvm_nd_array([input1, input2, input3, input4])
    expects = to_tvm_nd_array([expect1, expect2])
    gpu_profiling(mod, *inputs, *expects, repeat_time=400)
