...
from akg.ops.math_gpu.reduce_and import reduce_and
...
def gen_data(in_shape, in_dtype, axis, keepdims):
    ...
    expect = np.all(data, axis=axis, keepdims=keepdims)
    if axis == None and keepdims == False:
        expect = np.broadcast_to(expect, (1,))
    output = np.full(expect.shape, 1, in_dtype)
    return data, output, expect


def test_ms_reduce_and(in_shape, in_dtype, axis=None, keepdims=False, poly_sch=False):
    if poly_sch:
        mod = utils.op_build_test(reduce_and, (in_shape, ), (in_dtype, ), op_attrs=[
                             axis, keepdims], kernel_name="reduce_and", attrs={"target": "cuda", 
