...
def test_ms_cumprod(shape, dtype, axis=0, exclusive=False, reverse=False, poly_sch=False):
    def cumprod(input):
    ...
    if poly_sch:
        mod = utils.op_build_test(cumprod, [shape], [dtype], kernel_name="cumprod", attrs={"target": "cuda"})
