...
def test_fused_relu_grad_bn_double_reduce_grad(shape, out_shape, dtype="float32", layout="NHWC", out_dtype="float16", poly_sch=True, attrs=None):
    if not attrs:
        attrs = {"target": "cuda"}
    ...
    mod = utils.op_build_test(fused_relu_grad_bn_double_reduce_grad, shape_list, dtype_list, op_attrs=op_attrs,
            kernel_name="fused_relu_grad_bn_double_reduce_grad", polyhedral=poly_sch, attrs=attrs)
    ...
    if attrs["profiling"]:
        ...
        target_profiling(mod, *arg_list, target=target_name, repeat_time=attrs["repeat_time"])
