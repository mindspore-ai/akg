...
def test_ms_abs(shape, dtype, poly_sch=False, attrs=None):
    if not attrs:
        attrs = {"target": "cuda"}
    mod = utils.op_build_test(abs_data, [shape], [dtype], kernel_name="abs", polyhedral=poly_sch, attrs=attrs)
    ...
    if attrs["profiling"]:
        ...
        target_profiling(mod, data, output, target=target_name, repeat_time=attrs["repeat_time"])
