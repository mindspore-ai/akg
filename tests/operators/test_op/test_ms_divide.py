...
def test_ms_divide(shape, dtype, poly_sch=True, attrs=None):
    if not attrs:
        attrs = {"target": "cuda"}
    mod = utils.op_build_test(divide, [shape, shape], [dtype, dtype], kernel_name="divide", polyhedral=poly_sch, attrs=attrs)
