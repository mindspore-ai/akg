...
from akg.ops.array_gpu.tile import tile

def gen_data(shape, multiples, dtype):
    support_list = {"float16": np.float16, "float32": np.float32}
    data = random_gaussian(shape, miu=1, sigma=0.1).astype(support_list[dtype])
    expect = np.tile(data, multiples)
    output = np.full(expect.shape, np.nan, dtype)
    return data, output, expect

def test_ms_tile(shape, multiples, dtype, poly_sch=True, attrs=None):
    if not attrs:
        attrs = {"target": "cuda"}
    mod = utils.op_build_test(tile, [shape], [dtype], op_attrs=[multiples], kernel_name="tile", polyhedral=poly_sch, attrs=attrs)
