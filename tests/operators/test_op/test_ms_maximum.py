...
def gen_data(shape1, shape2, dtype):
    ...
    expect = np.maximum(lhs, rhs)
    output = np.full(expect.shape, np.nan, expect.dtype)
    return lhs, rhs, output, expect

def test_ms_maximum(shape1, shape2, dtype, poly_sch=True, attrs=None):
    if not attrs:
        attrs = {"target": "cuda"}
    mod = utils.op_build_test(maximum, (shape1, shape2), (dtype, dtype), kernel_name="maximum", polyhedral=poly_sch, attrs=attrs)
