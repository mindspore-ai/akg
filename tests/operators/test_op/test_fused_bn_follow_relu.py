...
from tests.common.test_op.resnet.fused_bn_follow_relu import fused_bn_follow_relu
...
def test_fused_bn_follow_relu(in_shape, in_dtype='float16', layout='NHWC', out_dtype='float16', poly_sch=True, attrs=None):
    if not attrs:
        attrs = {"target": "cuda"}
    ...
    mod = utils.op_build_test(fused_bn_follow_relu, input_shape_list, input_dtype_list,
            kernel_name="fused_bn_follow_relu", op_attrs=op_attrs, polyhedral=poly_sch, attrs=attrs)
    ...
    if attrs["profiling"]:
        ...
        target_profiling(mod, *arglist, target=target_name, repeat_time=attrs["repeat_time"])
