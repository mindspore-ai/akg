...
def test_ms_reshape(dtype, in_shape, out_shape, poly_sch=True, attrs=None):
    if not attrs:
        attrs = {"target": "cuda"}
    mod = utils.op_build_test(reshape, [in_shape], [dtype], [out_shape],
                    kernel_name="reshape", polyhedral=poly_sch, attrs=attrs)
