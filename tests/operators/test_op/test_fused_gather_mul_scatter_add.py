...
def test_fused_gather_mul_scatter_add(input1_shape, input2_shape, input3_shape, input4_shape, data_dtype, indices_type, axis, poly_sch=True, attrs=None):
    op_attrs = [axis]
    if not attrs:
        attrs = {"target": "cuda"}
    ...
    if attrs["profiling"]:
        ...
        target_profiling(mod, input1, input2, input3, input4, output, target=target_name, repeat_time=attrs["repeat_time"])
