...
from akg.ops.math_gpu.reduce_min import reduce_min
...
def test_ms_reduce_min(in_shape, in_dtype, axis=None, keepdims=False, poly_sch=True,attrs=None):
    if not attrs:
        attrs = {"target": "cuda"}
    attrs.update({"enable_akg_reduce_lib": True, "enable_atomic_add": True})
    mod = utils.op_build_test(reduce_min, (in_shape, ), (in_dtype, ), kernel_name="reduce_min",
                    op_attrs=[axis, keepdims], polyhedral=poly_sch, attrs=attrs)
