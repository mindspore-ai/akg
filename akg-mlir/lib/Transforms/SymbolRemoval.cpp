...
using namespace MemoryEffects;  // NOLINT(build/namespaces)
...
void SymbolRemovalPass::runOnOperation() {
  ...
  for (auto loadOp : loadOps) {
    ...
    // dimCount = loadOp.getIndices().size(), symbolCount = 0
    auto newMap = AffineMap::get(loadOp.getIndices().size(), 0, exprs, map.getContext());
  ...
  for (auto storeOp : storeOps) {
    ...
    // dimCount = storeOp.getIndices().size(), symbolCount = 0
    auto newMap = AffineMap::get(storeOp.getIndices().size(), 0, exprs, map.getContext());
