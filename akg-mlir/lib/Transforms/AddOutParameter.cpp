...
static LogicalResult buildInverseShapeChainOnOut(Value outArg, ArrayRef<ShapeOpInChain> chain,
                                                 memref::AllocOp rootAlloc, Value &allocViewFromOut) {
  if (chain.empty()) {
  ...
  for (const auto &it : chain) {
    ...
    if (auto c = dyn_cast<memref::CollapseShapeOp>(op)) {
      ...
      auto newExpand = b.create<memref::ExpandShapeOp>(c.getLoc(), srcTy, cur, reassoc);
      ...
      auto newCollapse = b.create<memref::CollapseShapeOp>(e.getLoc(), srcTy, cur, reassoc);
  ...
  if (cur.getType() != rootAlloc.getResult().getType()) {
    rootAlloc.emitError() << "inverse shape chain constructed value type " << cur.getType()
