...
Axis::Axis(size_t bandIdx, size_t axisIdx, affine::AffineForOp affineLoop) : bandIdx(bandIdx), axisIdx(axisIdx) {
...
Axis::Axis(size_t bandIdx, size_t axisIdx, mlir::scf::ForOp scfLoop) : bandIdx(bandIdx), axisIdx(axisIdx) {
  auto nameSuffix = std::to_string(bandIdx) + "_" + std::to_string(axisIdx);
  this->name = "Axis_" + nameSuffix;
  // Use empty deleter since MLIR manages Operation lifetime
  this->loop =
