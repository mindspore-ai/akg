...
static mlir::LogicalResult PrimeNumReplace(Operation *funcOp) {
  ...
  funcOp->walk([&builder, &ctx, &tool, funcOp](scf::ParallelOp parallelOp) {
    ...
    if (!isa<mlir::arith::ConstantOp>(upperBound.getDefiningOp())) {
      ...
      auto constAttr = builder.getIndexAttr(static_cast<int64_t>(primeNum));
      ...
      mlir::IntegerAttr int32Attr =
        mlir::IntegerAttr::get(mlir::IntegerType::get(ctx, 32), static_cast<int64_t>(primeNum));
...
static void RestoreValueAndRemoveKeepArgs(Operation *funcOp, gpu::LaunchOp &gpuLaunch,
                                          const std::vector<ConvertInfo> &convertInfos) {
...
// we can move on. When left passes can handle dynamic shape, we restore those prime numbers.
