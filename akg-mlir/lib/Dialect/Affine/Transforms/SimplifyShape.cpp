    ...
    m.walk([&](mlir::memref::GlobalOp globalop) {
      ...
      if (resultType != resultSimplifyType) {
        ...
        if (elementsAttr) {
          ...
          llvm::errs() << DEBUG_TYPE << " - Unknown initValue type cannot replace the GlobalOp";
        }
      }
    });
...
std::unique_ptr<mlir::OperationPass<mlir::ModuleOp>> mlir::createSimplifyShapePass() {
  return std::make_unique<mlir::SimplifyShapePass>();
}

std::unique_ptr<mlir::OperationPass<mlir::ModuleOp>> mlir::createSimplifyShapePass(bool keepArg) {
