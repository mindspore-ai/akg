...
std::string getOutputSymbol(mlir::DictionaryAttr dictAttr, const std::string &target) {
  for (auto &keyValuePair : dictAttr) {
    ...
    if (std::any_of(symbols.begin(), symbols.end(), [&target](const std::string &s) { return s == target; })) {
      return key;
    }
  }
...
std::tuple<int, int> getOutputFromSymbol(const std::string &target, const std::vector<ShapeInfo> &shapeInfoList) {
  int argIdx = 0;
