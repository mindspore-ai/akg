...
std::unique_ptr<OperationPass<func::FuncOp>> createAKGLoopTilingPass() { return std::make_unique<AKGLoopTiling>(); }
...
static std::pair<int, affine::AffineForOp> findInnermostLoopWithDepth(affine::AffineForOp loop) {
  std::pair<int, affine::AffineForOp> best{1, loop};
...
affine::AffineIfOp AKGLoopTiling::createperfectlyNestedCondition(SmallVector<affine::AffineForOp, 6> tiledLoops,
  ...
  for (auto loop : tiledLoops) {
    ...
    AffineExpr newExpr = upperExpr - 1 - getAffineDimExpr(dimNum++, context);
...
SmallVector<affine::AffineForOp> AKGLoopTiling::collectOutermostTiledLoops(affine::AffineForOp bandRootLoop) {
...
AKGLoopTiling::MappingContext AKGLoopTiling::collectAndSelectMappingDimension(affine::AffineForOp bandRootLoop) {
  MappingContext ctx = {};
...
affine::AffineForOp AKGLoopTiling::createKernelLoopAndMapFullBlock(OpBuilder &builder, func::FuncOp funcOp,
                                                                   MappingContext &ctx) {
  ...
  auto kernelLoop = builder.create<affine::AffineForOp>(loc, 0, ctx.numKernels, 1);
...
void AKGLoopTiling::mapTailBlockToKernel(OpBuilder &builder, affine::AffineForOp kernelLoop,
                                         affine::AffineForOp fullLoop, affine::AffineForOp tailLoop, int32_t numKernels,
                                         Value kernelId) {
  if (!tailLoop) {
    return;
  }
...
void AKGLoopTiling::addOuterKernelMappingLoop(affine::AffineForOp bandRootLoop) {
...
affine::AffineForOp AKGLoopTiling::findTiledBandRoot(Block *parentBlock, func::FuncOp funcOp) {
...
void AKGLoopTiling::collectAndMarkInnermostLoops(func::FuncOp funcOp) {
