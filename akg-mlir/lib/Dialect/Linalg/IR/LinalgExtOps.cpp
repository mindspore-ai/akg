...
static void addOperandSegmentSizesAttr(OpAsmParser &parser, OperationState &result, size_t numInputs,
                                       size_t numOutputs) {
  auto sizes =
    parser.getBuilder().getDenseI32ArrayAttr({static_cast<int32_t>(numInputs), static_cast<int32_t>(numOutputs)});
  if (result.propertiesAttr) {
    NamedAttrList attrs = llvm::cast<DictionaryAttr>(result.propertiesAttr);
    attrs.append("operandSegmentSizes", sizes);
    result.propertiesAttr = attrs.getDictionary(parser.getContext());
  } else {
    result.addAttribute("operandSegmentSizes", sizes);
  }
}
...
void TemplateOp::build(OpBuilder &builder, OperationState &result, TypeRange resultTensorTypes, ValueRange inputs,
                       ValueRange outputs, ArrayAttr indexingMaps, ArrayAttr iteratorTypes, StringAttr doc,
                       StringAttr libraryCall, function_ref<void(OpBuilder &, Location, ValueRange)> bodyBuild,
  ...
  Block *bodyBlock = builder.createBlock(&region, region.end(), blockArgTypes, blockArgLocs);
...
void TemplateOp::build(OpBuilder &builder, OperationState &result, TypeRange resultTensorTypes, ValueRange inputs,
  ...
  build(builder, result, resultTensorTypes, inputs, outputs, builder.getAffineMapArrayAttr(indexingMaps),
        ...
        libraryCall.empty() ? StringAttr() : builder.getStringAttr(libraryCall), bodyBuild, attributes);
}

void TemplateOp::build(OpBuilder &builder, OperationState &result, ValueRange inputs, ValueRange outputs,
                       ArrayRef<AffineMap> indexingMaps, ArrayRef<utils::IteratorType> iteratorTypes, StringRef doc,
                       StringRef libraryCall, function_ref<void(OpBuilder &, Location, ValueRange)> bodyBuild,
                       ArrayRef<NamedAttribute> attributes) {
  build(builder, result, TypeRange{}, inputs, outputs, indexingMaps, iteratorTypes, doc, libraryCall, bodyBuild,
        attributes);
}
...
void TemplateOp::print(OpAsmPrinter &p) {
  ...
  for (auto attr : (*this)->getAttrs()) {
    if (attr.getName() == getIteratorTypesAttrName()) {
      ...
      genericAttrs.emplace_back(getIteratorTypesAttrName(), ArrayAttr::get(getContext(), iteratorTypeNames));
...
void TemplateOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>> &effects) { return; }

#define GET_OP_CLASSES
#include "akg/Dialect/Linalg/IR/LinalgExtOps.cpp.inc"  // NOLINT(build/include)
