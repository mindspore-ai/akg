...
static bool isDimSequencePreservedV2(AffineMap indexingMap, ReassociationIndicesRef dimSequence) {
...
class FoldReshapeWithGenericOpByCollapsing : public OpRewritePattern<tensor::CollapseShapeOp> {
 public:
  ...
  LogicalResult matchAndRewrite(tensor::CollapseShapeOp collapseOp, PatternRewriter &rewriter) const override {
    ...
    auto fuseInitOperand = genericOp.getDpsInitOperand(producerResult.getResultNumber());

    SmallVector<ReassociationIndices> collapsableIterationDims =
      getCollapsableIterationSpaceDims(genericOp, fuseInitOperand, collapseOp.getReassociationIndices());
    if (collapsableIterationDims.empty()) {
      return rewriter.notifyMatchFailure(collapseOp, "index map cannot be collapsed");
    }
