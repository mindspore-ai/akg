...
static LogicalResult computeReshapeOutput(ArrayRef<int64_t> higherRankShape, ArrayRef<int64_t> lowerRankShape,
  ...
  for (int64_t i = higherRank - 1, j = lowerRank - 1; i >= 0 && j >= 0; i--, j--) {
...
static LogicalResult EqualizeRanks(PatternRewriter &rewriter, Location loc, Value &input1, Value &input2) {
  ...
  auto reshapeOutputType =
    RankedTensorType::get(ArrayRef<int64_t>(reshapeOutputShape), reshapeInputType.getElementType());

  auto reshapeLower = rewriter.create<mindspore::ReshapeOp>(loc, reshapeOutputType, lowerTensorValue,
...
LogicalResult reshapeLowerToHigher(PatternRewriter &rewriter, Location loc, RankedTensorType outputType, Value &input1,
                                   Value &input2) {
...
struct ConvertMindsporeOp : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy mindsporeBinaryOp, PatternRewriter &rewriter) const override {
...
struct ConvertMindsporeOp<mindspore::SelectOp> : public OpRewritePattern<mindspore::SelectOp> {
  ...
  LogicalResult matchAndRewrite(mindspore::SelectOp mindsporeOp, PatternRewriter &rewriter) const override {
...
struct AlignDimension : public impl::AlignDimensionBase<AlignDimension> {
 public:
