...
static void createBroadCastOp(PatternRewriter &rewriter, Value input, Value output) {
  ...
  auto newShapeAttr = DenseI64ArrayAttr::get(context, ArrayRef<int64_t>(outputType.getShape()));
  allAttrs.emplace_back(NamedAttribute(StringAttr::get(context, "new_shape"), newShapeAttr));
  DictionaryAttr symbolAttrs = addOpSymShapeAttr(input.getType(), context);
  allAttrs.emplace_back(StringAttr::get(context, getFrontendSymbolAttrName()), symbolAttrs);

  auto elemTy = getElementTypeOrSelf(input.getType());
  auto resultType = RankedTensorType::get(outputType.getShape(), elemTy);
  auto brcOp = rewriter.create<mindspore::BroadcastToOp>(input.getLoc(), resultType, input, allAttrs);
...
struct ConvertMindsporeOp : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy msOp, PatternRewriter &rewriter) const override {
...
struct ConvertMindsporeOp<mindspore::SelectOp> : public OpRewritePattern<mindspore::SelectOp> {
  ...
  LogicalResult matchAndRewrite(mindspore::SelectOp selectOp, PatternRewriter &rewriter) const override {
...
struct MindsporeMakeBroadcastable : public impl::MindsporeMakeBroadcastableBase<MindsporeMakeBroadcastable> {
 public:
