  ...
  LogicalResult matchAndRewrite(Operation *op, PatternRewriter &rewriter) const override {
    ...
    auto forOp = rewriter.create<scf::ForOp>(
      loc, c0, cTotal, cChunk, std::nullopt, [&](OpBuilder &b, Location bodyLoc, Value iv, ValueRange) {
        ...
        std::transform(
          reads.begin(), reads.end(), std::back_inserter(chunkOperands),
          [&](vector::TransferReadOp readOp) { return buildChunkTransferRead(b, bodyLoc, readOp, iv, cfg.chunkTy); });
...
static bool buildBroadcastDimMap(ArrayRef<int64_t> dimAttr, int64_t srcRank, SmallVector<int64_t, 4> &m) {
  if (dimAttr.empty()) {
    m.clear();
    for (int64_t i = 0; i < srcRank; ++i) m.push_back(i);
    return true;
  }
...
static bool sortAxesByDimMap(const SmallVector<int64_t, 4> &m, int64_t srcRank, SmallVector<int64_t, 4> &perm) {
  perm.resize(srcRank);
  for (int64_t i = 0; i < srcRank; ++i) perm[i] = i;
...
static Value applyBroadcastSourceTranspose(PatternRewriter &rewriter, Location loc, VectorType srcTy, Value srcVal,
  ...
  for (int64_t t = 0; t < srcRank; ++t) transShape.push_back(srcTy.getDimSize(perm[t]));
  laidOutTy = VectorType::get(transShape, elemTy);
  return rewriter.create<vector::TransposeOp>(loc, laidOutTy, srcVal, rewriter.getDenseI64ArrayAttr(perm)).getResult();
}

static bool fillBroadcastInsertShape(const SmallVector<int64_t, 4> &m, const SmallVector<int64_t, 4> &perm,
                                     int64_t srcRank, int64_t dstRank, VectorType laidOutTy,
...
struct ArithWithNpuTypesToVector : public RewritePattern {
  explicit ArithWithNpuTypesToVector(MLIRContext *ctx) : RewritePattern(Pattern::MatchAnyOpTypeTag{}, 1, ctx) {}
