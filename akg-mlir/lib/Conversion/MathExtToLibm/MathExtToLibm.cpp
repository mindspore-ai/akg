...
using namespace mlir::mathExt;    // NOLINT(build/namespaces)
...
LogicalResult VecOpToScalarOp<Op>::matchAndRewrite(Op op, PatternRewriter &rewriter) const {
  ...
  for (auto linearIndex = 0; linearIndex < numElements; ++linearIndex) {
    ...
    std::transform(op->getOperands().begin(), op->getOperands().end(), std::back_inserter(operands), [&](Value input) {
      return rewriter.create<vector::ExtractOp>(loc, input, positions)
