...
#include "akg/Conversion/FuncToLLVMExt/FuncToLLVMExt.h"
...
#define GEN_PASS_DEF_FUNCTOLLVMEXT
#include "akg/Conversion/Passes.h.inc"
}  // namespace mlir

using namespace mlir;        // NOLINT(build/namespaces)
using namespace mlir::LLVM;  // NOLINT(build/namespaces)

#define PASS_NAME "convert-func-to-llvm-ext"
...
static bool shouldUseBarePtrCallConv(Operation *op, const LLVMTypeConverter *typeConverter) {
  return (op && op->hasAttr(barePtrAttrName)) || typeConverter->getOptions().useBarePtrCallConv;
}
...
static void filterFuncAttributes(FunctionOpInterface func, SmallVectorImpl<NamedAttribute> &result) {
  for (const NamedAttribute &attr : func->getDiscardableAttrs()) {
    if (attr.getName() == linkageAttrName || attr.getName() == varargsAttrName ||
...
static void propagateArgResAttrs(OpBuilder &builder, bool resultStructType, FunctionOpInterface funcOp,
                                 LLVM::LLVMFuncOp wrapperFuncOp) {
  auto argAttrs = funcOp.getAllArgAttrs();
  if (!resultStructType) {
    if (auto resAttrs = funcOp.getAllResultAttrs()) wrapperFuncOp.setAllResultAttrs(resAttrs);
    if (argAttrs) wrapperFuncOp.setAllArgAttrs(argAttrs);
  } else {
  ...
  cast<FunctionOpInterface>(wrapperFuncOp.getOperation()).setVisibility(funcOp.getVisibility());
}
...
/// the extra arguments.
static void wrapForExternalCallers(OpBuilder &rewriter, Location loc, const LLVMTypeConverter &typeConverter,
                                   FunctionOpInterface funcOp, LLVM::LLVMFuncOp newFuncOp) {
  auto type = cast<FunctionType>(funcOp.getFunctionType());
  auto [wrapperFuncType, resultStructType] = typeConverter.convertFunctionTypeCWrapper(type);

  SmallVector<NamedAttribute> attributes;
  filterFuncAttributes(funcOp, attributes);

  auto wrapperFuncOp = rewriter.create<LLVM::LLVMFuncOp>(loc, llvm::formatv("_mlir_ciface_{0}", funcOp.getName()).str(),
  ...
  for (auto [index, argType] : llvm::enumerate(type.getInputs())) {
    ...
    if (auto memrefType = dyn_cast<MemRefType>(argType)) {
      Value loaded = rewriter.create<LLVM::LoadOp>(loc, typeConverter.convertType(memrefType), arg);
      MemRefDescriptor::unpack(rewriter, loc, loaded, memrefType, args);
      continue;
    }
    if (isa<UnrankedMemRefType>(argType)) {
      Value loaded = rewriter.create<LLVM::LoadOp>(loc, typeConverter.convertType(argType), arg);
...
static void wrapExternalFunction(OpBuilder &builder, Location loc, const LLVMTypeConverter &typeConverter,
                                 FunctionOpInterface funcOp, LLVM::LLVMFuncOp newFuncOp) {
  OpBuilder::InsertionGuard guard(builder);

  auto [wrapperType, resultStructType] =
    typeConverter.convertFunctionTypeCWrapper(cast<FunctionType>(funcOp.getFunctionType()));
  ...
  auto wrapperFunc = builder.create<LLVM::LLVMFuncOp>(loc, llvm::formatv("_mlir_ciface_{0}", funcOp.getName()).str(),
  ...
  if (resultStructType) {
    ...
    Value result = builder.create<LLVM::AllocaOp>(loc, resultType, resultStructType, one);
  ...
  for (Type input : type.getInputs()) {
    ...
    if (memRefType || unrankedMemRefType) {
      ...
      Value packed = memRefType ? MemRefDescriptor::pack(builder, loc, typeConverter, memRefType,
                                                         wrapperArgsRange.take_front(numToDrop))
                                : UnrankedMemRefDescriptor::pack(builder, loc, typeConverter, unrankedMemRefType,
                                                                 wrapperArgsRange.take_front(numToDrop));

      auto ptrTy = LLVM::LLVMPointerType::get(builder.getContext());
      Value one = builder.create<LLVM::ConstantOp>(loc, typeConverter.convertType(builder.getIndexType()),
                                                   builder.getIntegerAttr(builder.getIndexType(), 1));
      Value allocated = builder.create<LLVM::AllocaOp>(loc, ptrTy, packed.getType(), one, /*alignment=*/0);
  ...
  if (resultStructType) {
    Value result = builder.create<LLVM::LoadOp>(loc, resultStructType, args.front());
...
static void modifyFuncOpToUseBarePtrCallingConv(ConversionPatternRewriter &rewriter, Location loc,
  ...
  assert(blockArgs.size() == oldArgTypes.size() && "The number of arguments and types doesn't match");
  ...
  for (auto it : llvm::zip(blockArgs, oldArgTypes)) {
    ...
    // unranked memrefs.
    assert(!isa<UnrankedMemRefType>(argTy) && "Unranked memref is not supported");
    ...
    Value desc = MemRefDescriptor::fromStaticShape(rewriter, loc, typeConverter, memrefTy, arg);
    rewriter.replaceOp(placeholder, {desc});
  }
}

static void convertAndSetFunctionArgAttrs(FunctionOpInterface funcOp, LLVM::LLVMFuncOp newFuncOp,
                                          const LLVMTypeConverter &converter, PatternRewriter &rewriter,
                                          const TypeConverter::SignatureConversion &result, Type llvmType) {
  ArrayAttr argAttrDicts = funcOp.getAllArgAttrs();
  if (!argAttrDicts) return;
  SmallVector<Attribute> newArgAttrs(cast<LLVM::LLVMFunctionType>(llvmType).getNumParams());
  for (unsigned i = 0, e = funcOp.getNumArguments(); i < e; ++i) {
    ...
    for (const NamedAttribute &attr : attrsDict) {
      const auto convertTypeAttr = [&](const NamedAttribute &attr) {
        return TypeAttr::get(converter.convertType(cast<TypeAttr>(attr.getValue()).getValue()));
      };
      if (attr.getName().getValue() == LLVM::LLVMDialect::getByValAttrName()) {
        convertedAttrs.push_back(rewriter.getNamedAttr(LLVM::LLVMDialect::getByValAttrName(), convertTypeAttr(attr)));
      } else if (attr.getName().getValue() == LLVM::LLVMDialect::getByRefAttrName()) {
        convertedAttrs.push_back(rewriter.getNamedAttr(LLVM::LLVMDialect::getByRefAttrName(), convertTypeAttr(attr)));
      } else if (attr.getName().getValue() == LLVM::LLVMDialect::getStructRetAttrName()) {
        convertedAttrs.push_back(
          rewriter.getNamedAttr(LLVM::LLVMDialect::getStructRetAttrName(), convertTypeAttr(attr)));
      } else if (attr.getName().getValue() == LLVM::LLVMDialect::getInAllocaAttrName()) {
        convertedAttrs.push_back(
          rewriter.getNamedAttr(LLVM::LLVMDialect::getInAllocaAttrName(), convertTypeAttr(attr)));
      } else {
    ...
    if (mapping->size == 1) {
      newArgAttrs[mapping->inputNo] = DictionaryAttr::get(rewriter.getContext(), convertedAttrs);
      continue;
    }
    for (size_t j = 0; j < mapping->size; ++j)
      newArgAttrs[mapping->inputNo + j] = DictionaryAttr::get(rewriter.getContext(), {});
  }
  if (!newArgAttrs.empty()) newFuncOp.setAllArgAttrs(rewriter.getArrayAttr(newArgAttrs));
}

FailureOr<LLVM::LLVMFuncOp> mlir::convertFuncOpToLLVMExtFuncOp(FunctionOpInterface funcOp,
                                                               ConversionPatternRewriter &rewriter,
                                                               const LLVMTypeConverter &converter) {
  // Check the funcOp has `FunctionType`.
  auto funcTy = dyn_cast<FunctionType>(funcOp.getFunctionType());
  if (!funcTy) return rewriter.notifyMatchFailure(funcOp, "Only support FunctionOpInterface with FunctionType");

  // Convert the original function arguments. They are converted using the
  // LLVMTypeConverter provided to this legalization pattern.
  auto varargsAttr = funcOp->getAttrOfType<BoolAttr>(varargsAttrName);
  TypeConverter::SignatureConversion result(funcOp.getNumArguments());
  auto llvmType = converter.convertFunctionSignature(funcTy, varargsAttr && varargsAttr.getValue(),
                                                     shouldUseBarePtrCallConv(funcOp, &converter), result);
  if (!llvmType) return rewriter.notifyMatchFailure(funcOp, "signature conversion failed");

  // Create an LLVM function, use external linkage by default until MLIR
  // functions have linkage.
  LLVM::Linkage linkage = LLVM::Linkage::External;
  if (funcOp->hasAttr(linkageAttrName)) {
    auto attr = dyn_cast<mlir::LLVM::LinkageAttr>(funcOp->getAttr(linkageAttrName));
    if (!attr) {
      funcOp->emitError() << "Contains " << linkageAttrName << " attribute not of type LLVM::LinkageAttr";
      return rewriter.notifyMatchFailure(funcOp, "Contains linkage attribute not of type LLVM::LinkageAttr");
    }
  ...
  filterFuncAttributes(funcOp, attributes);
  auto newFuncOp =
    rewriter.create<LLVM::LLVMFuncOp>(funcOp.getLoc(), funcOp.getName(), llvmType, linkage,
                                      /*dsoLocal=*/false, /*cconv=*/LLVM::CConv::C, /*comdat=*/nullptr, attributes);
  cast<FunctionOpInterface>(newFuncOp.getOperation()).setVisibility(funcOp.getVisibility());
  ...
  if (funcOp->hasAttr(readnoneAttrName)) {
    ...
    if (!attr) {
      funcOp->emitError() << "Contains " << readnoneAttrName << " attribute not of type UnitAttr";
      return rewriter.notifyMatchFailure(funcOp, "Contains readnone attribute not of type UnitAttr");
    }
    auto memoryAttr = LLVM::MemoryEffectsAttr::get(
      rewriter.getContext(), {LLVM::ModRefInfo::NoModRef, LLVM::ModRefInfo::NoModRef, LLVM::ModRefInfo::NoModRef});
  ...
  if (ArrayAttr resAttrDicts = funcOp.getAllResultAttrs()) {
    ...
    if (funcOp.getNumResults() == 1) newFuncOp.setAllResultAttrs(resAttrDicts);
  }
  convertAndSetFunctionArgAttrs(funcOp, newFuncOp, converter, rewriter, result, llvmType);

  rewriter.inlineRegionBefore(funcOp.getFunctionBody(), newFuncOp.getBody(), newFuncOp.end());
  if (failed(rewriter.convertRegionTypes(&newFuncOp.getBody(), converter, &result))) {
    return rewriter.notifyMatchFailure(funcOp, "region types conversion failed");
  }
...
struct FuncOpConversionBase : public ConvertOpToLLVMPattern<func::FuncOp> {
 protected:
  ...
  FailureOr<LLVM::LLVMFuncOp> convertFuncOpToLLVMExtFuncOp(func::FuncOp funcOp,
                                                           ConversionPatternRewriter &rewriter) const {
    return mlir::convertFuncOpToLLVMExtFuncOp(cast<FunctionOpInterface>(funcOp.getOperation()), rewriter,
                                              *getTypeConverter());
  }
};
...
struct FuncOpConversion : public FuncOpConversionBase {
  explicit FuncOpConversion(const LLVMTypeConverter &converter) : FuncOpConversionBase(converter) {}

  LogicalResult matchAndRewrite(func::FuncOp funcOp, OpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const override {
    FailureOr<LLVM::LLVMFuncOp> newFuncOp = convertFuncOpToLLVMExtFuncOp(funcOp, rewriter);
    if (failed(newFuncOp)) return rewriter.notifyMatchFailure(funcOp, "Could not convert funcop");

    if (!shouldUseBarePtrCallConv(funcOp, this->getTypeConverter())) {
      if (funcOp->getAttrOfType<UnitAttr>(LLVM::LLVMDialect::getEmitCWrapperAttrName())) {
        ...
        if (newFuncOp->isExternal())
          wrapExternalFunction(rewriter, funcOp->getLoc(), *getTypeConverter(), funcOp, *newFuncOp);
        else
          wrapForExternalCallers(rewriter, funcOp->getLoc(), *getTypeConverter(), funcOp, *newFuncOp);
      }
    } else {
      modifyFuncOpToUseBarePtrCallingConv(rewriter, funcOp->getLoc(), *getTypeConverter(), *newFuncOp,
...
struct ConstantOpLowering : public ConvertOpToLLVMPattern<func::ConstantOp> {
  ...
  LogicalResult matchAndRewrite(func::ConstantOp op, OpAdaptor adaptor,
    ...
    if (!type || !LLVM::isCompatibleType(type)) return rewriter.notifyMatchFailure(op, "failed to convert result type");

    auto newOp = rewriter.create<LLVM::AddressOfOp>(op.getLoc(), type, op.getValue());
    for (const NamedAttribute &attr : op->getAttrs()) {
      if (attr.getName().strref() == "value") continue;
...
struct CallOpInterfaceLowering : public ConvertOpToLLVMPattern<CallOpType> {
  ...
  LogicalResult matchAndRewriteImpl(CallOpType callOp, typename CallOpType::Adaptor adaptor,
                                    ConversionPatternRewriter &rewriter, bool useBarePtrCallConv = false) const {
    ...
    if (numResults != 0) {
      if (!(packedResult = this->getTypeConverter()->packFunctionResults(resultTypes, useBarePtrCallConv)))
        return failure();
    }
#if 1
    ...
    auto promoted = this->getTypeConverter()->promoteOperands(callOp.getLoc(), /*opOperands=*/callOp->getOperands(),
                                                              adaptor.getOperands(), rewriter, useBarePtrCallConv);
    auto newOp = rewriter.create<LLVM::CallOp>(callOp.getLoc(), packedResult ? TypeRange(packedResult) : TypeRange(),
    ...
    if (numResults < 2) {
      ...
      for (unsigned i = 0; i < numResults; ++i) {
        results.push_back(rewriter.create<LLVM::ExtractValueOp>(callOp.getLoc(), newOp->getResult(0), i));
      }
    }
    ...
    if (useBarePtrCallConv) {
      ...
      // descriptors.
      assert(results.size() == resultTypes.size() && "The number of arguments and types doesn't match");
      this->getTypeConverter()->promoteBarePtrsToDescriptors(rewriter, callOp.getLoc(), resultTypes, results);
    } else if (failed(this->copyUnrankedDescriptors(rewriter, callOp.getLoc(), resultTypes, results,
                                                    /*toDynamic=*/false))) {
      return failure();
    }
...
class CallOpLowering : public CallOpInterfaceLowering<func::CallOp> {
 public:
  CallOpLowering(const LLVMTypeConverter &typeConverter,
      ...
      : CallOpInterfaceLowering<func::CallOp>(typeConverter, benefit), symbolTable(symbolTable) {}
  ...
  LogicalResult matchAndRewrite(func::CallOp callOp, OpAdaptor adaptor,
    ...
    if (getTypeConverter()->getOptions().useBarePtrCallConv) {
      ...
      // Fast lookup.
      Operation *callee = symbolTable->lookup(callOp.getCalleeAttr().getValue());
      useBarePtrCallConv = callee != nullptr && callee->hasAttr(barePtrAttrName);
    } else {
      // Warning: This is a linear lookup.
      Operation *callee = SymbolTable::lookupNearestSymbolFrom(callOp, callOp.getCalleeAttr());
      useBarePtrCallConv = callee != nullptr && callee->hasAttr(barePtrAttrName);
    }
...
struct CallIndirectOpLowering : public CallOpInterfaceLowering<func::CallIndirectOp> {
  using Super::Super;

  LogicalResult matchAndRewrite(func::CallIndirectOp callIndirectOp, OpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const override {
...
struct ReturnOpLowering : public ConvertOpToLLVMPattern<func::ReturnOp> {
  ...
  LogicalResult matchAndRewrite(func::ReturnOp op, OpAdaptor adaptor,
    ...
    bool useBarePtrCallConv = shouldUseBarePtrCallConv(funcOp, this->getTypeConverter());
    if (useBarePtrCallConv) {
      ...
      for (auto it : llvm::zip(op->getOperands(), adaptor.getOperands())) {
        ...
        if (isa<MemRefType>(oldTy) && getTypeConverter()->canConvertToBarePtr(cast<BaseMemRefType>(oldTy))) {
      ...
      (void)copyUnrankedDescriptors(rewriter, loc, op.getOperands().getTypes(), updatedOperands,
                                    /*toDynamic=*/true);
    }

    // If ReturnOp has 0 or 1 operand, create it and return immediately.
    if (numArguments <= 1) {
      rewriter.replaceOpWithNewOp<LLVM::ReturnOp>(op, TypeRange(), updatedOperands, op->getAttrs());
      return success();
    }

    // Otherwise, we need to pack the arguments into an LLVM struct type before
    // returning.
    auto packedType = getTypeConverter()->packFunctionResults(op.getOperandTypes(), useBarePtrCallConv);
    if (!packedType) {
    ...
    rewriter.replaceOpWithNewOp<LLVM::ReturnOp>(op, TypeRange(), packed, op->getAttrs());
    return success();
  }
};
}  // namespace

void mlir::populateFuncToLLVMExtFuncOpConversionPattern(LLVMTypeConverter &converter, RewritePatternSet &patterns) {
  patterns.add<FuncOpConversion>(converter);
}

void mlir::populateFuncToLLVMExtConversionPatterns(LLVMTypeConverter &converter, RewritePatternSet &patterns,
                                                   const SymbolTable *symbolTable) {
  populateFuncToLLVMExtFuncOpConversionPattern(converter, patterns);
...
class ConvertFuncToLLVMExtPass : public impl::FuncToLLVMExtBase<ConvertFuncToLLVMExtPass> {
 public:
  using Base::Base;
  ...
  void runOnOperation() override {
    ...
    auto dataLayoutAttr = dyn_cast_or_null<StringAttr>(m->getAttr(LLVM::LLVMDialect::getDataLayoutAttrName()));
    if (dataLayoutAttr) dataLayout = dataLayoutAttr.getValue();

    if (failed(LLVM::LLVMDialect::verifyDataLayoutString(
          dataLayout, [this](const Twine &message) { getOperation().emitError() << message.str(); }))) {
      signalPassFailure();
      return;
    }
    ...
    if (failed(applyPartialConversion(m, target, std::move(patterns)))) signalPassFailure();
  }
};
