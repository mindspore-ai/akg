...
// conversion.
// Implement a cleaner solution, factoring out the "matching" logic
...
static LogicalResult checkAffineLoopNestMappableImpl(affine::AffineForOp forOp, unsigned numDims) {
  ...
  for (unsigned i = 0, e = numDims; i < e; ++i) {
    ...
    auto end = forOp.getBody()->end();
    if (forOp.getBody()->empty() || std::next(begin, 2) != end)
      return forOp.emitError("expected perfectly nested loops in the body");

    if (!(forOp = dyn_cast<affine::AffineForOp>(nested))) return nested->emitError("expected a nested loop");
  }
  return success();
}
...
std::optional<affine::AffineForOp> AffineLoopToGpuConverter::collectBounds(affine::AffineForOp forOp,
  ...
  for (unsigned i = 0; i < numLoops; ++i) {
    ...
    if (i != numLoops - 1) currentLoop = cast<affine::AffineForOp>(&currentLoop.getBody()->front());
  }
  return currentLoop;
}
...
static LogicalResult convertAffineLoopNestToGPULaunch(affine::AffineForOp forOp, unsigned numBlockDims,
                                                      unsigned numThreadDims) {
  if (failed(checkAffineLoopNestMappable(forOp, numBlockDims, numThreadDims))) return failure();

  AffineLoopToGpuConverter converter;
  auto maybeInnerLoop = converter.collectBounds(forOp, numBlockDims + numThreadDims);
  if (!maybeInnerLoop) return failure();
...
static Value deriveStaticUpperBound(Value upperBound, PatternRewriter &rewriter) {
  ...
  if (auto minOp = upperBound.getDefiningOp<arith::MinSIOp>()) {
    if (auto staticBound = deriveStaticUpperBound(minOp.getLhs(), rewriter)) {
      return staticBound;
    } else if (auto staticBound = deriveStaticUpperBound(minOp.getRhs(), rewriter)) {
      return staticBound;
    }
  }

  if (auto multiplyOp = upperBound.getDefiningOp<arith::MulIOp>()) {
    auto lhsOp = dyn_cast_or_null<arith::ConstantIndexOp>(
      deriveStaticUpperBound(multiplyOp.getOperand(0), rewriter).getDefiningOp());
    if (!lhsOp) return {};
    auto rhsOp = dyn_cast_or_null<arith::ConstantIndexOp>(
      deriveStaticUpperBound(multiplyOp.getOperand(1), rewriter).getDefiningOp());
    if (!rhsOp) return {};
    if ((lhsOp.value() < 0) != (rhsOp.value() < 0)) return {};
    return rewriter.create<arith::ConstantIndexOp>(multiplyOp.getLoc(), lhsOp.value() * rhsOp.value());
  }
...
/// one scope-level up.
static bool isLaunchIndependent(Value val, gpu::LaunchOp launchOp) {
  return val.getParentRegion()->isAncestor(launchOp->getParentRegion());
}

static Value ensureLaunchIndependentValue(Value val, gpu::LaunchOp launchOp, PatternRewriter &rewriter) {
  if (isLaunchIndependent(val, launchOp)) return val;
  if (auto constOp = val.getDefiningOp<arith::ConstantOp>())
    return rewriter.create<arith::ConstantOp>(constOp.getLoc(), constOp.getValue());
  return {};
}

static bool isNotIndependentOrConstant(Value val, gpu::LaunchOp launchOp) {
  return !isLaunchIndependent(val, launchOp) && !isa_and_nonnull<arith::ConstantOp>(val.getDefiningOp());
}

static Value createSequentialGpuLoop(Location loc, Value lowerBound, Value upperBound, Value step,
                                     IRMapping &cloningMap, PatternRewriter &rewriter,
                                     SmallVectorImpl<Operation *> &worklist, gpu::LaunchOp launchOp) {
  auto loopOp = rewriter.create<scf::ForOp>(loc, cloningMap.lookupOrDefault(lowerBound),
                                            cloningMap.lookupOrDefault(upperBound), cloningMap.lookupOrDefault(step));
  worklist.push_back(launchOp.getOperation());
  rewriter.setInsertionPointToStart(loopOp.getBody());
  return loopOp.getInductionVar();
}
...
static LogicalResult processParallelLoop(ParallelOp parallelOp, gpu::LaunchOp launchOp, IRMapping &cloningMap,
  ...
  // Verify that this is a valid GPU mapping.
  ...
  // Support reductions.
  if (!mapping || parallelOp.getNumResults() != 0) return failure();
  ...
  for (auto config : llvm::zip(mapping, parallelOp.getInductionVars(), parallelOp.getLowerBound(),
    ...
    if (!annotation) return parallelOp.emitOpError() << "expected mapping attribute for lowering to GPU";
    Value newIndex;
    gpu::Processor processor = annotation.getProcessor();
    if (!isMappedToProcessor(processor)) {
      newIndex = createSequentialGpuLoop(loc, lowerBound, upperBound, step, cloningMap, rewriter, worklist, launchOp);
    ...
    newIndex =
      rewriter.create<AffineApplyOp>(loc, annotation.getMap().compose(lowerAndStep),
                                     ValueRange{operand, ensureLaunchIndependentValue(step, launchOp, rewriter),
                                                ensureLaunchIndependentValue(lowerBound, launchOp, rewriter)});
    // If there was also a bound, insert that, too.
    // Check that we do not assign bounds twice.
    ...
    if (isNotIndependentOrConstant(lowerBound, launchOp)) return failure();
    if (isNotIndependentOrConstant(step, launchOp)) return failure();
    bool boundIsPrecise = !isNotIndependentOrConstant(upperBound, launchOp);
    {
      ...
      AffineMap stepMap = AffineMap::get(
        1, 2,
        ((rewriter.getAffineDimExpr(0) - rewriter.getAffineSymbolExpr(0)).ceilDiv(rewriter.getAffineSymbolExpr(1))));
      Value launchBound = rewriter.create<affine::AffineApplyOp>(
        loc, annotation.getBound().compose(stepMap),
        ValueRange{ensureLaunchIndependentValue(cloningMap.lookupOrDefault(upperBound), launchOp, rewriter),
                   ensureLaunchIndependentValue(cloningMap.lookupOrDefault(lowerBound), launchOp, rewriter),
                   ensureLaunchIndependentValue(cloningMap.lookupOrDefault(step), launchOp, rewriter)});
  ...
  auto reverse = llvm::reverse(body->without_terminator());
  std::transform(reverse.begin(), reverse.end(), std::back_insert(worklist), [](Operation &op) { return &op; });
...
LogicalResult ParallelToGpuLaunchLowering::matchAndRewrite(ParallelOp parallelOp, PatternRewriter &rewriter) const {
  ...
  if (parallelOp->getParentOfType<ParallelOp>()) return failure();
  ...
  if (failed(processParallelLoop(parallelOp, launchOp, cloningMap, worklist, launchBounds, rewriter))) return failure();
  ...
  while (!worklist.empty()) {
    ...
    // This is only correct if there either is no further scf.parallel
    ...
    if (auto nestedParallel = dyn_cast<ParallelOp>(op)) {
      ...
      if (seenSideeffects) return failure();
      ...
      // Handle region side effects properly.
      ...
      if (seenSideeffects && leftNestingScope) return failure();
    }
  }
