"""Path manager module for handling profiling paths."""
import os
import re


class PathManager:
    """Manager for profiling-related paths."""

    @classmethod
    def get_cann_path(cls, profiler_path: str) -> str:
        """Get CANN profiling path from profiler directory."""
        ...
        for sub_dir in sub_dirs:
            ...
            if os.path.isdir(sub_path) and (
                re.match(r"^PROF_\d+_\d+_\d+_[0-9a-zA-Z]+", sub_dir) or
                re.match(r"^PROF_\d+_\d+_[0-9a-zA-Z]+", sub_dir)
            ):
                return sub_path
        return ""
    ...
    def get_start_info_path(cls, cann_path: str) -> str:
        """Get start info file path from CANN directory."""
    ...
    def get_device_all_file_list_by_type(cls, profiler_path: str, summary_or_timeline: str) -> list:
        """Get all files from device directory by type (summary or timeline)."""
        file_list = []
    ...
    def get_analyze_all_file(cls, profiler_path: str, analyze: str) -> list:
        """Get all files from analyze directory."""
        file_list = []
