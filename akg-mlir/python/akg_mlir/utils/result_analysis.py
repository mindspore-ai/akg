...
def result_compare(actual, bench_mark, r_tol=5e-3):
    ...
    if continue_err > max_continue:
        max_continue = continue_err
        max_end = last_err
    logging.debug("error num: %d/%d (%.2f%%)", error,
                  count, 100.0 * error / count)
    logging.debug("longest error range: [%d, %d]",
                  max_end - max_continue + 1, max_end)
    if max_continue >= 16:
        return False
    logging.debug("\n\n******************** test ok *****************\n\n")
    return True
...
def np_bisect_sum(inputs, axis=None, keepdims=True):
    ...
    for dim in axis:
        size = size * shape[dim]
    if size <= 2:
        expect = np_bisect_sum_fp16(
            inputs, axis=tuple(axis), keepdims=keepdims)
    else:
        expect = np.sum(inputs.astype("float32"), axis=tuple(
            axis), keepdims=keepdims).astype("float16")
    return expect
    ...
    for i, tensor in enumerate(tensors):
        ...
        while len(stack) > 0:
            ...
            if cur_tensor.dtype == "float16" and _precision_reduce(cur_tensor):
                logging.info("%s --> %s", str(tensor), str(cur_tensor))
...
def get_compare_tolerance(json_str: str, output_indexes: list):
    """Get Tolerance From the Json Str"""
    ...
    for input_desc in desc["input_desc"] if desc["input_desc"] is not None else []:
        io.append(
            IOInfo(input_desc[0]["tensor_name"], input_desc[0]["data_type"]))
    ...
    cast_from_fp16 = _precision_analyze(desc, analyze_outputs)
