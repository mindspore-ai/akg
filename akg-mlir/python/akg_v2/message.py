...
def get_kernel_meta_path():
    ...
    return os.path.join(
        os.path.realpath(os.getenv('MS_COMPILER_CACHE_PATH',
                                   str(pathlib.Path(__file__).absolute().parent))), kernel_meta_dir)
...
class AkgV2Driver(object):
    ...
    def __init__(self,
        ...
        with open(input_file, 'r') as f:
            ...
            if kernel_info.get("target_info"):
                compute_capability = kernel_info.get(
                    "target_info").get("compute_capability", "7.0")
    ...
    def compile(self):
        ...
        with os.fdopen(os.open(self.input_file, os.O_RDONLY, 0o755), 'r') as f_info:
            ...
            if self.backend == "cuda" and self.dynamic_shape and _is_single_op(
                    kernel_info) and "Reduce" not in self.kernel_name:
                return
        if self.backend == "cpu":
            self.run_cpu()
    ...
    def run_gpu(self):
        ...
        def _gen_static_tile_kernel(kernel_name):
            try:
                ...
                sub_input_file = os.path.join(
                    self.output_dir, sub_kernel_name + ".info")
                ...
                if self.log_level == "ERROR":
                    raise RuntimeError(
                        f"Compile error, kernel: {sub_kernel_name} is not generated") from exc
        ...
        try:
            ...
            if self.log_level == "ERROR":
                raise RuntimeError(
                    f"Compile error, kernel: {self.kernel_name} is not generated") from exc
    ...
    def _run_mlir_convert(self, kernel_name=None, input_file=None):
        ...
        cmd = [os.path.join(self.akg_tools_dir, 'bin/mindspore-translate'), "-json-to-mindspore", input_file,
               "-o", out_file]
        try:
    ...
    def _run_mlir_to_llvm(self, kernel_name):
        ...
        if self.profiling_trails > 0:
            input_file = wrap_timer_func(
                input_file, self.kernel_name, self.profiling_trails)
        out_file = os.path.join(self.output_dir, kernel_name + ".ll")
        cmd = [os.path.join(self.llvm_tools_dir, 'bin/mlir-translate'),
               input_file, "--mlir-to-llvmir", "-o", out_file]
        try:
            subprocess.run(cmd, check=True, capture_output=True)
        except subprocess.CalledProcessError:
            raise RuntimeError(
                'mlir to llvm failed in case: ' + input_file + '!\n')
    ...
    def _run_cpu_generate_binary(self, kernel_name):
        ...
        cmd = [os.path.join(self.bisheng_tools_dir, 'bin/llc'), input_file, "-relocation-model=pic", "-O3",
               "-opaque-pointers", "-o", out_file]
        try:
            subprocess.run(cmd, check=True, capture_output=True)
        except subprocess.CalledProcessError:
            raise RuntimeError(
                'generate .s failed in case ' + input_file + '!\n')

        cmd = [os.path.join(self.bisheng_tools_dir, 'bin/clang++'), out_file, "--rtlib=compiler-rt", "-fopenmp", "-O3",
               "--shared", "-fPIC", "-o", bin_file, "-L",
               os.path.join(self.bisheng_tools_dir, 'lib/'), "-lmlir_c_runner_utils"]
        if self.runtime_provider == "MLIR":
            cmd.extend(["-L", os.path.join(self.akg_tools_dir,
                       'lib/'), "-lmlir_akgParallelLaunch_runtime"])
        if self.profiling_trails > 0:
            cmd.extend(
                ["-L", os.path.join(self.llvm_tools_dir, 'lib/'), "-lmlir_runner_utils"])
        try:
            subprocess.run(cmd, check=True, capture_output=True)
        except subprocess.CalledProcessError:
            raise RuntimeError(
                'generate .so failed in case ' + input_file + '!\n')
    ...
    def _run_mlir_gpu_pipeline(self, dyn_shape, kernel_name, tiling_mode=None):
        ...
        cmd = [self.akg_tools_dir + "bin/akg-opt",
    ...
    def _run_mlir_gpu_codegen(self, kernel_name):
        ...
        cmd = [os.path.join(self.akg_tools_dir, 'bin/akg-opt'),
               input_file, "--gpu-codegen", "-o", out_file]
        try:
            subprocess.run(cmd, check=True, capture_output=True)
        except subprocess.CalledProcessError:
            raise RuntimeError(
                'gpu_codegen failed in case: %s' + kernel_name + '!\n')
    ...
    def _run_gpu_translate(self, kernel_name):
        ...
        cmd = [os.path.join(self.akg_tools_dir, 'bin/akg-translate'), "-gen-ptx", "-arch=sm_70", input_file,
               "--kernel-name=" + out_prefix]
        try:
            subprocess.run(cmd, check=True, capture_output=True)
        except subprocess.CalledProcessError:
            raise RuntimeError(
                'mlir to ptx failed in case: ' + kernel_name + '!\n')
    ...
    def _run_ptx_replace(self, dyn_shape, kernel_name):
        ...
        shape_arg_file = os.path.join(
            self.output_dir, kernel_name + "_shape_arg.txt")
        cmd = [os.path.join(self.akg_tools_dir, 'bin/akg-ptx-replace'),
               input_file, shape_arg_file, out_file]
        if dyn_shape:
        ...
        try:
            ...
            raise RuntimeError(
                'ptx replacement failed in case: ' + input_file + '!\n')
    ...
    def _run_ptx_dump_json(self, dyn_shape, kernel_name):
        ...
        if dyn_shape:
            shape_info_json = os.path.join(
                self.output_dir, kernel_name + "_shape_info.json")
            if not os.path.exists(shape_info_json):
                raise RuntimeError("Dynamic shape needs file {} to get the device shape. Otherwise, the result may be \
                                    incorrect.".format(shape_info_json))
            ...
            if len(title_dict[RUNTIME_VARS]) > 0:
                self._dump_static_tile_kernel_impl(
                    kernel_name, title_dict, out_file)
    ...
    def _dump_static_tile_kernel_impl(self, kernel_name, title_dict, out_file):
        static_json_file = os.path.join(
            self.output_dir, kernel_name + "_static.json")
        static_ptx_file = os.path.join(
            self.output_dir, kernel_name + "_static.ptx")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-akg-tools-dir", type=str,
                        help="akg-mlir tools build dir.", default="")
    parser.add_argument("-llvm-tools-dir", type=str,
                        help="llvm tools build dir", default="")
    parser.add_argument("-bisheng-tools-dir", type=str,
                        help="bisheng cpp tools build dir", default="")
    parser.add_argument("-d", "--dynamic-shape", type=bool,
                        help="Specifies dynamic shape or not", default=False)
    args = parser.parse_args()
    logging.info(args)

    driver = AkgV2Driver(input_file=args.f, output_dir=args.o, akg_tools_dir=args.akg_tools_dir,
                         llvm_tools_dir=args.llvm_tools_dir, bisheng_tools_dir=args.bisheng_tools_dir,
