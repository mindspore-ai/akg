...
void VectorizationState::getScalarValueReplacementsFor(ValueRange inputVals, SmallVectorImpl<Value> &replacedVals) {
  for (Value inputVal : inputVals) replacedVals.push_back(valueScalarReplacement.lookupOrDefault(inputVal));
}
...
static Operation *vectorizeAffineForOp(AffineForOp forOp, VectorizationState &state) {
  ...
  if (!isLoopVecDim) {
    for (auto operand : forOp.getInits()) vecIterOperands.push_back(vectorizeOperand(operand, state));
  } else {
    ...
    for (auto redAndOperand : llvm::zip(reductions, forOp.getInits())) {
      vecIterOperands.push_back(
        createInitialVector(std::get<0>(redAndOperand).kind, std::get<1>(redAndOperand), state));
    }
  }
...
static Operation *widenOp(Operation *op, VectorizationState &state) {
  ...
  for (Value result : op->getResults())
    vectorTypes.push_back(VectorType::get(state.strategy->vectorSizes, result.getType()));
...
static void vectorizeLoops(Operation *parentOp, DenseSet<Operation *> &loops, ArrayRef<int64_t> vectorSizes,
