...
class OpRegister {
 public:
  OpRegister(const std::string &newName, const CreatorFunc &func) : name(newName) {
    OpRegistry::getInstance().registerOpFunc(name, func);
  }
  ~OpRegister() = default;
...
void initAllLiteOps() {
  ...
  // reshape ops
  static bool liteOpsReged = false;
  if (liteOpsReged) {
    return;
  }
  liteOpsReged = true;
  ...
  OP_REGISTER("ReduceAny", ReduceOp);
  OP_REGISTER("ReduceAll", ReduceOp);
  ...
  OP_REGISTER("Slice", OpaqueOp);
  OP_REGISTER("SquareSum", OpaqueOp);
  ...
  OP_REGISTER("Const", OpaqueOp);
  OP_REGISTER("Split", OpaqueOp);
  // virtual ops
