...
static bool buildCone(Operation *rootOp,
                      const DenseSet<Operation *> &sliceSet,
                      func::FuncOp func,
...
computeIterSpaceExtents(linalg::GenericOp op, OpBuilder &b, Location loc) {
...
static SliceSpec computeOperandSlice(linalg::GenericOp op,
                                     const SliceSpec &outSlice,
                                     OpOperand *operand,
  ...
  SmallVector<OpFoldResult> iterExtents =
      computeIterSpaceExtents(op, b, loc);
  ...
  for (unsigned d = 0; d < inMap.getNumResults(); ++d) {
    ...
    if (auto dimE = dyn_cast<AffineDimExpr>(e)) {
      ...
      if (ShapedType::isDynamic(shape[d])) {
        in.sizes.push_back(
            b.createOrFold<tensor::DimOp>(loc, operand->get(), d));
      } else {
...
static Value buildNewInit(linalg::GenericOp gen,
                          const SliceSpec &outSlice,
  ...
  return b.create<tensor::ExtractSliceOp>(
      loc, origInit, outSlice.offsets, outSlice.sizes, outSlice.strides);
}

/// Recursively materialize a sliced version of `val`.
static Value materializeSliced(Value val, const SliceSpec &slice,
                               const DenseSet<Operation *> &cone,
  ...
  if (!def || !cone.count(def)) {
    ...
    result = b.create<tensor::ExtractSliceOp>(
        loc, val, slice.offsets, slice.sizes, slice.strides);
  } else {
    ...
    for (OpOperand *opnd : gen.getDpsInputOperands()) {
      ...
      Value newIn =
          materializeSliced(opnd->get(), inSlice, cone, cache, b, loc);
    ...
    Value newInit =
        buildNewInit(gen, slice, newOutType, dynSizes, b, loc);
    ...
    auto newGen = b.create<linalg::GenericOp>(
        loc, TypeRange{newOutType}, newInputs, ValueRange{newInit},
...
struct HoistTensorSlice : public impl::HoistTensorSliceBase<HoistTensorSlice> {
 public:
  ...
  void runOnOperation() override {
    ...
    funcOp.walk([&](tensor::ExtractSliceOp es) {
      bySource[es.getSource()].push_back(es);
    });
    ...
    for (auto &kv : bySource) {
      ...
      for (tensor::ExtractSliceOp es : slices) {
        ...
        DenseMap<Value, Value> cache;
        Value newVal = materializeSliced(source, outSlice, cone, cache, b,
                                         es.getLoc());
      ...
      llvm::sort(coneVec, [](Operation *a, Operation *b) {
