...
def MaxPoolWithArgmax(x, pad_mode="valid", window=1, pad=0, stride=1, target=utils.CCE):
    ...
    return nn.MaxPoolWithArgmax(x, kernel, stride_, strategy, target)
...
def Conv2DBackpropInput(out_backprop, input_sizes, filter, filter_shape, pad_list, stride=1, dilation=1, target=utils.CCE):
