...
def FusedBnGrad1(dy, data, mean, target=utils.CCE):
...
def FusedBnGrad2(dgamma_red_hw, dbeta_red_hw, var, gamma, eps, data_shape, target=utils.CCE):
...
@utils.check_input_type(akg.tvm.tensor.Tensor, akg.tvm.tensor.Tensor,
                          akg.tvm.tensor.Tensor, akg.tvm.tensor.Tensor,
                          akg.tvm.tensor.Tensor, (str, type(None)))
def FusedBnGrad3(dy, rs, dgamma_dx, dbeta_dx, data_minus_mean, target=utils.CCE):
    ...
    dx_dbeta = akg.tvm.compute(shape,
                               lambda *i:
                               akg.lang.ascend.vmadd(dy(*i), rs(*map_index(i)), dbeta_dx(*map_index(i))),
                               name="dx_dbeta")
    dx = akg.tvm.compute(shape,
                         lambda *i:
                         akg.lang.ascend.vmla(dx_dbeta(*i), data_minus_mean(*i), dgamma_dx(*map_index(i))),
                         name="bn_grad_dx")
    if ori_dtype == "float16":
