# Ascend 910B3 NPU硬件规格说明

## 1. 硬件架构

**芯片配置:**
- 20个AI Core，每个包含: 2个VEC(向量计算单元) + 1个CUBE(矩阵计算单元) + 1个SU(标量计算单元)
- 整个芯片有40个VEC，20个CUBE，20个SU
...
|---------|------|---------|------|------|
| GM | 64GB | 全设备 | - | 设备主存储 |
| L2 Cache | 192MB | 20个AI Core | - | 自动缓存 |
...
**并行:** 20个AI Core按需并行分配任务，CUBE与VEC可同时执行不同任务
