...
class CoderOnlyTask(TaskBase):
    ...
    async def run(self, action_type=None, init_parsed_code=ParsedCode()) -> Tuple[str, bool]:
        """
        运行CoderOnlyTask，执行coder然后直接验证。

        Args:
            action_type: 动作类型，默认为None
        ...
        try:
            ...
            try:
                ...
                if self.impl_type == "triton":
                    parsed_code = ParsedCode(triton_code=parsed_result.code)
                elif self.impl_type == "swft":
                    parsed_code = ParsedCode(swft_code=parsed_result.code)
                else:
            ...
            try:
                ...
                logger.info(f"Verification result: {verify_res}")
                return self.op_name, verify_res
