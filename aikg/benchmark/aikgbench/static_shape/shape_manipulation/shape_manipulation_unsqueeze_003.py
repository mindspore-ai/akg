...
def get_inputs():
    # Unsqueezing adds a new dimension at the last position to get (64, 32, 3, 1)
    input_tensor = torch.randn(64, 32, 3, dtype=torch.float16)
    return [input_tensor]


def get_init_inputs():
    # Specific dim value for unsqueeze
    dim = -1  # Insert dimension at the last position
    return [dim]
