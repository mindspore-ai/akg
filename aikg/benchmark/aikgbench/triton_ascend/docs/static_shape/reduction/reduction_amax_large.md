...
**任务特点**：操作类型为归约最后一根轴的reduce，reduce轴的每一步计算都依赖于前一步的结果，所以常规情况下不能并行。该任务的非reduce轴并行处理，reduce轴较大，进行向量化，由于UB大小有限，需要对reduce轴进行切分。
...
```
**优化内容**：利用计算重组的思想，通过改变计算顺序和数据结构来提升性能。优化前每次循环都要对data_block进行行归约，归约操作相对耗时，重复次数较多时，严重影响性能，优化后利用curr_max保持矩阵结构，维护中间结果，减少归约次数，从而减少计算开销并提升性能。
