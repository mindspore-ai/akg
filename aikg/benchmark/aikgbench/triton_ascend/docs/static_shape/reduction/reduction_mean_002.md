...
**数据尺寸**：(1024, 4096) -> reduce轴中等，非reduce轴中等
...
```
**优化内容**：利用计算重组的思想，通过改变计算顺序和数据结构来提升性能。优化前每次循环都要对block_vals进行行规约，规约操作相对耗时，重复次数较多时，严重影响性能，优化后利用col_sum保持矩阵结构，维护中间结果，减少规约次数，从而减少计算开销并提升性能。
