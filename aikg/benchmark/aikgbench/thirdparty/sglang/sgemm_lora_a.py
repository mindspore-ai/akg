# SGLang参考信息
# ============================================================================
# 源文件: python/sglang/srt/lora/triton_ops/sgemm_lora_a.py
# 测试文件: 无独立测试文件
# SGLang API调用:
#   sgemm_lora_a_fwd(x, weights, batch_info, stack_num=1)
# Triton Kernel:
#   sgemm_lora_a_kernel - 这个内核函数主要实现计算LoRA A矩阵的乘积。
# ============================================================================
...
def sgemm_lora_a_fwd(
    ...
    grid = (
        triton.cdiv(max_len, BLOCK_S) * triton.cdiv(R, BLOCK_R),
        bs,
    )
...
class ModelSglang(nn.Module):
    ...
    def forward(
        ...
        return sgemm_lora_a_fwd(x, weights, batch_info, stack_num)
...
def get_inputs():
    ...
    # weights (qkv_lora_a): (num_lora, stack_num * r, input_dim) - LoRA A weights
    # 假设有 2 个 LoRA adapters
    num_loras = 2
    weights = torch.randn(
        num_loras, stack_num * rank, input_dim, dtype=torch.float16
    )
