...
# 测试文件: test/srt/test_triton_attention_kernels.py
# SGLang API调用:
#   extend_attention_fwd(q_extend, k_extend, v_extend, o_extend, k_buffer, v_buffer,
#       qo_indptr, kv_indptr, kv_indices, custom_mask, is_causal, mask_indptr, max_len_extend)
# Triton Kernel:
#   _fwd_kernel - extend attention
# ============================================================================
...
def _fwd_kernel(
    ...
    qo_indptr, kv_indptr, kv_indices, mask_ptr, mask_indptr, sink_ptr, window_kv_offset_ptr,
    sm_scale, kv_group_num,
    stride_qbs, stride_qh, stride_kbs, stride_kh, stride_vbs, stride_vh,
    stride_obs, stride_oh, stride_buf_kbs, stride_buf_kh, stride_buf_vbs, stride_buf_vh,
    SLIDING_WINDOW_SIZE: tl.constexpr, logit_cap: tl.constexpr, xai_temperature_len: tl.constexpr,
    Lq: tl.constexpr, Lv: tl.constexpr, BLOCK_DMODEL: tl.constexpr, BLOCK_DPE: tl.constexpr,
    BLOCK_DV: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr,
    USE_CUSTOM_MASK: tl.constexpr, IS_CAUSAL: tl.constexpr, SKIP_PREFIX_CUSTOM_MASK: tl.constexpr,
    STORE_TRANSPOSE: tl.constexpr, HAS_SINK: tl.constexpr,
):
    ...
    if BLOCK_DPE > 0:
        ...
        offs_qpe = (
            (cur_seq_extend_start_idx + cur_block_m * BLOCK_M + offs_m[:, None])
            * stride_qbs + cur_head * stride_qh + offs_dpe[None, :]
        )
    ...
    for start_n in range(0, cur_seq_len_prefix, BLOCK_N):
        ...
        if USE_CUSTOM_MASK and not SKIP_PREFIX_CUSTOM_MASK:
            custom_mask = tl.load(
                ...
                + (cur_block_m * BLOCK_M + offs_m[:, None]) * (cur_seq_len + window_kv_offset)
                + window_kv_offset + start_n + offs_n[None, :],
                mask=(mask_m[:, None] & mask_n[None, :]), other=0,
            )
            final_mask &= custom_mask

        if SLIDING_WINDOW_SIZE > 0:
            window_mask = (cur_seq_len_prefix + cur_block_m * BLOCK_M + offs_m[:, None]) <= (
    ...
    cur_block_m_end = cur_seq_len_extend if not IS_CAUSAL else tl.minimum(cur_seq_len_extend, (cur_block_m + 1) * BLOCK_M)
    for start_n in range(0, cur_block_m_end, BLOCK_N):
        ...
        if USE_CUSTOM_MASK:
            custom_mask = tl.load(
                mask_ptr + cur_seq_mask_start_idx
                + (cur_block_m * BLOCK_M + offs_m[:, None]) * (cur_seq_len + window_kv_offset)
                + window_kv_offset + cur_seq_len_prefix + start_n + offs_n[None, :],
                mask=(mask_m[:, None] & mask_n[None, :]), other=0,
            )
        ...
        if SLIDING_WINDOW_SIZE > 0:
            window_mask = (cur_block_m * BLOCK_M + offs_m[:, None]) <= (start_n + offs_n[None, :] + SLIDING_WINDOW_SIZE)
            final_mask &= window_mask
    ...
    offs_o = (
        (cur_seq_extend_start_idx + cur_block_m * BLOCK_M + offs_m[:, None])
        * stride_obs + cur_head * stride_oh + offs_dv[None, :]
    )
    if STORE_TRANSPOSE:
        tl.store(O_Extend + offs_o.T, (acc / deno[:, None]).T, mask=(mask_m[:, None] & mask_dv[None, :]).T)
    else:
        tl.store(O_Extend + offs_o, acc / deno[:, None], mask=mask_m[:, None] & mask_dv[None, :])


def extend_attention_fwd_impl(
    q_extend, k_extend, v_extend, o_extend, k_buffer, v_buffer,
    qo_indptr, kv_indptr, kv_indices, custom_mask, is_causal, mask_indptr, max_len_extend,
    sm_scale=None, logit_cap=0.0, skip_prefix_custom_mask=True, sliding_window_size=-1,
    sinks=None, window_kv_offsets=None, xai_temperature_len=-1,
):
    ...
    _fwd_kernel[grid](
        q_extend, k_extend, v_extend, o_extend, k_buffer, v_buffer,
        qo_indptr, kv_indptr, kv_indices, custom_mask, mask_indptr, sinks, window_kv_offsets,
        sm_scale, kv_group_num,
        ...
        SLIDING_WINDOW_SIZE=sliding_window_size, logit_cap=logit_cap, xai_temperature_len=xai_temperature_len,
        BLOCK_DMODEL=BLOCK_DMODEL, BLOCK_DPE=BLOCK_DPE, BLOCK_DV=BLOCK_DV, BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N,
        Lq=Lq, Lv=Lv, USE_CUSTOM_MASK=USE_CUSTOM_MASK, IS_CAUSAL=is_causal,
        SKIP_PREFIX_CUSTOM_MASK=SKIP_PREFIX_CUSTOM_MASK, HAS_SINK=HAS_SINK, STORE_TRANSPOSE=_is_hip,
...
class Model(nn.Module):
    ...
    def forward(self, q_extend, k_extend, v_extend, k_buffer, v_buffer, qo_indptr, kv_indptr, kv_indices, max_len_extend):
        o_extend = torch.empty_like(q_extend)
        extend_attention_fwd_impl(
            q_extend, k_extend, v_extend, o_extend, k_buffer, v_buffer,
            qo_indptr, kv_indptr, kv_indices, None, self.is_causal, None, max_len_extend
        )
        return o_extend
...
class ModelSglang(nn.Module):
    ...
    def forward(self, q_extend, k_extend, v_extend, k_buffer, v_buffer, qo_indptr, kv_indptr, kv_indices, max_len_extend):
        """
        调用SGLang extend_attention_fwd API
        """
        ...
        extend_attention_fwd(
            q_extend, k_extend, v_extend, o_extend, k_buffer, v_buffer,
            qo_indptr, kv_indptr, kv_indices, custom_mask=None, is_causal=self.is_causal,
            mask_indptr=None, max_len_extend=max_len_extend
        )
        return o_extend
...
def get_inputs():
    ...
    q_extend = torch.randn(total_extend, num_heads, head_dim, dtype=dtype)
    k_extend = torch.randn(total_extend, num_kv_heads, head_dim, dtype=dtype)
    v_extend = torch.randn(total_extend, num_kv_heads, head_dim, dtype=dtype)
    k_buffer = torch.randn(total_prefix + total_extend, num_kv_heads, head_dim, dtype=dtype)
    v_buffer = torch.randn(total_prefix + total_extend, num_kv_heads, head_dim, dtype=dtype)
    qo_indptr = torch.tensor([0, extend_len, 2 * extend_len], dtype=torch.int32)
    kv_indptr = torch.tensor([0, prefix_len, 2 * prefix_len], dtype=torch.int32)
    kv_indices = torch.arange(total_prefix, dtype=torch.int32)
    max_len_extend = extend_len
    return [q_extend, k_extend, v_extend, k_buffer, v_buffer, qo_indptr, kv_indptr, kv_indices, max_len_extend]
