...
def mean_reduction_over_a_dimension_impl_npu(gm_input, gm_output):
    ...
    for i in range(SAMPLES_PER_CORE):
        ...
        # Calculate mean (sum / DIM1)
        inv_dim1 = Scalar("FP32", 1.0 / DIM1)
        reduce_buf = vmuls(reduce_buf_fp32, inv_dim1)
        
        # Convert back to FP16
        ub_output = vconv(reduce_buf, "FP16")
        
        # Store result back to GM
        insert_to_gm(gm_output, ub_output, [current_batch, 0], [1, DIM2])

def mean_reduction_over_a_dimension_host_run():
    set_context("310P")
    ...
    mean_reduction_over_a_dimension_impl_npu(gm_input, gm_output)
