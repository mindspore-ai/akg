...
@triton.jit
def matmul_with_transposed_both_kernel(
    # 指针参数
    A_ptr: tl.tensor,
    B_ptr: tl.tensor,
    C_ptr: tl.tensor,
    ...
    for k_block in range(0, K, TILE_K):
        ...
        b_tile_trans = tl.trans(b_tile)
        
        # 执行矩阵乘法并累加 (fp16输入, fp32累加)
        acc = tl.dot(a_tile_trans, b_tile_trans, acc)
...
#         # 执行矩阵乘法并累加 (fp16输入, fp32累加)
#         acc = tl.dot(b_tile, a_tile, acc)
    
#     # 转换回fp16并存储结果
#     # acc_trans = tl.trans(acc)
#     c_block_ptr = tl.make_block_ptr(
#         base=C_ptr,
#         shape=(M, N),
#         strides=(stride_cm, stride_cn),
...
def matmul_with_transposed_both_triton_torch(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    """
    Triton 矩阵乘法启动函数 (A.T @ B.T)
    ...
    # 启动内核
    matmul_with_transposed_both_kernel[grid](
