...
def matmul_lowertri_kernel(
    ...
    """Kernel for lower triangular matrix multiplication."""
    pid = tl.program_id(0)
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    pid_m = pid // num_pid_n
    pid_n = pid % num_pid_n

    # Only compute blocks in or below the diagonal (for lower triangular)
    if pid_m < pid_n:
        return
    ...
    for k in range(0, N, BLOCK_SIZE_K):
        ...
        # Load A with proper lower triangular masking (row >= col)
        a_mask = (offs_m[:, None] < N) & (k_offs[None, :] < N) & (offs_m[:, None] >= k_offs[None, :])
        a = tl.load(a_ptrs, mask=a_mask, other=0.0)
        
        # Load B with proper lower triangular masking (row >= col)
    ...
    # Store only lower triangular part (row >= col)
