...
def linear_triton_ascend_torch(x, weight, bias):
    """
    Triton Linear for torch framework (legacy function, kept for compatibility)
    """
    import torch
...
if HAS_MINDSPORE and 'mindspore' in sys.modules:
    ...
    class ModelNew(ms.nn.Cell):
        def __init__(self, in_features, out_features):
            ...
            self.bias = Parameter(linear.bias.clone(), name="bias") if linear.bias is not None else None

        def construct(self, x: torch.Tensor) -> torch.Tensor:
            """
            Triton Linear for MindSpore framework
            """
            batch_size, in_features = x.shape
            ...
            # 分配输出张量
            output = ms.mint.empty((batch_size, out_features), dtype=x.dtype)
