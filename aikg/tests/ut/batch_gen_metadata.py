...
'''
# 使用示例：
python tests/ut/batch_gen_metadata.py --dsl "triton" --framework "torch" --backend "ascend" --arch "ascend910b4" --path "temp"
'''
...
async def insert_one_case(dsl: str, framework: str, backend: str, arch: str, path: str):
    """处理指定架构下的所有算子目录，生成或更新metadata.json"""
    db_system = Database()
    impl_code = get_fixed_suffix_content(suffix=dsl, path=path)
...
async def insert_multi_case(dsl: str, framework: str, backend: str, arch: str, path: str):
    """处理指定架构下的所有算子目录，生成或更新metadata.json"""
    db_system = Database()
...
async def main():
    ...
    parser.add_argument('--dsl', required=True, help='实现类型（如triton、swft等）')
