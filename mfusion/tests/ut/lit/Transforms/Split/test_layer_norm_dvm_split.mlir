...
module {
  // Loose LN ops (sum/mean style) → tag → cluster → split; tag_barrier keeps sqrt(eps) with body.
  ...
  // Post-cluster fused backward IR; split keeps vector prologue and body in separate fused regions.
  ...
  func.func @test_layer_norm_dvm_split_bwd(%x: tensor<2x4x8xf32>, %saved: tensor<2x4x8xf32>,
    ...
    %body = mfuse.fused %x, %rstd, %c8 {fusion_type = "dvm"}
    ...
    ^bb0(%arg0: tensor<2x4x8xf32>, %arg1: tensor<2x1x1xf32>, %arg2: tensor<f32, {is_scalar = ""}>):
      %sum = mfuse.reduce_sum %arg0 {dimensions = [1], keepdim = true, mfusion.layer_norm_dvm}
          : (tensor<2x4x8xf32>) -> tensor<2x1x1xf32>
      %mean = mfuse.div %sum, %arg2 {mfusion.layer_norm_dvm}
          : (tensor<2x1x1xf32>, tensor<f32, {is_scalar = ""}>) -> tensor<2x1x1xf32>
      %mean_bc = mfuse.broadcast_to %mean {mfusion.layer_norm_dvm} : (tensor<2x1x1xf32>) -> tensor<2x4x8xf32>
      %sub = mfuse.sub %arg0, %mean_bc {mfusion.layer_norm_dvm} : (tensor<2x4x8xf32>, tensor<2x4x8xf32>) -> tensor<2x4x8xf32>
      %rstd_bc = mfuse.broadcast_to %arg1 {mfusion.layer_norm_dvm} : (tensor<2x1x1xf32>) -> tensor<2x4x8xf32>
      %div = mfuse.div %sub, %rstd_bc {mfusion.layer_norm_dvm} : (tensor<2x4x8xf32>, tensor<2x4x8xf32>) -> tensor<2x4x8xf32>
      mfuse.yield %div : tensor<2x4x8xf32>
    }
    %vec = mfuse.fused %body, %saved, %rstd, %eps {fusion_type = "dvm"}
    ...
    ^bb0(%arg3: tensor<2x4x8xf32>, %arg4: tensor<2x4x8xf32>, %arg5: tensor<2x1x1xf32>,
      ...
      %add_eps = mfuse.add %arg5, %arg6 {mfusion.layer_norm_dvm}
          : (tensor<2x1x1xf32>, tensor<f32, {is_scalar = ""}>) -> tensor<2x1x1xf32>
      %add_bc = mfuse.broadcast_to %add_eps {mfusion.layer_norm_dvm} : (tensor<2x1x1xf32>) -> tensor<2x4x8xf32>
      %div2 = mfuse.div %arg3, %add_bc {mfusion.layer_norm_dvm} : (tensor<2x4x8xf32>, tensor<2x4x8xf32>) -> tensor<2x4x8xf32>
      %mul_s = mfuse.mul %div2, %arg4 {mfusion.layer_norm_dvm} : (tensor<2x4x8xf32>, tensor<2x4x8xf32>) -> tensor<2x4x8xf32>
      %sum = mfuse.reduce_sum %mul_s {dimensions = [0, 1], keepdim = true, mfusion.layer_norm_dvm}
