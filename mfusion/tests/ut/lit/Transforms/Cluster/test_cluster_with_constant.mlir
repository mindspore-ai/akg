...
// CHECK: %[[CONST:.*]] = mfuse.constant dense<2.000000e+00> : tensor<f64, {is_scalar = ""}>
// CHECK: %[[MUL:.*]] = mfuse.mul %arg0, %[[CONST]]
// CHECK-SAME: : (tensor<4x4xf32>, tensor<f64, {is_scalar = ""}>) -> tensor<4x4xf32>
// CHECK: return %[[MUL]]
...
// CHECK-SAME: %arg0: tensor<4x4xf32>
// CHECK: %[[CONST:.*]] = mfuse.constant dense<0x7FF0000000000000> : tensor<f64, {is_scalar = ""}>
// CHECK: %[[MUL:.*]] = mfuse.mul %arg0, %[[CONST]]
// CHECK-SAME: : (tensor<4x4xf32>, tensor<f64, {is_scalar = ""}>) -> tensor<4x4xf32>
// CHECK: return %[[MUL]]
func.func @test_no_cluster_with_infinity_constant(%arg0: tensor<4x4xf32>) -> tensor<4x4xf32> {
  %0 = mfuse.constant dense<0x7FF0000000000000> : tensor<f64, {is_scalar = ""}>
  %1 = mfuse.mul %arg0, %0 : (tensor<4x4xf32>, tensor<f64, {is_scalar = ""}>) -> tensor<4x4xf32>
  return %1 : tensor<4x4xf32>
}
...
// CHECK: %[[CONST1:.*]] = mfuse.constant dense<2.000000e+00> : tensor<f64, {is_scalar = ""}>
// CHECK: %[[CONST2:.*]] = mfuse.constant dense<3.000000e+00> : tensor<f64, {is_scalar = ""}>
...
func.func @test_cluster_with_multiple_constants(%arg0: tensor<4x4xf32>, %arg1: tensor<4x4xf32>) -> tensor<4x4xf32> {
  %0 = mfuse.constant dense<2.0> : tensor<f64, {is_scalar = ""}>
  %1 = mfuse.constant dense<3.0> : tensor<f64, {is_scalar = ""}>
  %2 = mfuse.mul %arg0, %0 : (tensor<4x4xf32>, tensor<f64, {is_scalar = ""}>) -> tensor<4x4xf32>
  %3 = mfuse.add %2, %1 : (tensor<4x4xf32>, tensor<f64, {is_scalar = ""}>) -> tensor<4x4xf32>
...
// CHECK: ^bb0(%arg2: tensor<4x4xf32>):
// CHECK: %[[CST1:.*]] = mfuse.constant dense<2.000000e+00> : tensor<f64, {is_scalar = ""}>
// CHECK: %[[CST2:.*]] = mfuse.constant dense<3.000000e+00> : tensor<f64, {is_scalar = ""}>
...
// CHECK: ^bb0(%arg2: tensor<4x4xf32>):
// CHECK: %[[CST3:.*]] = mfuse.constant dense<2.000000e+00> : tensor<f64, {is_scalar = ""}>
// CHECK: %[[CST4:.*]] = mfuse.constant dense<3.000000e+00> : tensor<f64, {is_scalar = ""}>
...
func.func @test_cluster_with_multiple_constants_and_users(%arg0: tensor<4x4xf32>, %arg1: tensor<4x4xf32>) -> (tensor<4x4xf32>, tensor<4x4xf32>) {
  ...
  %2 = mfuse.mul %arg0, %0 : (tensor<4x4xf32>, tensor<f64, {is_scalar = ""}>) -> tensor<4x4xf32>
  %3 = mfuse.add %2, %1 : (tensor<4x4xf32>, tensor<f64, {is_scalar = ""}>) -> tensor<4x4xf32>
  %4 = mfuse.mul %arg1, %0 : (tensor<4x4xf32>, tensor<f64, {is_scalar = ""}>) -> tensor<4x4xf32>
  %5 = mfuse.add %4, %1 : (tensor<4x4xf32>, tensor<f64, {is_scalar = ""}>) -> tensor<4x4xf32>
