...
module {
  ...
  func.func @fuse_conv2d_cast(%input: tensor<1x2x4x4xf16>, %weight: tensor<4x2x2x2xf16>) -> tensor<1x4x3x3xf32> {
    %conv = mfuse.aclnn.conv2d %input, %weight {stride = [1, 1], padding = [0, 0], dilation = [1, 1], transposed = false, output_padding = [0, 0], groups = 1 : i64} : (tensor<1x2x4x4xf16>, tensor<4x2x2x2xf16>) -> tensor<1x4x3x3xf16>
    %result = mfuse.cast %conv : (tensor<1x4x3x3xf16>) -> tensor<1x4x3x3xf32>
    // After fusion: cast replaced by conv2d with f32 result; cast eliminated
    // CHECK-NOT: mfuse.cast
    // CHECK: %[[R:.*]] = mfuse.aclnn.conv2d %[[INPUT]], %[[WEIGHT]]{{.*}} : (tensor<1x2x4x4xf16>, tensor<4x2x2x2xf16>) -> tensor<1x4x3x3xf32>
    // CHECK: return %[[R]]
  ...
  func.func @no_fusion_conv_middle_op_then_cast(%input: tensor<1x2x4x4xf16>, %weight: tensor<4x2x2x2xf16>) -> tensor<1x4x3x3xf32> {
    ...
    %result = mfuse.cast %mid : (tensor<1x4x3x3xf16>) -> tensor<1x4x3x3xf32>
    // CHECK: mfuse.aclnn.conv2d
    // CHECK: mfuse.reshape
    // CHECK: mfuse.cast
    return %result : tensor<1x4x3x3xf32>
