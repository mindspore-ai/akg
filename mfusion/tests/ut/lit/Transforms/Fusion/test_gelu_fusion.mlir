...
module {
  ...
  func.func @test_gelu_fusion(%x: tensor<2x4xf32>) -> tensor<2x4xf32> {
    ...
    %pow3 = mfuse.pow %x, %c3 : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
    %c004 = arith.constant dense<4.471500e-02> : tensor<f64>
    %mul_c = mfuse.mul %pow3, %c004 : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
    %add1 = mfuse.add %x, %mul_c : (tensor<2x4xf32>, tensor<2x4xf32>) -> tensor<2x4xf32>
    %c_sqrt = arith.constant dense<7.978846e-01> : tensor<f64>
    %mul_s = mfuse.mul %add1, %c_sqrt : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
    ...
    %half = mfuse.mul %x, %c05 : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
  ...
  func.func @test_no_fusion_wrong_exponent(%x: tensor<2x4xf32>) -> tensor<2x4xf32> {
    ...
    %pow2 = mfuse.pow %x, %c2 : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
    %c004 = arith.constant dense<4.471500e-02> : tensor<f64>
    %mul_c = mfuse.mul %pow2, %c004 : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
    %add1 = mfuse.add %x, %mul_c : (tensor<2x4xf32>, tensor<2x4xf32>) -> tensor<2x4xf32>
    %c_sqrt = arith.constant dense<7.978846e-01> : tensor<f64>
    %mul_s = mfuse.mul %add1, %c_sqrt : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
    %tanh = mfuse.aclnn.tanh %mul_s : (tensor<2x4xf32>) -> tensor<2x4xf32>
  ...
  func.func @test_no_fusion_wrong_coeff(%x: tensor<2x4xf32>) -> tensor<2x4xf32> {
    ...
    %mul_c = mfuse.mul %pow3, %c05_coeff : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
    %add1 = mfuse.add %x, %mul_c : (tensor<2x4xf32>, tensor<2x4xf32>) -> tensor<2x4xf32>
    %c_sqrt = arith.constant dense<7.978846e-01> : tensor<f64>
    %mul_s = mfuse.mul %add1, %c_sqrt : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
    %tanh = mfuse.aclnn.tanh %mul_s : (tensor<2x4xf32>) -> tensor<2x4xf32>
  ...
  func.func @test_gelu_fusion_commutative(%x: tensor<2x4xf32>) -> tensor<2x4xf32> {
    ...
    %pow3 = mfuse.pow %x, %c3 : (tensor<2x4xf32>, tensor<f64>) -> tensor<2x4xf32>
    %c004 = arith.constant dense<4.471500e-02> : tensor<f64>
    %mul_c = mfuse.mul %c004, %pow3 : (tensor<f64>, tensor<2x4xf32>) -> tensor<2x4xf32>
    %add1 = mfuse.add %x, %mul_c : (tensor<2x4xf32>, tensor<2x4xf32>) -> tensor<2x4xf32>
    %c_sqrt = arith.constant dense<7.978846e-01> : tensor<f64>
    %mul_s = mfuse.mul %c_sqrt, %add1 : (tensor<f64>, tensor<2x4xf32>) -> tensor<2x4xf32>
    %tanh = mfuse.aclnn.tanh %mul_s : (tensor<2x4xf32>) -> tensor<2x4xf32>
    %ones = arith.constant dense<1.0> : tensor<f32>
    %add2 = mfuse.add %ones, %tanh : (tensor<f32>, tensor<2x4xf32>) -> tensor<2x4xf32>
    %c05 = arith.constant dense<5.000000e-01> : tensor<f64>
    %half = mfuse.mul %c05, %x : (tensor<f64>, tensor<2x4xf32>) -> tensor<2x4xf32>
    %result = mfuse.mul %half, %add2 : (tensor<2x4xf32>, tensor<2x4xf32>) -> tensor<2x4xf32>
