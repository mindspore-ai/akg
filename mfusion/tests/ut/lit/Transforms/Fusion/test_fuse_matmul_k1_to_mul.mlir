// RUN: mfusion-opt %s --fuse-matmul-k1-to-mul | FileCheck %s
...
module {
  ...
  // Batch matmul (ND) with transpose: (2,1,3) @ (2,4,1), trans_x1/trans_x2 true -> reshape then mul.
  ...
  func.func @batch_matmul_k1_with_trans(%arg0: tensor<2x1x3xf32>, %arg1: tensor<2x4x1xf32>) -> tensor<2x3x4xf32> {
    %0 = mfuse.matmul %arg0, %arg1 {trans_x1 = true, trans_x2 = true} : (tensor<2x1x3xf32>, tensor<2x4x1xf32>) -> tensor<2x3x4xf32>
    // CHECK: mfuse.reshape
    // CHECK: mfuse.reshape
    // CHECK: mfuse.mul
  ...
  // Batch matmul (ND) (2,3,1) @ (2,1,4) no transpose: no reshape.
  ...
  func.func @batch_matmul_k1_no_trans(%arg0: tensor<2x3x1xf32>, %arg1: tensor<2x1x4xf32>) -> tensor<2x3x4xf32> {
    %0 = mfuse.matmul %arg0, %arg1 {trans_x1 = false, trans_x2 = false} : (tensor<2x3x1xf32>, tensor<2x1x4xf32>) -> tensor<2x3x4xf32>
    // CHECK-NOT: mfuse.reshape
