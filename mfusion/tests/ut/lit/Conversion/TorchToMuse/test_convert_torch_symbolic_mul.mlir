// RUN: mfusion-opt %s --convert-torch-to-muse --convert-torch-symbol-to-muse | FileCheck %s
...
module {
  ...
  // CHECK: %[[CAST0:.*]] = builtin.unrealized_conversion_cast %arg1 : !torch.vtensor<[2,?],f32> to tensor<2x?xf32, #muse.symshape<["2", "s10"]>>
  // CHECK: %[[CAST1:.*]] = builtin.unrealized_conversion_cast %arg2 : !torch.vtensor<[2,?],f32> to tensor<2x?xf32, #muse.symshape<["2", "s10"]>>
  // CHECK: %[[SYM:.*]] = torch.symbolic_int "s10" {min_val = 2, max_val = 9223372036854775807} : !torch.int
  // CHECK: %[[MUL:.*]] = muse.mul %[[CAST1]], %[[CAST0]] : (tensor<2x?xf32, #muse.symshape<["2", "s10"]>>, tensor<2x?xf32, #muse.symshape<["2", "s10"]>>) -> tensor<2x?xf32, #muse.symshape<["2", "s10"]>>
  // CHECK: %[[OUT:.*]] = builtin.unrealized_conversion_cast %[[MUL]] : tensor<2x?xf32, #muse.symshape<["2", "s10"]>> to !torch.vtensor<[2,?],f32>
