...
bool isComplexDataType(Operation *op) {
  ...
  if (op->getName().getStringRef() != "muse.cast" || op->getNumOperands() < 1) {
    return false;
  }
...
class DvmSupportChecker {
 ...
 private:
  ...
  void InitializeCheckFunc() {
    ...
    // cast op
    check_func_["muse.cast"] = {cast_check};
    // reduce sum op
    check_func_["muse.reduce_sum"] = {ReduceSumCheck, input_check_first};
    // cmp op
    check_func_["muse.eq"] = {compare_check, input_same_type};
    check_func_["muse.ne"] = {compare_check, input_same_type};
    check_func_["muse.gt"] = {compare_check, input_same_type};
    check_func_["muse.ge"] = {compare_check, input_same_type};
    check_func_["muse.lt"] = {compare_check, input_same_type};
    check_func_["muse.le"] = {compare_check, input_same_type};
    check_func_["muse.is_finite"] = {compare_check, IsFiniteOpCheck};
    // select op
    check_func_["muse.select"] = {SelectOpCheck, [](Operation *op) { return InputCheck(op, {1, 2}); }};
    // int op
    check_func_["muse.add"] = {int_op_check, input_check_all};
    check_func_["muse.sub"] = {int_op_check, input_check_all};
    check_func_["muse.mul"] = {MulOpCheck};
    check_func_["muse.maximum"] = {int_op_check, input_check_all};
    check_func_["muse.minimum"] = {int_op_check, input_check_all};
    check_func_["muse.neg"] = {int_op_check, input_check_all};
    check_func_["muse.abs"] = {int_op_check, input_check_all};
    // TODO: Add Assign check. There is no corresponding op in aten.
    check_func_["muse.broadcast_to"] = {int_op_check, input_check_first};
    // slice op
    check_func_["muse.slice"] = {SliceSupported, input_check_first};
    // TODO: Add StridedSlice check. There is no corresponding op in aten.
    //  matmul op
    check_func_["muse.matmul"] = {MatMulOpCheck, input_check_all};
    check_func_["muse.batch_matmul"] = {MatMulOpCheck, input_check_all};
    // TODO: Add GroupedMatmul check. There is no corresponding op in aten.
    // transpose op
    check_func_["muse.permute"] = {transpose_op_check, input_check_all};
    // collective comm op
    // TODO: Add AllReduce
    check_func_["muse.reshape"] = {ReshapeOpCheck};
  }
  ...
  static bool ReshapeOpCheck(Operation *op) {
    ...
    if (op->getName().getStringRef() == "muse.matmul" || op->getName().getStringRef() == "muse.batch_matmul" ||
        op->getName().getStringRef() == "muse.grouped_matmul") {
  ...
  static bool MatMulShapeCheck(Operation *op) {
    ...
    if (op->getName().getStringRef() == "muse.matmul" && c_shape.back() <= kMinDimSize && c_shape.size() >= 2 &&
        c_shape[c_shape.size() - 2] <= kMinDimSize) {
      return false;
    }
    if (op->getName().getStringRef() == "muse.batch_matmul" && c_shape.size() > 4) {
      return false;
    }
    return true;
  }
...
llvm::DenseSet<llvm::StringRef> DVMCluster::GetClusterableOps() {
  ...
  return llvm::DenseSet<llvm::StringRef>({
    "muse.abs",          "muse.add",
    "muse.broadcast_to", "muse.cast",
    "muse.exp",          "muse.log",
    "muse.maximum",      "muse.minimum",
    "muse.mul",          "muse.neg",
    "muse.pow",          "muse.div",
    "muse.real_div",     "muse.reciprocal",
    "muse.rsqrt",        "muse.sqrt",
    "muse.sub",          "muse.eq",
    "muse.ne",           "muse.gt",
    "muse.ge",           "muse.lt",
    "muse.le",           "muse.logical_and",
    "muse.logical_or",   "muse.logical_not",
    "muse.select",       "muse.assign",
    "muse.reduce_sum",   "muse.is_finite",
    "muse.reshape",      "muse.permute",
    "muse.floor",        "muse.ceil",
    "muse.trunc",        "muse.matmul",
    "muse.batch_matmul", "muse.grouped_matmul",
  });
}
...
std::unique_ptr<Pass> createDVMClusterPass() { return std::make_unique<muse::DVMClusterPass>(); }
