...
#include "mfusion/Dialect/Muse/Transforms/Decompose/OpBuilder.h"
...
class GeluDecomposePattern : public OpRewritePattern<muse::AclnnGeluOp> {
 public:
  using OpRewritePattern<muse::AclnnGeluOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(muse::AclnnGeluOp geluOp, PatternRewriter &rewriter) const override {
    ...
    mlir::muse::OpBuilder builder(rewriter, loc);
...
class TanhDecomposePattern : public OpRewritePattern<muse::AclnnTanhOp> {
 public:
  using OpRewritePattern<muse::AclnnTanhOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(muse::AclnnTanhOp tanhOp, PatternRewriter &rewriter) const override {
    // Get the input tensor
    ...
    if (!tensorType || !tensorType.getElementType().isF32()) {
      llvm::errs() << "Invalid input type for muse.tanh\n";
      return failure();
    }
...
class GeluBackwardDecomposePattern : public OpRewritePattern<muse::AclnnGeluBackwardOp> {
 public:
  using OpRewritePattern<muse::AclnnGeluBackwardOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(muse::AclnnGeluBackwardOp geluBackwardOp, PatternRewriter &rewriter) const override {
...
class SigmoidDecomposePattern : public OpRewritePattern<muse::AclnnSigmoidOp> {
 public:
  using OpRewritePattern<muse::AclnnSigmoidOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(muse::AclnnSigmoidOp sigmoidOp, PatternRewriter &rewriter) const override {
    // Get the input tensor
