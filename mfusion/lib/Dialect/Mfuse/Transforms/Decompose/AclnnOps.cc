...
/// Pattern to fuse aclnn matmul + aclnn.add into mfuse.matmul_with_bias.
/// Converts aclnn.mm/matmul/batch_matmul + aclnn.add (with alpha=1) to mfuse.matmul_with_bias.
...
class ConvertAclnnMatmulAddToMatMulWithBiasPattern : public OpRewritePattern<AclnnAddOp> {
 public:
  ...
  LogicalResult matchAndRewrite(AclnnAddOp addOp, PatternRewriter &rewriter) const override {
    if (!mfuse::isConstOne(addOp.getAlpha())) {
      return failure();
    }
    Value x = addOp.getX();
    Value y = addOp.getY();
...
std::unique_ptr<Pass> createConvertAclnnMatmulToMatmulPass() {
  return std::make_unique<mfuse::ConvertAclnnMatmulToMatmulPass>();
}
