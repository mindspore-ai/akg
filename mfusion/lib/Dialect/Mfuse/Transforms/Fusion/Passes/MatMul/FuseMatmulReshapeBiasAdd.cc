...
static LogicalResult fuseMatmulReshapeBiasAdd(Value addLhs, Value addRhs, Operation *addOp, PatternRewriter &rewriter) {
  ...
  int64_t biasSize =
    std::accumulate(biasShape.begin(), biasShape.end(), kBiasSizeInit, [](int64_t a, int64_t b) { return a * b; });
