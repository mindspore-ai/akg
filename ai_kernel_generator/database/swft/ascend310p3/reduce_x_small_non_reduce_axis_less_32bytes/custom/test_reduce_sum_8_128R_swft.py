...
"""
场景：非reduce轴比较小，小于32字节的时候需要在1个核内处理
"""
BLOCK_DIM = 1
reduce_axis = -1
...
def reduce_sum_op_impl_npu(gm_input, gm_output):
    ...
    ub_input = slice_to_ub(gm_input, [0, 0], slicesize=[rows, cols])
    ...
    insert_to_gm(gm_output, ub_output, [0], slicesize=[rows])
...
def reduce_sum_op_host_run():
    ...
    gm_input = Tensor("GM", "FP16", [8, 128], format="ND", multi_core=False)
    gm_output = Tensor("GM", "FP16", [8], format="ND", multi_core=False)
