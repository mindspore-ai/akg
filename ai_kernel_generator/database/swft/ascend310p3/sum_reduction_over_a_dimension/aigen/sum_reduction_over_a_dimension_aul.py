...
def sum_reduction_over_a_dimension_op_impl_npu(input_np: U.TensorPtr, output_np: U.TensorPtr, tiling: list):
    # 硬编码参数（来自Tiling函数）
    BATCH_SIZE = 16
    DIM1 = 256
    DIM2 = 256
    ...
    for i in U.Pipelined(iterations=SAMPLES_PER_CORE):
        ...
        U.vunary_op(op="vector_dup", dst=reduce_tile, fill_shape=[1, 1, DIM2], fill_value=0.0)
        ...
        for j in U.Pipelined(iterations=DIM1):
            ...
            # 沿axis=1归约求和
            U.vbinary_op(op="add", dst=reduce_tile, src1=reduce_tile, src2=input_fp32_tile)
...
# Tiling函数
def sum_reduction_over_a_dimension_op_impl_tiling(input_np, output_np, tiling):
    # 设置核数（8核并行处理16个batch）
    BLOCK_DIM = 8
    WORKSPACE_SIZE = 0
