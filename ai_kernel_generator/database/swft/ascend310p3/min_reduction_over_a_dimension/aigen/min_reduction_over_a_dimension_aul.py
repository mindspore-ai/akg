...
def min_reduction_over_a_dimension_op_impl_npu(input_np: U.TensorPtr, output_np: U.TensorPtr, tiling: list):
    # 硬编码参数（来自Tiling函数）
    BATCH_SIZE = 16
    DIM1 = 256
    DIM2 = 256
    ...
    for i in U.Pipelined(iterations=SAMPLES_PER_CORE):
        ...
        U.vunary_op(op="vector_dup", dst=output_tile, fill_shape=[1, DIM2], fill_value=65504)
        ...
        for j in U.Pipelined(iterations=DIM1):
            ...
            U.vbinary_op(op="min", dst=output_tile, src1=output_tile, src2=input_tile)
...
# Tiling函数
def min_reduction_over_a_dimension_op_impl_tiling(input_np, output_np, tiling):
    # 设置核数（8核并行处理16个batch）
    BLOCK_DIM = 8
    WORKSPACE_SIZE = 0
