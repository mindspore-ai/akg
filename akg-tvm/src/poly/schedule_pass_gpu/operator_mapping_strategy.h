...
class OperatorMappingStrategy {
 public:
  ...
  isl::schedule_node MapDimToThreadsBlocks(const isl::schedule_node &node, const bool is_insert_marker = true);
 ...
 protected:
  ...
  isl::schedule_node GetTiledOuterBand(const isl::schedule_node &orig_node);
...
class ReduceMappingStrategy : public OperatorMappingStrategy {
 ...
 private:
  ...
  std::string GetReduceMarkerName(isl::schedule_node &orig_node, size_t &n_thread_map);
