...
class String : public NodeRef {
 public:
  ...
  char at(size_t pos) const {
    if (pos < size()) {
      ...
      throw std::out_of_range("air::String index out of bounds");
    }
  }
  ...
  TVM_DEFINE_NOTNULLABLE_OBJECT_REF_METHODS(String, NodeRef, StringObj);
 ...
 private:
  ...
  friend struct air::runtime::ObjectEqual;
};
...
inline String& String::operator=(std::string other) {
  if(!this->defined()){
    ...
    auto data = static_cast<StringObj::FromStd*>(data_.get());
    data->data_container = other;
    data->size = other.size();
    data->data = data->data_container.data();
  }
  return *this;
}
