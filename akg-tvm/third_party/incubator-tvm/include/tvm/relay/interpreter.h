 ...
 * can be produced by a Relay program and are exposed via air::Node's
...
class ClosureNode : public ValueNode {
 public:
  ...
  TVM_DLL static Closure make(air::Map<Var, Value> env, Function func);
...
struct TupleValueNode : ValueNode {
  ...
  void VisitAttrs(air::AttrVisitor* v) { v->Visit("fields", &fields); }

  TVM_DLL static TupleValue make(air::Array<Value> value);
...
struct TensorValueNode : ValueNode {
  ...
  void VisitAttrs(air::AttrVisitor* v) { v->Visit("data", &data); }
