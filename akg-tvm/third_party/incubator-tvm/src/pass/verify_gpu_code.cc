...
class GPUCodeVerifier : public IRVisitor {
 public:
  ...
  void Visit_(const AttrStmt *op) {
    if (op->attr_key == attr::storage_scope) {
      ...
      if (op_value == "local") {
        visited_local_buffers_.insert(op->node.as<air::Variable>());
      } else if (op_value == "shared") {
        visited_shared_buffers_.insert(op->node.as<air::Variable>());
      }
    } else if (op->attr_key == attr::thread_extent) {
      VarExpr var = op->node.as<air::IterVarNode>()->var;
 ...
 private:
  ...
  std::unordered_set<const air::Variable *> visited_local_buffers_;
  std::unordered_set<const air::Variable *> visited_shared_buffers_;
