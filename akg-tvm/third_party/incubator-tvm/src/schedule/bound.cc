...
/*
 * 2019.12.30 - Define new function AlignRootBound.
 */
...
void AlignRootBound(const Stage &stage,
                    std::unordered_map<IterVar, Range> *rmap) {
  CHECK(rmap != nullptr);
  if (!stage->specify_realize_align ||
      stage->attach_type == kInlinedAlready ||
      stage->is_output ||
      stage->op.as<PlaceholderOpNode>()) {
    return;
  }
  for (auto &it : stage->realize_aligns) {
    auto iv = it.first;
    auto min_align = it.second.first;
    auto extent_align = it.second.second;


    auto range = (*rmap)[iv];
    auto min = range->min;
    auto extent = range->extent;
    auto max = min + extent - 1;

    auto aligned_min = ir::Simplify(min/min_align*min_align);
    auto new_extent = max - aligned_min + 1;
    auto aligned_extent = ir::Simplify((new_extent+extent_align-1)/extent_align*extent_align);
    auto aligned_range = Range::make_by_min_extent(aligned_min, aligned_extent);

    (*rmap)[iv] = aligned_range;
  }
}
...
Map<IterVar, Range> InferBound(const Schedule& sch) {
  ...
  for (size_t i = sch->stages.size(); i != 0; --i) {
    ...
    // align root axis bound
    AlignRootBound(stage, &ret);
