...
air::Array<TypeVar> FreeTypeVars(const Expr& expr, const Module& mod) {
  return TypeVarEVisitor(mod).Free(expr);
}

air::Array<TypeVar> FreeTypeVars(const Type& type, const Module& mod) {
  return TypeVarEVisitor(mod).Free(type);
}

air::Array<TypeVar> BoundTypeVars(const Expr& expr, const Module& mod) {
  return TypeVarEVisitor(mod).Bound(expr);
}

air::Array<TypeVar> BoundTypeVars(const Type& type, const Module& mod) {
  return TypeVarEVisitor(mod).Bound(type);
}

air::Array<TypeVar> AllTypeVars(const Expr& expr, const Module& mod) {
  return TypeVarEVisitor(mod).All(expr);
}

air::Array<TypeVar> AllTypeVars(const Type& type, const Module& mod) {
  return TypeVarEVisitor(mod).All(type);
}

air::Array<Var> FreeVars(const Expr& expr) {
  return VarVisitor().Free(expr);
}

air::Array<Var> BoundVars(const Expr& expr) {
  return VarVisitor().Bound(expr);
}

air::Array<Var> BoundVars(const Pattern& pat) {
  return VarVisitor().Bound(pat);
}

air::Array<Var> AllVars(const Expr& expr) {
...
Type TypeSubst(const Type& type, const TypeVar& tvar, const Type& subst) {
  return TypeSubst(type, air::Map<TypeVar, Type>({{tvar, subst}}));
}

Expr TypeSubst(const Expr& expr, const TypeVar& tvar, const Type& subst) {
  return TypeSubst(expr, air::Map<TypeVar, Type>({{tvar, subst}}));
}

Type TypeSubst(const Type& type, const air::Map<TypeVar, Type>& subst_map) {
  return Bind(type, subst_map);
}

Expr TypeSubst(const Expr& expr, const air::Map<TypeVar, Type>& subst_map) {
  class TypeSubstMutator : public ExprMutator, public PatternMutator {
   public:
    explicit TypeSubstMutator(const air::Map<TypeVar, Type>& subst_map) : subst_map_(subst_map) { }
   ...
   private:
    const air::Map<TypeVar, Type>& subst_map_;
  };
