...
class IndexedForwardGraph {
 public:
  ...
  std::unordered_map<const air::Node*, Node*> node_map;
...
class IndexedForwardGraph::Creator : private ExprVisitor {
 ...
 private:
  ...
  void AddNode(const air::Node* key) {
    auto it = graph_.node_map.find(key);
...
class FuseMutator : private ExprMutator {
 ...
 private:
  ...
  Expr MakeNewFunction(GraphPartitioner::Group* group, Type ret_type, Expr body) {
    ...
    func = FunctionSetAttr(func, "Primitive", air::Integer(visitor.has_call));
    return CallNode::make(func, ginfo.arguments, Attrs());
  }

  Array<Expr> GetNewArguments(const air::Array<Expr>& args,
