...
class GraphRuntimeCodegen
    : public ::air::relay::ExprFunctor<std::vector<GraphNodeRef>(const Expr&)> {
 public:
 ...
 protected:
   ...
   */
  std::vector<int64_t> _ShapeToJSON(air::Array<IndexExpr> shape) {
  ...
  virtual PackedFunc GetFunction(const std::string& name,
     ...
     if (name == "init") {
       return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) {
         ...
         void* mod = args[0];
         Map<Integer, air::Target> tmp = args[1];
         TargetsMap targets;
      ...
      return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) {
        ...
        for (const auto &kv : this->output_.params) {
          air::Expr name = ir::StringImm::make(kv.first);
          ret.push_back(name);
        }
        *rv = ret;
      });
...
template <>
struct Handler<std::shared_ptr<air::relay::backend::GraphNode>> {
  inline static void Write(dmlc::JSONWriter* writer,
                           const std::shared_ptr<air::relay::backend::GraphNode>& data) {
    data->Save(writer);
  }
  inline static void Read(dmlc::JSONReader* reader,
                          std::shared_ptr<air::relay::backend::GraphNode>* data) {
    LOG(FATAL) << "Not implemented.";
  }
};
