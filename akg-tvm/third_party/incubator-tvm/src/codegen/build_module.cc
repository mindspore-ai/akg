...
Target CreateTarget(const std::string& target_name,
  ...
  if (target_name == "c" || target_name == "llvm") {
    ...
    t->keys_array.push_back(ir::StringImm::make("cce"));
  } else {
...
struct TVMTargetThreadLocalEntry {
  ...
  std::stack<air::Target> context_stack;
};
...
air::Target Target::Current(bool allow_not_defined) {
  TVMTargetThreadLocalEntry *entry = TVMTargetThreadLocalStore::Get();
...
air::BuildConfig BuildConfig::Current() {
  TVMBuildConfigThreadLocalEntry *entry = TVMBuildConfigThreadLocalStore::Get();
  if (entry->context_stack.size() > 0) {
    return entry->context_stack.top();
  }
...
.set_dispatch<BuildConfigNode>([](const ObjectRef& node, IRPrinter *p) {
  ...
  p->stream << "dump_mlir=" << op->dump_mlir << ", ";
  p->stream << ")";
});
...
.set_body([](TVMArgs args, TVMRetValue* ret) {
  ...
  for (int i = 1; i < args.size(); i += 2) {
    add_lower_pass.push_back(std::make_pair(
      args[i].operator int(),
      args[i + 1].operator air::runtime::PackedFunc()));
  }
...
.set_body([](TVMArgs args, TVMRetValue* ret) {
  ...
  for (auto& tag : tags) {
    tags_vector.push_back(tag.as<air::ir::StringImm>()->value);
  }
