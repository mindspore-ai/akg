...
void GraphRuntime::SetupStorage() {
  ...
  for (const std::string& s_type : attrs_.dltype) {
    vtype.push_back(air::runtime::String2TVMType(s_type));
  }
...
std::pair<std::function<void()>, std::shared_ptr<GraphRuntime::OpArgs> > GraphRuntime::CreateTVMOp(
  ...
  // code.
  air::runtime::PackedFunc pf = module_.GetFunction(param.func_name, true);
