...
std::string GetCustomTypeName(uint8_t type_code) {
  auto f = air::runtime::Registry::Get("_datatype_get_type_name");
...
uint8_t GetCustomTypeCode(const std::string& type_name) {
  auto f = air::runtime::Registry::Get("_datatype_get_type_code");
...
bool GetCustomTypeRegistered(uint8_t type_code) {
  auto f = air::runtime::Registry::Get("_datatype_get_type_registered");
...
int TVMCbArgToReturn(TVMValue* value, int code) {
  API_BEGIN();
  air::runtime::TVMRetValue rv;
  rv = air::runtime::TVMArgValue(*value, code);
  int tcode;
...
// set device api
TVM_REGISTER_GLOBAL(air::runtime::symbol::tvm_set_device)
