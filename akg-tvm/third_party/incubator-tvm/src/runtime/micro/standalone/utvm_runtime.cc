...
void* UTVMRuntimeCreate(const char* json, size_t json_len, void* module) {
  return new air::micro::MicroGraphRuntime(
      std::string(json, json + json_len),
      reinterpret_cast<air::micro::DSOModule*>(module));
}

void UTVMRuntimeDestroy(void* handle) {
  delete reinterpret_cast<air::micro::MicroGraphRuntime*>(handle);
}

void UTVMRuntimeSetInput(void* handle, int index, void* tensor) {
  reinterpret_cast<air::micro::MicroGraphRuntime*>(handle)->SetInput(
      index, reinterpret_cast<DLTensor*>(tensor));
}

void UTVMRuntimeRun(void* handle) {
  reinterpret_cast<air::micro::MicroGraphRuntime*>(handle)->Run();
}

void UTVMRuntimeGetOutput(void* handle, int index, void* tensor) {
  reinterpret_cast<air::micro::MicroGraphRuntime*>(handle)->CopyOutputTo(
      index, reinterpret_cast<DLTensor*>(tensor));
}
void* UTVMRuntimeDSOModuleCreate(const char* so, size_t so_len) {
  return new air::micro::DSOModule(std::string(so, so + so_len));
}

void UTVMRuntimeDSOModuleDestroy(void* module) {
  delete reinterpret_cast<air::micro::DSOModule*>(module);
}
