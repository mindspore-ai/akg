...
*/
inline air::Tensor dense(const air::Tensor& data,
  ...
  auto matmul = air::compute(
    { batch, out_dim },
    [&](Var i, Var j) {
      return air::sum(air::cast(out_dtype, data(i, k)) *
                      air::cast(out_dtype, weight(j, k)), { k });
    }, "tensor", "dense");
  ...
  if (bias.defined()) {
    matmul = air::compute(
      { batch, out_dim },
      [&](Var i, Var j) {
        return matmul(i, j) + air::cast(out_dtype, bias(j));
      }, "tensor", kBroadcast);
  }
