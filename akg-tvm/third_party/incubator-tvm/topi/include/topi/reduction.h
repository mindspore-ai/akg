...
inline Array<IterVar> MakeReduceAxes(const std::vector<int>& real_axis, const Tensor& data) {
  ...
  for (auto i : real_axis) {
    ...
    reduce_axes.push_back(
      air::reduce_axis(Range(0, data->shape[i]), name));
  }
  return reduce_axes;
}
 ...
 * \param func The reduction function eg. air::sum
...
inline Tensor DoCommReduce(const Tensor& data,
  ...
  return air::compute(target_shape, compute, data->op->name + "_red", kCommReduce);
}
...
inline Tensor CommReduceIdx(const Tensor& data,
  ...
  auto temp_idx_val = air::compute(target_shape, compute,
...
inline FCommReduce MakeCommReducer(FCombine fcombine,
  ...
  (Array<Expr> exprs, const Array<IterVar>& axis, Expr* condition) {
    ...
    auto cond = condition != nullptr ? *condition : air::const_true();

    auto combiner = air::ir::CommReducerNode::make(lhs, rhs, result, id_elem);
    Array<Expr> outputs;
    for (size_t i = 0; i < exprs.size(); ++i) {
      outputs.push_back(air::ir::Reduce::make(combiner, exprs, axis, cond, static_cast<int>(i)));
    }
    return outputs;
  };
}

/*! \brief Wrap air::min to ensure we get the correct overload */
inline Expr MinOp(Expr source, Array<IterVar> axis) {
  return air::min(source, axis);
}

/*! \brief Wrap air::max to ensure we get the correct overload */
inline Expr MaxOp(Expr source, Array<IterVar> axis) {
  return air::max(source, axis);  // NOLINT(*)
}

/*! \brief Wrap air::prod to ensure we get the correct overload */
inline Expr ProdOp(Expr source, Array<IterVar> axis) {
  return air::prod(source, axis);  // NOLINT(*)
}
...
inline Tensor sum(const Tensor& data,
                  ...
                  bool keepdims = false,
                  bool atleast1d = true) {
  return CommReduce(data, axis, air::sum, keepdims, atleast1d);
}
...
inline Tensor collapse_sum(const Tensor& data, Array<Expr> target_shape) {
  ...
  return DoCommReduce(data, air::sum, target_shape, reduce_axes, squeeze_axes);
}
...
inline Tensor all(const Tensor& data,
                  ...
                  bool keepdims = false,
                  bool atleast1d = true) {
  return CommReduce(data, axis, air::all, keepdims, atleast1d);
}
...
inline Tensor any(const Tensor& data,
  ...
  return CommReduce(data, axis, air::any, keepdims, atleast1d);
}
...
inline Tensor argmin(const Tensor& data,
  ...
  auto fcombine = [](Array<Var> lhs, Array<Var> rhs) {
    Array<Expr> result;
    result.push_back(air::ir::Select::make(lhs[1] <= rhs[1], lhs[0], rhs[0]));  // idx
    result.push_back(air::ir::Select::make(lhs[1] <= rhs[1], lhs[1], rhs[1]));  // val
    return result;
  };
  auto fidentity = [](std::vector<Type> types) {
    Array<Expr> result;
    result.push_back(air::make_const(types[0], -1));  // idx
