...
inline bool IsConstInt(Expr expr) {
  return
    expr->IsInstance<air::ir::IntImm>() ||
    expr->IsInstance<air::ir::UIntImm>();
}
...
inline int64_t GetConstInt(Expr expr) {
  if (expr->IsInstance<air::ir::IntImm>()) {
    return expr.as<air::ir::IntImm>()->value;
  }
  if (expr->IsInstance<air::ir::UIntImm>()) {
    return expr.as<air::ir::UIntImm>()->value;
  }
 ...
 * \note This is stronger equality check than air::ir::Equal
 *
...
inline bool EqualCheck(Expr lhs, Expr rhs) {
  bool result = air::ir::Equal(lhs, rhs);
  if (!result) {
    Expr zero(0);
    result = air::ir::Equal(air::ir::CanonicalSimplify(lhs-rhs), zero);
  }
  return result;
}
