...
template <typename T>
air::Expr Map(const air::Array<air::Expr>& exprs, T op) {
  CHECK_GE(exprs.size(), 1);
  air::Expr res = exprs[0];
 ...
 */
template <typename T>
inline air::Tensor relu(const air::Tensor& t,
                        T threshold = static_cast<T>(0),
                        std::string name = "T_relu",
                        std::string tag = kElementWise) {
  return air::compute(
      t->shape,
      [&](const air::Array<air::Var>& i) {
        auto threshold_const = air::make_const(t->dtype, threshold);
        return air::max(t(i), threshold_const);
      },
      name,
      tag);
}
...
*/
inline air::Tensor leaky_relu(const air::Tensor& t,
                              double alpha = 0.1,
  ...
  return air::compute(
    ...
    [&](const air::Array<air::Var>& i) {
      auto value = t(i);
      auto calpha = air::make_const(value.type(), alpha);
      return air::ir::Select::make(value > 0, value, value * calpha);
    },
    name,
    tag);
}
 ...
 */
inline air::Tensor prelu(const air::Tensor &x,
                         const air::Tensor &slope,
                         const int axis = 1,
  ...
  return air::compute(x->shape,
                     [&](const air::Array<air::Var> &indices) {
                        auto xval = x(indices);
                        return air::ir::Select::make(
                            xval > 0,
                            xval,
 ...
 *
 */
inline air::Tensor pad(const air::Tensor& t,
                       const air::Array<air::Expr>& pad_before,
                       air::Array<air::Expr> pad_after = air::Array<air::Expr>(),
                       Expr pad_value = Expr(),
  ...
  air::Array<air::Expr> output_shape;
  air::Array<air::Expr> pad_before_int32;
  air::Array<air::Expr> pad_after_int32;
  for (const auto &ele : pad_before) {
    pad_before_int32.push_back(air::cast(air::Int(32), ele));
  }
  for (const auto &ele : pad_after) {
    pad_after_int32.push_back(air::cast(air::Int(32), ele));
  }
  for (size_t i = 0; i < t->shape.size(); ++i) {
    if (i >= pad_before.size()) {
      ...
      output_shape.push_back(
          air::ir::Simplify(t->shape[i] + pad_before_int32[i] + pad_after_int32[i]));
    }
  }

  if (!pad_value.defined()) {
    pad_value = air::make_const(t->dtype, 0);
  }
  auto l = [&](air::Array<air::Var> ovars) {
    air::Array<air::Expr> indices;
    air::Array<air::Expr> sel;
    air::Array<air::Expr> pad_idx;
    for (size_t i = 0; i < t->shape.size(); ++i) {
      ...
      if (!topi::detail::EqualCheck(pad_after_int32[i], 0)) {
        sel.push_back(air::ir::Simplify(ovars[i] < pad_before_int32[i] + t->shape[i]));
      }
      if (pad_mode == "edge") {
        pad_idx.push_back(air::if_then_else(
            ovars[i] < pad_before[i],
            0,
            air::if_then_else(ovars[i] >= pad_before[i] + t->shape[i],
                              t->shape[i] - 1,
                              ovars[i] - pad_before[i])));
    ...
    if (sel.size() != 0) {
      if (pad_mode == "constant") {
        return air::if_then_else(
            detail::Map(sel, air::ir::And::make), t(indices), pad_value);
      } else if (pad_mode == "edge" || pad_mode == "reflect") {
        return air::if_then_else(
            detail::Map(sel, air::ir::And::make), t(indices), t(pad_idx));
      }
    }
    return t(indices);
  };
  return air::compute(output_shape, l, name, tag);
}
 ...
 * layout)
 */
inline air::Tensor conv2d_nchw(const air::Tensor& I,
                               const air::Tensor& W,
                               int pad_h = 0,
                               int pad_w = 0,
                               int stride_h = 1,
  ...
  auto i = air::reduce_axis(air::Range{0, I->shape[1]}, "i");
  auto kh = air::reduce_axis(air::Range{0, W->shape[2]}, "kh");
  auto kw = air::reduce_axis(air::Range{0, W->shape[3]}, "kw");
  auto T = (pad_h == 0 && pad_w == 0)
               ? I
               : pad(I, {air::Expr(0), air::Expr(0), pad_h, pad_w});
  auto l = [&](air::Var b, air::Var o, air::Var h, air::Var w) {
    return air::sum(
 ...
 * (HWCN layout)
 */
inline air::Tensor conv2d_hwcn(const air::Tensor& I,
                               const air::Tensor& W,
                               int pad_h = 0,
                               int pad_w = 0,
                               int stride_h = 1,
  ...
  auto i = air::reduce_axis(air::Range{0, I->shape[3]}, "i");
  auto kh = air::reduce_axis(air::Range{0, W->shape[0]}, "kh");
  auto kw = air::reduce_axis(air::Range{0, W->shape[1]}, "kw");
 ...
 * (NCHW layout)
 */
inline air::Tensor depthwise_conv2d_nchw(const air::Tensor& I,
                                         const air::Tensor& W,
                                         int pad_h = 0,
                                         int pad_w = 0,
                                         int stride_h = 1,
  ...
  auto T = (pad_h == 0 && pad_w == 0)
               ...
               : pad(I, {air::Expr(0), air::Expr(0), pad_h, pad_w});
  auto l = [&](air::Var b, air::Var o, air::Var h, air::Var w) {
    return air::sum(T(b, indexdiv(i, pCM), stride_h * h + kh, stride_w * w + kw) *
...
inline air::Tensor depthwise_conv2d_nhwc(const air::Tensor& I,
                                         const air::Tensor& W,
                                         int pad_h = 0,
                                         int pad_w = 0,
                                         int stride_h = 1,
  ...
  auto T = (pad_h == 0 && pad_w == 0)
               ? I
               : pad(I, {air::Expr(0), pad_h, pad_w, air::Expr(0)});
  auto l = [&](air::Var b, air::Var h, air::Var w, air::Var o) {
    return air::sum(T(b, stride_h * h + kh, stride_w * w + kw, indexdiv(i, pCM)) *
 ...
 * (NCHW layout)
 */
inline air::Tensor group_conv2d_ngchw(const air::Tensor& I,
                                      const air::Tensor& W,
                                      int pad_h = 0,
                                      int pad_w = 0,
                                      int stride_h = 1,
  ...
  auto i = air::reduce_axis(air::Range{0, I->shape[2]}, "i");
  auto kh = air::reduce_axis(air::Range{0, W->shape[3]}, "kh");
  auto kw = air::reduce_axis(air::Range{0, W->shape[4]}, "kw");

  auto T = (pad_h == 0 && pad_w == 0)
               ? I
               : pad(I, {air::Expr(0), air::Expr(0), air::Expr(0), pad_h, pad_w});
  auto l = [&](air::Array<air::Var> args) {
    air::Var b = args[0];
    air::Var g = args[1];
    air::Var o = args[2];
    air::Var h = args[3];
