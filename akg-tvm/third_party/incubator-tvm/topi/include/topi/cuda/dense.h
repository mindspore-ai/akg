...
*/
inline air::Tensor dense_cuda(const Target& target,
...
inline Schedule schedule_dense(const Target &target, const Array<Tensor>& outs) {
  ...
  auto _schedule = [&](const Tensor& dense) {
    ...
    s[out].bind(s[out]->op.as<ComputeOpNode>()->axis[0], air::thread_axis(Range(), "blockIdx.y"));
    s[out].bind(s[out]->op.as<ComputeOpNode>()->axis[1], air::thread_axis(Range(), "blockIdx.x"));

    auto tx = s[dense]->op.as<ComputeOpNode>()->reduce_axis[0];
    auto thread_x = air::thread_axis(Range(), "threadIdx.x");
