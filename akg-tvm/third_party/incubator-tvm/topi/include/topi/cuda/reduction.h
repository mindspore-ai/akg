...
Schedule ScheduleReduce(const Target& target,
  ...
  if (out_stage->op.as<ComputeOpNode>()->axis.size() > 0) {
    ...
    block_x = air::thread_axis(Range(), "blockIdx.x");
    thread_x = air::thread_axis(Range(0, num_thread), "threadIdx.x");
    thread_y = air::thread_axis(Range(0, num_thread), "threadIdx.y");
  } else {
    all_reduce = true;
