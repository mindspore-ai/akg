...
inline Schedule schedule_pool(const Target &target, const Array<Tensor>& outs) {
  ...
  auto _schedule = [&](const Tensor& padded_input, const Tensor& pool) {
    ...
    s[out].bind(bx, air::thread_axis(Range(), "blockIdx.x"));
    s[out].bind(tx, air::thread_axis(Range(), "threadIdx.x"));
    if (detail::contains(s->outputs, pool->op)) {
...
inline Schedule schedule_global_pool(const Target &target, const Array<Tensor>& outs) {
  ...
  auto _schedule = [&](const Tensor& pool) {
    auto num_thread = 8;
    auto block_x = air::thread_axis(Range(), "blockIdx.x");
    auto block_y = air::thread_axis(Range(), "blockIdx.y");
    auto thread_x = air::thread_axis(Range(0, num_thread), "threadIdx.x");
    auto thread_y = air::thread_axis(Range(0, num_thread), "threadIdx.y");
    Tensor out;
    Tensor OL;
    if (detail::contains(s->outputs, pool->op)) {
      out = pool;
