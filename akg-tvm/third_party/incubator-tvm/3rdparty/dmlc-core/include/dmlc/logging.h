...
/*!
 * 2019.12.30 - modify LOG to support AKG LOG
 */
...
#define LOG_INFO dmlc::CustomLogMessage(__FILE__, __LINE__, "INFO")
#else
#define LOG_INFO dmlc::LogMessage(__FILE__, __LINE__)
#endif
#define LOG_ERROR dmlc::CustomLogMessage(__FILE__, __LINE__, "WARNING")
#define LOG_WARNING dmlc::CustomLogMessage(__FILE__, __LINE__, "WARNING")
#define LOG_DEBUG dmlc::CustomLogMessage(__FILE__, __LINE__, "DEBUG")
#define LOG_ERROR_INFO dmlc::CustomLogMessage(__FILE__, __LINE__, "ERROR")
...
class CustomLogMessage {
 public:
  CustomLogMessage(const char* file, int line, const std::string& level) {
    log_stream_ << level << " " << file << ":"
                << line << ": ";
  }
  ~CustomLogMessage() {
    Log(log_stream_.str());
...
#endif
    LOG(ERROR_INFO) << log_stream_.str();
