...
TEST(TestMarkFuseOp, TestCase1) {
  isl::schedule input_sch;
  isl::schedule expect_ouput_sch;
  std::tie(input_sch, expect_ouput_sch) = ScheduleTreeHelper("mark_fuse_op_case").Prepare();
  input_sch = ir::poly::MarkFuseOp().Run(input_sch);
