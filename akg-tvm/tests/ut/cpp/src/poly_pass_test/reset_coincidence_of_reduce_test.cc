      ...
      for (int dim = 0; dim < num_dims; ++dim) {
        ...
        stmt_list.foreach([&scop_info](const isl::pw_aff &stmt) {
          ...
          if (stmt_id.to_str() == "S_2" && isl_pw_aff_involves_dims(stmt.get(), isl_dim_in, 1, 1) &&
              scop_info.analysis_result_.GetReduceTensorInfoMap().count(stmt_id) == 0) {
            std::vector<std::string> reduce_axis_list = {"k0", "k1"};
            ir::poly::ReduceTensorInfo reduce_tensor_info;
            reduce_tensor_info.axis_vec = reduce_axis_list;
            reduce_tensor_info.stmt_map = isl::union_map::empty(isl::space(scop_info.ctx_, 0));
            scop_info.analysis_result_.RecordReduceTensorInfoMap(stmt_id, reduce_tensor_info);
            return;
          }
        });
      }
