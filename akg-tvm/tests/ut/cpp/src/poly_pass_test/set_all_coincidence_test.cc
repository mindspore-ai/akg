...
TEST(TestSetAllCoincidence, TestCase1) {
  isl::schedule input_sch;
  isl::schedule expect_ouput_sch;
  std::tie(input_sch, expect_ouput_sch) = ScheduleTreeHelper("set_all_coincidence_case").Prepare();
  input_sch = ir::poly::SetAllCoincidence().Run(input_sch);
