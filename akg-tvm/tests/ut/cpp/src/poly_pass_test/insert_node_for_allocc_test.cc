...
TEST(TestInsertNodeForAllocC, TestCase1) {
  isl::schedule input_sch;
  isl::schedule expect_ouput_sch;
  std::tie(input_sch, expect_ouput_sch) = ScheduleTreeHelper("insert_node_for_allocc_case").Prepare();
  input_sch = ir::poly::InsertNodeForAllocC().Run(input_sch);
