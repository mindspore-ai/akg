...
# Specific GPU tests
from tests.st.ops.gpu.common_functions.fused_bn_update_mindtricks import fused_bn_update_mindtricks
from tests.st.ops.gpu.common_functions.fused_relu_grad__mindtricks import fused_relu_grad_mindtricks

@pytest.mark.level0
@pytest.mark.platform_x86_gpu_training
@pytest.mark.env_onecard
def test_mindtricks():
    run_mindtricks(["gpu", "composite"])
    return True


########################################################################################################################
pwd = os.path.dirname(os.path.abspath(__file__))
mind_trick_cases_dir = pwd + "/mind-trick_cases"
...
def run_gpu_cases():
    ...
    with open(tricks_dir + "/" + tricks_filenames["fused_bn_update"], "r") as trick:
        fused_bn_update_mindtricks((2048,), poly_sch=True, mind_trick=trick.read())
    with open(tricks_dir + "/" + tricks_filenames["fused_relu_grad"], "r") as trick:
        fused_relu_grad_mindtricks((256, 56, 56, 256), poly_sch=True, mind_trick=trick.read())

    return True

def single_composite_file(input_file, attrs, poly):
...
def mindtrick(operator_path, trick_path):
    if os.path.isfile(operator_path) and os.path.isfile(trick_path):
        ...
        single_composite_file(operator_path, attrs, poly=True)
    return True

def composite_operator(target, with_autogen=True, with_trick=False, without_trick=False, attrs=None):
    operator = composite_operators_dir + "/" + target + ".json"
    ...
    if with_autogen:
        ...
        single_composite_file(operator, current_attrs, poly=True)
    if test_with_trick:
        mindtrick(operator, trick)
    if test_without_trick:
        single_composite_file(operator, attrs, poly=True);

def batch_test_targets(reason, targets, with_autogen=True, with_trick=False, without_trick=False, attrs=None):
    """Quickly test multiple targets using composite_operator()"""
    ...
    for target in targets:
        composite_operator(target, with_autogen, with_trick, without_trick, attrs)
    logging.info("")

    return True

def run_composite_cases(operators):
...
def run_mindtricks(cases):
    if "gpu" in cases:
        run_gpu_cases()
    if "composite" in cases:
        run_composite_cases(composite_targets)
