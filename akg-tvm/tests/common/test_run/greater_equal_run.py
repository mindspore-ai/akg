...
from akg.ops.math import greater_equal
from tests.common.gen_random import random_gaussian
from akg.utils.result_analysis import target_profiling
from akg.utils.format_transform import to_tvm_nd_array

def greater_equal_run(shapes, dtype, kernel_name="greater_equal", attrs_op=None, attrs=None):
    ...
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(greater_equal, shapes, [dtype, dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        mod = utils.op_build_test(greater_equal, shapes, [dtype, dtype], kernel_name=kernel_name, attrs=attrs)
        benchMark, inputs, output = gen_data(shapes)
...
def gen_data(shapes):
    ...
    for i in range(len(shapes)):
        shape = shapes[i]
        input_ = random_gaussian(shape, miu=1, sigma=0.1).astype(np.float16)
