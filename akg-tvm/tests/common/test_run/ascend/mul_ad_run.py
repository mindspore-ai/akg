...
from tests.common.test_op.ascend.mul_ad import mul_ad
...
def mul_ad_run(ashape, bshape, dtype, kernel_name, attrs):
    if 'tuning' in attrs.keys():
        ...
        out = np.multiply(a, b)
        mod = utils.op_build_test(mul_ad, [out.shape, ashape, bshape], [dtype, dtype, dtype], kernel_name=kernel_name, attrs=attrs, tuning=t)
        if t:
        ...
        out = np.multiply(a, b)
        expect, head_np, output = gen_data(b, dtype, out)
        mod = utils.op_build_test(mul_ad, [out.shape, ashape, bshape], [dtype, dtype, dtype], kernel_name=kernel_name, attrs=attrs)
