...
import numpy as np
from tests.common.test_op.ascend.squeeze_ad import squeeze_ad
...
def squeeze_ad_run(shape, axis, dtype, attrs):
    if 'tuning' in attrs.keys():
        ...
        mod = utils.op_build_test(squeeze_ad, [forward_output.shape, shape], [dtype, dtype], kernel_name=kernel_name,
        ...
        input_np = np.random.uniform(low=-1.0, high=1.0, size=shape).astype(dtype)
        forward_output = np.squeeze(input_np, axis=axis)
        mod = utils.op_build_test(squeeze_ad, [forward_output.shape, shape], [dtype, dtype], kernel_name='squeeze_ad',
