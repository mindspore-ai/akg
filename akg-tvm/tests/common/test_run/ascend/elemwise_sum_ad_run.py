...
from tests.common.test_op.ascend.elemwise_sum_ad import elemwise_sum_ad
from tests.common.tensorio import compare_tensor
from tests.common.gen_random import random_gaussian

def elemwise_sum_ad_run(shape, dtype, attrs_op={}, polyhedral=True, attrs={}):
    attrs.update(attrs_op)
    if 'tuning' in attrs.keys():
        t = attrs.get("tuning", False)
        mod = utils.op_build_test(elemwise_sum_ad, [shape, shape, shape], [dtype, dtype, dtype],
                                  kernel_name='elemwise_sum_ad', attrs=attrs, tuning=t)
        if t:
        ...
        expect, head_np, input1, input2, output = gen_data(dtype, shape)
        mod = utils.op_build_test(elemwise_sum_ad, [shape, shape, shape], [dtype, dtype, dtype],
                                kernel_name='elemwise_sum_ad', polyhedral=polyhedral, attrs=attrs)
