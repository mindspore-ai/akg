...
"""run function for quantized_avg_pool"""

import numpy as np

from tests.common.tensorio import compare_tensor
from akg.utils import kernel_exec as utils
from tests.common.test_run.ascend.avgpool_run import benchmark as avgpool_benchmark
from tests.common.gen_random import random_gaussian
from tests.common.base import get_rtol_atol
from tests.common.test_op.ascend.quantized_avg_pool import quantized_avg_pool

def benchmark(data, aided_inputs, ksize, strides, padding, data_format,
              quant_algo, *_):
    """caculate output for quantized avgpool"""
    ...
    out = avgpool_benchmark(data, kernel, stride, padding)
...
def compare_int(output, expect):
    """compare int-dtype result"""
    return len(np.where(abs(output.astype("int32") - expect.astype("int32")) > 1)[0]) == 0
...
def gen_data(shape, dtype1, shape_list, dtype2, ksize, strides, padding,
    ...
    """generate data"""
    data = random_gaussian(shape, miu=1, sigma=0.1).astype(dtype1)
    inputs = [data]
    aided_inputs = []
    if shape_list is not None:
        # Here use the data's avg and min to calculate the requantize scale and
