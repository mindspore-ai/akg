...
def resize_bilinear_grad(resized_data, original_data):
    ...
    utils.check_shape(resized_shape, 4, "resized_data")
    utils.check_shape(original_shape, 4, "original_data")
    utils.check_equal("input batchsize", "output batchsize", resized_shape[0], original_shape[0])
    utils.check_equal("input channel num", "output channel num", resized_shape[3], original_shape[3])
    utils.ops_dtype_check([resized_data.dtype, original_data.dtype], utils.DtypeForDavinci.ALL_FLOAT)
    ...
    resW = akg.lang.ascend.vadd(resW1, resW2)
